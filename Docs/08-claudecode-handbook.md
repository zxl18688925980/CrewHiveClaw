...
| **Main agent（MiniMax M2.7）** | 22 个工具：`get_system_status` / `get_logs` / `read_file` / `restart_service` / `restart_gateway` / `run_shell` / `test_lucas` / `exec_script` / `send_file` / `write_file` / `trigger_finetune` / `evaluate_l0`~`l4` / `evaluate_system` / `evaluate_trend` / `evaluate_local_model` / `scan_pipeline_health` / `scan_lucas_quality` / `inspect_agent_context` / `update_heartbeat` / `log_improvement_task` |

**原则**：能在企业微信里说一句话解决的，不开电脑。Main agent 覆盖了大部分日常运维操作。评估类工具自动附 Web 仪表盘链接，企业微信点开即看图形化趋势分析。
...
### 4.4 系统工程师不替代，只教导——协作链失败的正确响应

**场景**：Andy→Lisa 协作链输出了不完整或错误的结果，业主想尽快解决，Claude Code 倾向直接补全。

**做法**：Claude Code 的职责是「诊断 + 修复协作链」，而不是「替 Lisa 写代码」。正确步骤：
...
4. **修协作链**：改 prompt 模板、改 saveCode 逻辑，让 Lisa 自己重跑出正确结果
5. **验证**：用真实 spec 再跑一次，确认协作链输出正确，不是手工打补丁
...
**为什么有效**：每次直接替 Lisa 写代码，就是在绕过积累过程。每次修协作链，才是在真正让系统变强。
...
**案例**：`member_deep` 意图新增时，`isDevIntent` 把它当成开发任务，导致 wecom-entrance 带着 `callbackUrl` 调用时走了 Andy 的 SE 协作链，而不是同步走成员 Agent 路径。修复：在 `isDevIntent` 里明确排除 `member_deep`。
...
**根因**：系统设计是「Lucas 根据触发条件自主决定什么时候创建成员 Agent」，但 Claude Code 直接替 Lucas 做了这个决策，跳过了协作链。这是**系统工程师越权**。
...
**为什么有效**：慢 Agent 只能在竞争池排队，永远抢不到 Lucas 的专属槽位。Lucas 的对话响应能力和 Andy/Lisa 的协作链彻底解耦。这是「信任崩塌防线」——系统跑不出新特性可以接受，但 Lucas 不响应是致命的。
...
**场景**：家人说了一句模糊的话（"要是有个 XX 就好了"），系统工程师需要判断 Lucas 是否该触发开发协作链。
...
```
架构与设计/
  01-整体架构演进.md     ← OpenClaw集成、V字协作链、per-message session 等
...
- Gateway 日志在 `/tmp/openclaw/openclaw-YYYY-MM-DD.log`，含工具调用链的完整协作链记录
...
**可复用性**：任何需要 Agent 在无交互场景读取结构化数据的场景（定时任务、watchdog 触发、自动化协作链），都应在触发方预计算注入，不期望 Agent 自己 exec。
...
### 写入侧 vs 读取侧：基础设施完成 ≠ 行为完成（2026-04-11）

**现象**：为 Lucas 实现了 `【当前进行中任务】` 注入、followup-queue、HEARTBEAT 任务3/6，但 Lucas 实际上从不主动提及任务状态、pipeline 成功后不给预期、【待告知家人任务】不主动带出。

**根因**：所有建设都在「写入侧」（数据如何进入上下文），没有做「读取侧」（收到这些数据后 Agent 该做什么的 AGENTS.md 行为规则）。数据到了，但模型没有指令驱动去用它。

**修复模式**：每次给 Agent 的 `before_prompt_build` 加新注入块时，必须同步在 AGENTS.md 写对应的触发规则——

> 「当上下文含 `【XXX块】` 时 → 必须/应当 做什么」
