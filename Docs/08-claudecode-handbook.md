...
| **Main agent（GLM-5.1）** | 21 个工具：`get_system_status` / `get_logs` / `read_file` / `restart_service` / `restart_gateway` / `run_shell` / `test_lucas` / `exec_script` / `send_file` / `write_file` / `trigger_finetune` / `evaluate_l0`~`l3` / `evaluate_system` / `evaluate_local_model` / `scan_pipeline_health` / `scan_lucas_quality` / `inspect_agent_context` / `update_heartbeat` / `log_improvement_task` |
