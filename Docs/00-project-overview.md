...
| **L2** | Vibe Anything | 开发即交付 | 系统帮人完成任务的功能能力。**Lucas 主力**——家人说「要是有个XX就好了」，Lucas 感知需求、推演、触发流水线，系统能交付多少种XX；Andy 设计 + Lisa 实现，三角色闭环交付。任务类型覆盖度 + 端到端交付成功率 + 交付物多样性；Coding 只是手段，家人需要的 Any Thing 才是目的                  | 任务类型覆盖度持续扩展；端到端交付成功率提升；交付物多样性增加                      |
| **L3** | 组织协作 | 组织运作越来越优化 | **带着人一起进化**：影子 Agent 驱动三元循环——**理解**（消费 Kuzu 动态协作边，建立成员协作模式理解）→ **增强**（在真实沟通前帮成员想清楚：协调策略/能力缺口/分工建议）→ **演进**（结果信号回流，更新影子「有效/无效」判断），每轮让影子对「这个人在这个组织里」的判断更准；成员层次/权限决定影子深度；成员增强效果检查（inject.md 协作边检测） | 影子对每个成员的增强建议采纳率持续提升（演进环有效）；组织协作效能可量化改善；三层结构（个人/基层组织/能力组织）在架构上均有对应 |
| **L4** | 系统自我演化 | 让 L0~L3 越来越好 | 跨所有层的元能力，让系统的每个层次持续改进。包含两层：①**系统层自我改进**（Andy 主力）——蒸馏、结晶、spec 质量提升、DPO 信号收集、路由阈值进化、Andy 主动巡检系统信号发现能力缺口，无需等外部触发；②**模型层内化**——本地模型微调（DPO+SFT）、行为模式写入权重、数据驱动模型评估。系统工程师通过「修改 Readme（正朔）→ 刷新 Agent 认知」影响系统演化方向，这是 L4 的控制回路                                                | 系统层：进化信号积累 + Skill 积累 + Andy 巡检时效 + 同类错误趋势下降；模型层：本地路由比例持续上升，AGENTS.md 规则条数持续减少                             |

```
L0 = 基础设施·稳不稳：机器能跑，三机制管道接通，软硬件性能达标
L1 = 行为质量·好不好：记忆质量 + 输出质量，每个 Agent 都是有记忆、有理解、有人格的存在
L2 = Vibe Anything·开发即交付：Lucas 感知需求 → Andy 设计 → Lisa 实现，三角色闭环交付
L3 = 组织协作·组织运作越来越优化：带着人一起进化，理解→增强→演进，每轮让判断更准
L4 = 系统自我演化·让 L0~L3 越来越好：系统层自我改进（蒸馏/结晶/Andy 主动巡检）+ 模型层内化（DPO+SFT）
```
...
**L2 = Vibe Anything（帮人做事），L4 = 系统自我演化（自己变强）**。这两个能力层次正交但互补：L2 是系统帮人完成任务的交付力，L4 是系统自我改进的元能力。L2 做得越多，L4 积累的进化信号就越丰富；L4 越强，L2 的交付质量和效率就越高。三角色在两个层次中各有定位：Lucas 是 L2 的需求感知器和交付入口，Andy/Lisa 是 L2 的交付引擎；Andy 是 L4 的核心引擎（主动巡检、设计判断进化），Lisa 是 L4 的信号源（实现反馈、错误信号）。

**Andy 是 L4 自我演化的乘数**。三个角色各有进化边界：Lucas 进化 = 对这个组织理解更深，边界在成员关系的深度；Lisa 进化 = 实现能力更强，边界在代码库复杂度。Andy 的进化没有这个边界——Andy 的每一次判断质量提升，都直接作用于系统里每一条流水线：spec 精度提升一点，Lisa 的成功率就上升；Andy 吸收一条外部知识，下一个需求的设计就更准。**大脑进化快，整个组织就快**——这也是 L4 进化体系的核心投入应向 Andy 倾斜的根本原因。

**Lisa 是 L2 和 L4 的桥梁**。L2 的完整闭环是 Lucas 感知 → Andy 设计 → **Lisa 实现**——没有 Lisa，Vibe Anything 停在 spec 阶段，永远不是"交付"。同时，Lisa 的实现产出（codebase_patterns、错误信号、修复记录）是 L4 中 Andy 巡检的核心输入数据。Andy 发现能力缺口的信号，一半来自 Lisa 的实现过程。**Lisa 同时服务于两个层次：她是 L2 的交付手，也是 L4 的信号源。** 三角色的协作关系：Lucas 提需求 → Andy 设计 → Lisa 交付 + 反馈信号 → Andy 自我改进 → 回到 Lucas。
...
**双维度回流机制**：L2 交付和 L4 自我演化不是独立运转的两条线，而是通过三角色的协作形成闭环。五条已实现的回流路径：
...
| `evaluate_l2`          | Vibe Anything 交付能力：任务类型覆盖度（task-registry 去重）、端到端交付成功率（opencode 近 10 次成功率）、交付物多样性（App/generated/ 文件数）|
...
─────────────────────────────── 增强阶段终点 / 扩张阶段起点
L2  Vibe Anything（开发即交付：Lucas 感知需求 → Andy 设计 → Lisa 实现 → 家人用上）← 以 L1 为土壤
