...
| 层次     | 名称               | 核心目标                                                                                                                                                                   | 完成标志                                                              |
| ------ | ---------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------- |
| **L0** | 基础设施   | 软硬件选型（模型/数据库/部署方式）+ 信息架构设计（Data/Knowledge/Capability 三层 schema 和数据流）；记忆 / 蒸馏 / 上下文注入三个通用机制接通（写入端 + 注入端）                                                                          | 信息流能被捕获持久化；进化信号结构化字段有落地；三机制管道可跑通，即使初始数据稀少                         |
| **L1** | 行为质量 | 三个通用机制真正运转并产生质量：对话记忆可检索（记忆工程）、组织成员理解沉淀为知识（知识蒸馏）、每次响应前动态注入最相关上下文（上下文工程）。系统里每个 Agent 从无状态响应器变成有记忆、懂对方、能以人格自主响应的存在                                                        | 系统里每个 Agent 无论角色，都能记住对方说过的话、理解对方是谁、在对话中展现连贯人格；人感知不到「每次从零开始」的断层    |
| **L2** | 系统自进化               | 系统自己跟自己玩：成熟行为模式自动结晶成 Skill/Tool；蒸馏管道提炼设计判断；跨角色知识持续流动与沉淀                                                                                                                                    | 成熟行为模式能被识别并自动固化；Andy 能自主判断固化形式；角色间知识共享机制正常运转                      |
| **L3** | 组织协作进化         | **带着人一起进化**：影子 Agent 驱动三元循环——**理解**（消费 Kuzu 动态协作边，建立成员协作模式理解）→ **增强**（在真实沟通前帮成员想清楚：协调策略/能力缺口/分工建议）→ **演进**（结果信号回流，更新影子「有效/无效」判断），每轮让影子对「这个人在这个组织里」的判断更准；成员层次/权限决定影子深度 | 影子对每个成员的增强建议采纳率持续提升（演进环有效）；组织协作效能可量化改善；三层结构（个人/基层组织/能力组织）在架构上均有对应 |
| **L4** | 深度学习             | 双路径训练本地模型：路由 SFT（云端处理且无 DPO 的对话 → 本地能力扩张）+ DPO 对齐（任意来源负向信号 → 本地行为矫正）；Kuzu Agent 行为模式天然去标识化后反哺云端；云端 Agent 永久停在 L2 提示控制层 | 本地路由比例持续上升；AGENTS.md 规则条数持续减少（核心量化指标）                           |

```
L0 = 基础设施：软硬件选型 + 信息架构设计 + 三机制管道接通
L1 = 行为质量：系统里每个 Agent 都是有记忆、有理解、有人格的存在
L2 = 系统自进化：系统自己跟自己玩，越用越好
L3 = 组织协作进化：带着人一起进化，理解→增强→演进，每轮让判断更准
L4 = 深度学习：机器真正成为组织一部分
```
