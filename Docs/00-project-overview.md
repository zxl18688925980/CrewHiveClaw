...
> **与 Palantir 的对比**：Palantir 的 Ontology 是同一套哲学的企业版——把组织的原始数据结构化成对象图谱，让 AI 在上面操作。但 Palantir 的 Ontology 需要数据工程师人工构建和维护，他们解决的是「更好地利用数据」。CrewHiveClaw 要解决的是下一个问题：**Ontology 能不能自己从运行数据里生长出来？** 系统每次对话，都在隐式地更新这张图——最终目标是这张图不需要人来维护结构，它自己知道自己变了。这是永动机和工具之间最本质的区别。
...
从组织行为学视角看，L0→L4 描述的是**协调机制的成熟路径**——不只是技术能力的增长，而是组织从依赖外部规则走向内在能力的过程：
...
**L2 = 帮人做事，L4 = 自己变强；L3 = 带着人，L2 不带**——这三个差别是设计决策，不能混淆，评估时也不能混同。

---

**Andy 是 L4 自进化的乘数**。三个角色各有进化边界：Lucas 进化 = 对这个组织理解更深，边界在成员关系的深度；Lisa 进化 = 实现能力更强，边界在代码库复杂度。Andy 的进化没有这个边界——Andy 的每一次判断质量提升，都直接作用于系统里每一条协作链：spec 精度提升一点，Lisa 的成功率就上升。**大脑进化快，整个组织就快**——这是 L4 进化体系的核心投入应向 Andy 倾斜的根本原因。

**Lisa 是 L2 和 L4 的桥梁**。没有 Lisa 的实现，Engineering Anything 停在 spec 阶段，永远不是「交付」。同时，Lisa 的实现过程产生的信号（实现洞察/错误模式/能力缺口），是 Andy 在 L4 巡检时的核心输入。**Lisa 同时服务于两个层次：她是 L2 的交付手，也是 L4 的信号源。**

---

**L2 的本质：交付力，不是代码力**。L2 衡量的核心问题不是「系统能写多好的代码」，而是「组织成员说要是有个 XX 就好了，系统能做到吗」。帮分析一个菜谱、帮准备一份商业计划、帮整理学习进度——这些都是 L2 应该覆盖的交付形态。**Coding 只是手段，组织成员需要的 Any Thing 才是目的。**

**L2 的三个质量维度为什么是这三个**：CLI 工具之所以体验好，不是因为「终端」这个交互媒介，而是三个本质能力——稳定输出（人提需求，系统可靠交付）、过程可见（人能看到系统在做什么）、可选干预（人可以随时介入也可以不介入）。交互媒介不重要，这三个能力才是 L2 的真正竞争维度。

**异步是 L2 的优势，不是缺陷**：同步阻塞要求人坐在终端前等待，复杂度高 = 等待成本高。异步渠道让人提完需求可以去做别的事——异步 + 过程可见 + 可选干预，形成的体验比同步 CLI 更强。**异步让复杂度不再等于人的成本**，这是非阻塞渠道在 L2 交付上最本质的优势。

**进度反馈必须经过成员理解层**：协作链的进度不直接推给用户，而是通过 Frontend Agent（Lucas）转达——因为不同成员需要完全不同的沟通方式。直接推系统消息绕过了成员理解层，违背了「带着人」的设计原则。这不是功能需求，是角色边界。

**方案审批用超时默认，不用阻塞等待**：即时消息渠道不是终端，成员不会一直盯着对话框。多方案时给出选项 + 超时后自动推进，是非阻塞渠道的正确审批模式。阻塞等待会导致任务永久卡住。

**L2 与 L3 的核心差别：L3 带着人**。L2 是系统帮人做事——交付力越强，组织能做的事越多，但人不在进化循环里。L3 才是带着人一起进化：通过成员理解机制（画像/图谱/影子）持续深化对每个成员的认知，在真实沟通前帮人想清楚，让协作洞察回流影响下一次判断。**成员理解不是家庭专属**——家庭理解家人的情绪与关系，企业理解员工的职责与协作偏好，学校理解学生的学情与社交动态。机制相同，内容由实例注入。L2 让系统更强，L3 让组织运作更优——这是两件不同的事。

**L2 与 L4 的核心差别：元层级**。L2 是「做什么」（功能能力），L4 是「如何做得更好」（元能力）。L2 的改进靠人（系统工程师修改 Readme → 刷新 Agent 认知 → 系统按新方向行动），L4 的改进靠系统自身（蒸馏/结晶/DPO 信号积累 → 自动提升）。**「正朔→认知」是系统工程师影响 L4 的最关键工作流**——修改 Readme 定义方向，刷新 Agent 认知让系统按新方向行动，Agent 日常行为产生新数据，L4 机制从中提炼新能力，新能力反馈给系统工程师观察，形成闭环。

**L2/L3 的回流设计**：交付过程产生的信号（实现洞察/需求频率/错误模式）持续流回进化层，形成「运行即进化」的闭环。关键原则：回流不只给系统工程师看，同时直接驱动 Andy 的自我改进；外部知识通过 Lucas 路由给 Andy，而不是系统工程师直接注入（合法性原则）。
...
这个转型有一个对应的系统侧信号：随着微调成熟，SOUL.md 会变得越来越薄。今天 SOUL.md 里有大量铁律和行为约束——这些是「模型还不可靠，所以必须写成外部规则靠注入来强制」。等规则通过 DPO 微调烧进权重后，外部约束就失去必要性，SOUL.md 里只需要保留三件事：身份本质（模型换代时不能丢失的自我认知）、动态事实（实时更新的成员状态，微调无法覆盖）、家庭契约（家人可以读懂并参与维护的内容）。**「SOUL.md 越来越薄」是 L4 成熟度的可观测指标之一**——规则文件条数持续减少，意味着判断在被内化，不再依赖外部提醒。
...
**协调机制成熟路径**（Mintzberg 组织协调理论的 AI 映射）：

```
L0~L1  增强阶段    → 建立认知地基：搭架构、调管道，同时给每个 Agent 注入记忆和人格
L2     工作流标准化 → V 字协作链稳定跑，人提需求、系统可靠交付、过程可见
L3     相互调整    → 系统深度理解每个成员、成员信任系统，两者相互适应、共同进化
L4     价值观内化  → 判断烧进权重，外部规则文件逐步消失，协调从内部涌现
```

「增强而后扩张」的实质，是**协调机制的成熟**——从外部规则驱动，到组织内部能力涌现。系统工程师干预的频率，是系统成熟度的反向指标：干预越少，系统越强；干预越多，说明哪条管道还没跑稳。
...
**设计取舍**：节点注册不需要独立服务，Gateway 内存注册表足够（当前阶段），独立服务只增加故障点。Gateway 重启后内存注册表清空，持久化方案（文件或 Redis）待节点数量增长后补充。
...
| **Main**                | 远程系统工程师代理，主动监控，工具执行，进化意见生成            | wecom-entrance 内置、Claude Sonnet 4.6（配置：openclaw.json `main` 条目）|
| **模型层**                 | 本地推理 + 云端增强                               | homeai-assistant（Ollama）/ DeepSeek / GLM-5.1 / Claude Sonnet 4.6 |
