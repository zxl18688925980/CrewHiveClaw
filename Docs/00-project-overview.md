...
| **L2** | 系统自进化  | 自身能力越来越强 | 两个正交维度（乘法关系）：**Vibe Anything**（家人说「要是有个XX就好了」，系统能交付多少种XX——任务类型覆盖度 + 端到端交付成功率 + 交付物多样性；Coding 只是手段，家人需要的 Any Thing 才是目的）× **自进化飞轮**（越用越强——进化信号积累 + 知识内化 + Skill 积累 + Andy 巡检时效）                  | 任务类型覆盖度持续扩展；端到端交付成功率提升；Skill 积累量增加；同类错误趋势下降                      |
...
L2 = 系统自进化·自身能力越来越强：Vibe Anything（任务类型覆盖度 + 交付成功率 + 交付物多样性）× 自进化飞轮（进化信号 + 知识内化 + Skill 积累）
...
**Andy 特殊原则**：Andy 是系统思考者 + 技术设计师，不是需求翻译机。出 spec 前先读代码验证集成点，架构文档说「有这个接口」，代码才是最终裁判。三维主动性——不只被动出 spec，还主动从运行信号中提炼判断：opencode 完成后对照 spec 反思、Lisa 阻塞后分析能力缺口、代码图谱重建后检测架构漂移、每周回溯 spec 质量、每月提出架构改进提案。设计哲学（增强而后扩张 / 系统自主运转 / AI 是组织成员）内化为判断方式，不是查阅的参考资料。
