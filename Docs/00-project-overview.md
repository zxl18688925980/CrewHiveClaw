...
**测试集构造原则**：
- **对准薄弱点**：从历史失败模式、边界条件、承诺未落地的事中出题，不出普通情况的题
- **基于真实数据**：题目来自真实对话/真实交付/真实决策，不是合成的
- **时间跨度测试**：三个月前的对话还能召回吗？季度前的 spec 决策还能解释吗？
- **对立情境测试**：相近但结论相反的两个 decisions，系统能分清吗？
- **承诺追踪测试**：Agent 承诺过但没有落地的事，系统还记得吗？

**各层外循环评估题型**：

| 层次 | 外循环评估重点 | 典型题型 |
|------|-------------|---------|
| **L0** | 写入质量的系统性评估 | 随机取 50 条记录，检查四维度填充率是否随时间退化；embedding 有效率趋势 |
| **L1** | 召回的系统性盲区扫描 | 针对"三月前对话""相反结论的 decisions""承诺未落地的事"构造 probe，评估 Hit Rate@3 |
| **L2** | 交付能力的实际覆盖面 | 随机取历史需求，评估哪类需求实际被满足、哪类反复失败、哪类从未被尝试 |
| **L3** | 成员画像的准确性 | 用成员真实说过的话构造对照组，检验 inject.md 中的画像是否与实际一致 |
| **L4** | 能力积累的可及性 | 已结晶的 Skill 能否被正确召回并应用？过去的 spec_reflection 是否真正影响了后续 spec？ |

**外循环的执行机制**：Main 在 evaluate_system 工具中主动构造测试集并评估，结果写入 main-pending-tasks.json，系统工程师审阅后决定是否介入。评估结果同时通过 HEARTBEAT 日报汇总给系统工程师，形成周期性的系统健康画像。
