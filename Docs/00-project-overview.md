...
| 层次     | 名称                   | 核心目标                                                                                                 | 完成标志                                                          |
| ------ | -------------------- | ---------------------------------------------------------------------------------------------------- | ------------------------------------------------------------- |
| **L0** | 基础设施稳定性              | 进程稳定与关键服务可达：PM2 关键进程全部 online（wecom-entrance / chromadb / gateway-watchdog / cloudflared-tunnel）、重启频率低；Gateway / ChromaDB / Ollama 可达且延迟正常；磁盘/内存未超限；企业微信 aibot WebSocket 认证正常。**L0 是一切的前提——进程挂了，上面的 L1~L5 全是空转。** | PM2 关键进程 all_online；重启次数 <5 次；Gateway <500ms；磁盘 <70%；aibot authenticated |
...
监控状态记录在 `~/.openclaw/workspace-main/HEARTBEAT.md`（含 `上次健康检查` / `上次任务健康检查` / `上次质量扫描` / `上次L2~L5巡检` / `上次日报发送` 五个时间戳）。Main 在响应模式中也可直接调用 `evaluate_system`（L0~L5 全景评分卡）进行按需诊断。
...
| `evaluate_l0`          | 基础设施稳定性：PM2 关键进程全部 online + 重启次数、Gateway/ChromaDB/Ollama 可达性与延迟、aibot WebSocket 状态、磁盘/内存使用率 |
