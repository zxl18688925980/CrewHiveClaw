...
| **L2** | 行为结晶               | 成熟行为模式自动结晶成 Skill/Tool；跨角色知识持续流动与沉淀                                                                                                                                    | 成熟行为模式能被识别并自动固化；Andy 能自主判断固化形式；角色间知识共享机制正常运转                      |
| **L3** | 组织能力扩张飞轮         | 影子 Agent 驱动三元循环：**理解**（消费 Kuzu 动态协作边，建立成员协作模式理解）→ **增强**（在真实沟通前帮成员想清楚：协调策略/能力缺口/分工建议）→ **演进**（结果信号回流，更新影子「有效/无效」判断），每轮让影子对「这个人在这个组织里」的判断更准，持续提升组织完成目标的能力；成员层次/权限决定影子深度 | 影子对每个成员的增强建议采纳率持续提升（演进环有效）；组织协作效能可量化改善；三层结构（个人/基层组织/能力组织）在架构上均有对应 |
| **L4** | 行为内化             | 双路径训练本地模型：路由 SFT（云端处理且无 DPO 的对话 → 本地能力扩张）+ DPO 对齐（任意来源负向信号 → 本地行为矫正）；Kuzu Agent 行为模式天然去标识化后反哺云端；云端 Agent 永久停在 L2 提示控制层 | 本地路由比例持续上升；AGENTS.md 规则条数持续减少（核心量化指标）                           |

```
L0 = 机器能跑，三机制管道接通
L1 = 系统里每个 Agent 都是有记忆、有理解、有人格的存在
L2 = 机器越用越好
L3 = 影子 Agent 驱动组织能力扩张飞轮：理解→增强→演进，每轮让判断更准
L4 = 机器真正成为组织一部分
```
...
| `notify_engineer` | 向系统工程师发信号：实现阻塞 / 交付通报 |
...
- **任务协调管理**：Lucas 是任务的主动协调者，不只是触发者。注入的任务状态块（`【当前进行中任务】` / `【待告知家人任务】`）绑定到四类行为：① 进行中任务距上次提交 >6h 且话题相关 → 先通过 `ask_lisa` 确认进展再回；② pipeline 触发成功后必须给家人一句时间预期（「通常今晚完成，完成了我通知你」）；③ `【待告知家人任务】` 在本次对话里自然带出，不等家人主动问；④ 任务完成告知后跟进家人确认好用，再调 `record_outcome_feedback` 关闭任务
...
#### 代码结构图谱（Code Graph）— Andy 专属增强

**设计目标**：Andy 在写 spec 时常需要理解函数调用关系（「这个函数被谁调用？它依赖哪些函数？」），纯文件搜索（ripgrep）只能定位代码行，无法回答调用链问题。代码图谱把代码结构化为可查询的图，让 Andy 获得比人类 IDE 更系统的代码理解能力。

**实现**：`HomeAILocal/Scripts/build-code-graph.py`（tree-sitter 解析 TypeScript + Python `ast` 解析 Python），产出两类节点和一类边，存入 Kuzu 独立的 schema（与家人知识图谱共库但独立命名空间）：

```
CodeNode（节点）
  id        STRING  PRIMARY KEY  ← "{file}::{name}"
  name      STRING               ← 函数/类/方法名
  file      STRING               ← 相对于 PROJECT_ROOT 的路径
  line      INT64                ← 定义行号
...
**全量构建**：约 9684 节点、39403 边，覆盖整个 PROJECT_ROOT，耗时约 8 分钟。系统工程师手动运行：`python3 build-code-graph.py`

**快速模式（增量触发）**：`--incremental --paths CrewClaw/crewclaw-routing HomeAILocal/Scripts`，仅处理核心路径（约 89 文件，39 秒）。两个触发源：
- `proc.on("close")`：每次 opencode 完成后 detached spawn，不阻塞主流程
- gateway-watchdog 凌晨 5 点：每日定时增量重建，保持图谱与最新代码同步
...
─────────────────────────────── 增强阶段终点 / 扩张阶段起点
L2  行为结晶（成熟模式 → Skill/Tool 结晶 + 跨角色知识流动）← 以 L1 为土壤
...
- **Andy**：HEARTBEAT 分两层——**Check 0（目标闭环，两步强制）**：① 先强制更新上轮 `andy-goals.jsonl` 中所有 `in_progress` 目标的状态（done / blocked / continued），不得跳过；② 再读 gateway-watchdog 预注入的系统健康快照（opencode matchRate 趋势 / decisions 各类型条目数），识别最薄弱指标、生成新目标并立即行动。**Check 1~7（响应式处理）**：巡检 Kuzu `has_pattern`（`confidence ≥ 0.8`）+ `skill-candidates.jsonl` pending 条目，判断是否直接调 `trigger_lisa_implementation` 结晶 Skill（不再等系统工程师审批，自主实现）；**透明化**：HEARTBEAT 结果非 `HEARTBEAT_OK` 时（有主动行动），基础设施层自动 fire-and-forget 推送工程师通道，工程师可随时感知 Andy 在做什么
...
| **Andy**                | 工具设计，自主写 spec，触发 Lisa，验收 Lisa 交付         | OpenClaw embedded agent、DeepSeek Reasoner R1       |
