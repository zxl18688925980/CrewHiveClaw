...
| 层次     | 名称                   | 核心目标                                                                                       | 完成标志                                       |
| ------ | -------------------- | ------------------------------------------------------------------------------------------ | ------------------------------------------ |
| **L0** | Agents基础设施           | 信息管道建立：运行框架跑通；Data/Knowledge/Capability 三层写入通道定义清楚；记忆/蒸馏/上下文注入三机制接通                        | 信息流能被正确捕获；进化信号有结构化落地；三机制管道可跑通              |
| **L1** | Agents行为质量+自进化       | 人格化与自进化：每个 Agent 从无状态响应器变成有记忆、懂对方、能以人格自主响应、能自我积累技能的存在                                      | 人感知不到「每次从零开始」的断层；反复出现的行为模式自动结晶为可复用 Skill   |
