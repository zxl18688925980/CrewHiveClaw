...
**云端 Agent 永久停在 L4 系统层**：Andy（DeepSeek R1）/ Lisa（MiniMax）等云端 API 无法微调，行为控制永远依赖 AGENTS.md 提示注入。DPO 信号对云端的价值 = 驱动 AGENTS.md 规则更新；对本地的价值 = 直接进入权重。L4 模型层是本地模型的专项旅程。
...
| **Lisa**                | 直接编码优先，复杂任务 run_opencode（background 异步），验证，结果回传 Andy 验收     | OpenClaw embedded agent、MiniMax M2.7               |
