...
| 层次     | 名称     | 口语     | 核心目标                                                                                                                                                                  | 完成标志                                                              |
| ------ | ------ | ------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------- |
| **L0** | 基础设施   | 稳不稳   | Agent 基础设施 Ready：运行框架跑通；Data/Knowledge/Capability 三层 schema 和写入渠道定义清楚；记忆 / 蒸馏 / 上下文注入三个通用机制接通（写入端 + 注入端）；软硬件性能评估（磁盘/内存/Gateway 延迟/ChromaDB 延迟）                                                                         | 信息流能被捕获持久化；进化信号结构化字段有落地；三机制管道可跑通，即使初始数据稀少                         |
| **L1** | 行为质量+自进化   | 好不好+自成长   | Agent 自身人格化/自进化 Ready：三个通用机制真正运转并产生质量——**记忆质量**（上下文组装准确率 + recall 命中率）+ **输出质量**（幻觉检测 + 人格化程度）+ **Skill 固化**（反复出现的行为模式自动结晶为可复用 Skill）。系统里每个 Agent 从无状态响应器变成有记忆、懂对方、能以人格自主响应、能自我积累技能的存在                                                       | 系统里每个 Agent 无论角色，都能记住对方说过的话、理解对方是谁、在对话中展现连贯人格、自主积累技能；人感知不到「每次从零开始」的断层    |
| **L2** | Engineering Anything | 开发即交付 | 多Agent智能体协作框架 Ready：系统帮人完成任务的功能能力。**Frontend Agent 主力**——用户说「要是有个XX就好了」，前端 Agent 感知需求、推演、触发流水线，系统能交付多少种XX；Designer 设计 + Implementor 实现，三角色闭环交付。**交付可靠性由基础设施层保证**（非模型自觉）：① 验证门（交付物独立编译+文件检查，不通过自动重试）② 分层升级（失败后简化spec→换方案→上报，非线性重试）③ 进度反馈（前端 Agent 对所有组织成员一致：收到→方案中→实现中→完成/失败）。任务类型覆盖度 + 端到端交付成功率 + 交付物多样性                  | 任务类型覆盖度持续扩展；端到端交付成功率提升；交付物多样性增加；三角色流水线健康（completed 比例 ≥80%）；所有组织成员（内部+外部）体验一致                      |
| **L3** | 组织级AI | 带着人一起进化 | **组织成员理解 + 协作优化 + 和人一起进化组织**：四层框架机制——**①成员画像**（从交互数据蒸馏每个成员的角色/沟通风格/关注点/当前状态，不靠手写档案）；**②关系图谱**（成员间关系边——不只是家庭关系，包括汇报关系/协作关系/影响力关系/分工关系，由实例定义关系类型）；**③影子 Agent**（per-member 专属增强视图，理解→增强→演进三元循环）；**④跨成员感知**（前端 Agent 作为协作中介，感知组织级信号并推动优化）。**外部用户管理**（guest/客户/合作伙伴）与核心成员隔离但体验对齐。**框架 vs 实例**：机制（蒸馏管道/图谱遍历/影子创建/跨成员扫描）= 框架层，内容（关系类型/画像维度/隐私规则）= 实例层 | 影子对每个成员的增强建议采纳率持续提升（演进环有效）；组织协作效能可量化改善；内部成员与外部用户体验一致 |
...
```
L0 = 基础设施·Agent基础设施Ready：机器能跑，三机制管道接通，软硬件性能达标
L1 = 行为质量+自进化·好不好+自成长：记忆质量 + 输出质量 + Skill 固化，每个 Agent 都是有记忆、有理解、有人格、能自我积累技能的存在
L2 = Engineering Anything·开发即交付：多Agent智能体协作框架Ready，前端Agent感知需求 → Designer设计 → Implementor实现，三角色闭环交付；基础设施层保证交付可靠（验证门+分层升级+进度反馈）
L3 = 组织级AI·带着人一起进化：成员画像（从交互蒸馏）+ 关系图谱（实例定义类型）+ 影子增强（理解→增强→演进）+ 跨成员感知（协作中介）
...
| `evaluate_l3`          | 组织级AI 四维度：**①成员画像**（inject.md 含蒸馏信息比例——行为模式/沟通风格/协作关系等关键词覆盖率）、**②关系图谱**（Kuzu 协作边 co_discusses/requests_from/supports/role_in_context）、**③影子Agent**（ChromaDB shadow_interactions 演进环记录数 + 访客 Registry active/dormant/archived 统计）、**④跨成员感知**（关系蒸馏日志上次运行时间 + 成员档案含协作信息比例）|
