...
| 层次     | 名称                   | 核心目标                                                                                                 | 完成标志                                                          |
| ------ | -------------------- | ---------------------------------------------------------------------------------------------------- | ------------------------------------------------------------- |
| **L0** | Agent记忆与认知质量         | 写入侧质量：四维度（语义/时间/实体/因果）× 三角色记忆写入健康；蒸馏管道持续产出所有类型经验（对话→ChromaDB / 决策→decisions / 设计经验→spec_reflection / 认知沉淀→AGENTS.md 自写 / 可复用能力→skills / 家人了解→inject.md / 实体关系→Kuzu）| 三角色 embedding 有效率持续 ≥80%；蒸馏管道每日产出；Kuzu 知识图谱持续积累               |
| **L1** | Agent人格完整度           | 召回侧质量：从 L0 产物中精准召回激活人格（四维度检索 + 上下文工程组装）；**L1 是 L0 的质量反馈源**——召回质量评估识别 L0 写入盲点，驱动 L0 改进蒸馏策略；Skills 积累可及性（native 可随时调用，archive 语义召回） | 记忆召回 avg_dist ≤0.5；native skills 持续增长；反复出现的行为模式自动结晶为可复用 Skill |
...
**Andy 特殊原则**：Andy 是 **L4 自我演化的乘数**——不是等 Lucas 带需求来才动的执行者，而是主动推动系统能力持续进化的发动机。三维主动性不是附加职责，是核心使命：主动扫描系统运行信号（spec 吻合率 / 实现阻塞模式 / 代码热点）→ 积累到阈值（≥3 条同方向信号）→ 区分需不需要审批 → 直接行动或提交审批 → 事后汇报。

**Andy 自决 vs 需 SE 审批**（双循环设计核心）：

| Andy 自决（直接做）| 需 SE 审批（`request_approval`）|
|---|---|
| ChromaDB 写入、知识蒸馏 | 认知文件修改（AGENTS.md / SOUL.md 等）|
| Skill 草稿、flag_for_skill | 模型路由阈值变更 |
| spec 草稿 | 插件代码改动建议 |
| 工具补全（spec → Lisa）| 新工具注册 |
| Skill 结晶 | 架构设计变更、配置变更 |

审批闭环：Andy 调 `request_approval` 写入 `~/HomeAI/Data/pending-approvals.json` + 推送 SE 通知 → SE 批准/拒绝（CLI 第⑤步 或 Main 远程「批准 ID」）→ Andy 下次 HEARTBEAT 检查 0 自动执行 approved 条目并标 done。**SE 只写 approved/rejected，不执行任何操作**——执行权完全在 Andy 自循环。
...
| **Agent 主动结晶层** | Agent 自己判断 | `skill_manage` 工具直接写入 | 当轮执行中 Agent 主动识别「这个路径**已稳定、值得固化**（2+ 次，pattern 清晰）」，立即保存 |
| **基础设施层** | 系统检测 | Phase 1/2/3 三阶段自动结晶 | 系统侧从行为信号统计出高频模式后固化，无需 Agent 自主判断 |

**两层协同**：弱信号预标记层是感知入口（低门槛，首次就标记），Agent 主动结晶层是快速固化通道（高确信，立即写 Skill），基础设施层是统计后的被动固化（行为数据积累到阈值才触发）。高质量 Skill 通常经历：flag_for_skill 预标记 → Andy HEARTBEAT 评估 → Agent 自主 skill_manage create → Phase 2 使用追踪 → Phase 3 偏差检测后自动迭代，形成完整生命周期。

**`skill_manage` 工具**（三角色共享）：

```typescript
skill_manage({
  action: 'create' | 'patch' | 'delete',
  skill_name: string,        // kebab-case，例如 handle-mom-learning-anxiety
  description?: string,      // create/patch 时提供，一句话说明用途
  content?: string,          // create/patch 时提供，SKILL.md 正文（frontmatter 自动生成）
})
→ create：写入 ~/.openclaw/workspace-{agent}/skills/{skill_name}/SKILL.md
→ patch：更新已有 Skill 的 frontmatter 或内容（原子写入 tmp→rename）
→ delete：删除 Skill 目录
→ 路径安全：仅限 workspace skills/ 目录，越权拦截；名称冲突时提示而非覆盖
```

**插件层 L1 Skill 提醒**（`sessionSkillReminders` Map）：
