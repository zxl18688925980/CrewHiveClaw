...
> **实例实现参考（HomeAI）**：ChromaDB（向量数据库）承载语义维度；timestamp + timeWeightedRerank 承载时间维度；Kuzu（图数据库）承载实体维度（v645 图增强查询）和因果维度（causal_relation Facts）。具体代码在 `crewclaw-routing/index.ts` 的 `queryMemories` 函数。第二个工程师可用任何等价组件替代。
...
| 层次     | 名称               | 核心目标                                                                                                                                                                   | 完成标志                                                              |
| ------ | ---------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------- |
| **L0** | 基础设施（稳不稳）   | 软硬件选型（模型/数据库/部署方式）+ 信息架构设计（Data/Knowledge/Capability 三层 schema 和数据流）；记忆 / 蒸馏 / 上下文注入三个通用机制接通（写入端 + 注入端）；软硬件性能持续监控（磁盘/内存/延迟）                                                                          | 信息流能被捕获持久化；进化信号结构化字段有落地；三机制管道可跑通，即使初始数据稀少                         |
| **L1** | 行为质量（好不好） | 两大核心维度：①记忆质量——上下文组装完整性 + 关键信息 recall 准确率 ②输出质量——幻觉率/格式合规/空回复 + 人格化程度（是否把握住做人的感觉）。系统里每个 Agent 从无状态响应器变成有记忆、懂对方、能以人格自主响应的存在                                                        | 系统里每个 Agent 无论角色，都能记住对方说过的话、理解对方是谁、在对话中展现连贯人格；人感知不到「每次从零开始」的断层    |
| **L2** | 系统自进化（自身能力越来越强）               | 三个原始诉求：①开发流水线成效——Andy 方案质量、Lisa 实现成功率、端到端交付效果 ②自进化机制运转——蒸馏管道是否在产出、结晶是否在发生、同类错误是否在减少（看轨迹而非快照）③喂养成效——投喂内容是否被提炼、L0~L2 得分趋势是否持续上升                                                                                                                                    | 成熟行为模式能被识别并自动固化；Andy 能自主判断固化形式；角色间知识共享机制正常运转                      |
| **L3** | 组织协作进化（组织运作越来越优化）         | **带着人一起进化**：影子 Agent 驱动三元循环——**理解**（消费 Kuzu 动态协作边，建立成员协作模式理解）→ **增强**（在真实沟通前帮成员想清楚：协调策略/能力缺口/分工建议）→ **演进**（结果信号回流，更新影子「有效/无效」判断），每轮让影子对「这个人在这个组织里」的判断更准；成员层次/权限决定影子深度 | 影子对每个成员的增强建议采纳率持续提升（演进环有效）；组织协作效能可量化改善；三层结构（个人/基层组织/能力组织）在架构上均有对应 |
| **L4** | 深度学习（内化能力越来越强）             | 双路径训练本地模型：路由 SFT（云端处理且无 DPO 的对话 → 本地能力扩张）+ DPO 对齐（任意来源负向信号 → 本地行为矫正）；Kuzu Agent 行为模式天然去标识化后反哺云端；本地模型能力评估（数据驱动：Kuzu 知识掌握 + ChromaDB 对话能力）；系统内化能力越强，对外依赖越少 | 本地路由比例持续上升；AGENTS.md 规则条数持续减少（核心量化指标）                           |

```
L0 = 基础设施（稳不稳）：软硬件选型 + 信息架构设计 + 三机制管道接通 + 性能监控
L1 = 行为质量（好不好）：记忆质量 + 输出质量，每个 Agent 都是有记忆、有理解、有人格的存在
L2 = 系统自进化（自身能力越来越强）：流水线成效 + 自进化机制运转 + 喂养成效
L3 = 组织协作进化（组织运作越来越优化）：带着人一起进化，理解→增强→演进，每轮让判断更准
L4 = 深度学习（内化能力越来越强）：DPO + SFT + 模型能力评估，对外依赖越来越少
```
...
**主动监控**：Main 不只是被动响应器。wecom-entrance 内置 4h 监控循环，定时触发 Main agent 按六步协议执行 L0~L4 全链路巡检。所有推送报告带身份标识和告警等级（🔴 紧急/🟡 关注/🟢 正常）：
...
- **Step 2（每日一次）**：`evaluate_l1` — 行为质量评估：①记忆质量（上下文组装完整性 + recall 准确率）②输出质量（幻觉 + 人格化程度）+ Andy/Lisa 活跃度
...
| `evaluate_l0`          | 基础设施评估（稳不稳）：进程存活状态 + 软硬件性能指标（磁盘空间/内存占用/Gateway 响应延迟/ChromaDB 响应延迟）+ 定时任务执行健康 + 数据层健康（Kuzu Fact 总数/ChromaDB 对话量/家人档案新鲜度） |
| `evaluate_l1`          | 行为质量评估（好不好，两大核心维度）：①记忆质量——上下文组装完整性 + 关键信息 recall 准确率 ②输出质量——幻觉率/格式合规/空回复 + 人格化程度（是否把握住做人的感觉）；附带 Andy/Lisa 活跃度 |
| `evaluate_l2`          | 系统自进化评估（自身能力越来越强，三个原始诉求）：①开发流水线成效——Andy 方案质量、Lisa 实现成功率、端到端交付效果 ②自进化机制运转——蒸馏管道是否在产出、结晶是否在发生、同类错误是否在减少 ③喂养成效——投喂内容是否被提炼、L0~L2 得分趋势是否上升 |
| `evaluate_l3`          | 组织协作进化评估（组织运作越来越优化）：Kuzu 协作边 + 演进环记录 + 访客影子生命周期 + 关系蒸馏运行状态 + 成员增强效果 |
| `evaluate_l4`          | 深度学习评估（内化能力越来越强）：DPO 信号积累/趋势 + 本地模型就绪度 + 模型能力评估（调用 evaluate_local_model）+ 规则文件消减进度 |
| `evaluate_local_model` | 数据驱动本地模型智力评测：从 Kuzu 抽家人事实→生成知识题（0.4 权重），从 ChromaDB 抽真实对话→测对话能力（0.6 权重，三维度：需求理解/推理/边界），综合 ≥3.5 通过。零硬编码，实例有数据就能跑 |
...
Main 通过 `evaluate_l4` 持续跟踪三个维度：① DPO 信号积累进度（按 pattern_type 统计，距内化阈值的缺口，近 7 天 vs 前 7 天趋势）；② 本地模型就绪状态 + 模型能力评估（`evaluate_local_model` 从 Kuzu + ChromaDB 真实数据自动生成测试题，零硬编码）；③ 规则文件消减进度（AGENTS.md 幻觉禁令条数，判断补丁是否在收敛）。L2 是 L4 未就绪前的临时拦截手段；L4 就绪后，对应 L2 补丁随微调完成逐步撤除，SOUL.md 同步瘦身。
