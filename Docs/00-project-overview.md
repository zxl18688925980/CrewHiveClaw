...
| **L2** | Engineering Anything | 开发即交付 | 多Agent智能体协作框架 Ready：系统帮人完成任务的功能能力。**Lucas 主力**——家人说「要是有个XX就好了」，Lucas 感知需求、推演、触发流水线，系统能交付多少种XX；Andy 设计 + Lisa 实现，三角色闭环交付。任务类型覆盖度 + 端到端交付成功率 + 交付物多样性                  | 任务类型覆盖度持续扩展；端到端交付成功率提升；交付物多样性增加                      |
| **L3** | 组织级AI | 带着人一起进化 | **组织理解力 + 协作优化 + 和人一起进化组织**：影子 Agent 驱动三元循环——**理解**（消费 Kuzu 动态协作边，建立成员协作模式理解）→ **增强**（在真实沟通前帮成员想清楚：协调策略/能力缺口/分工建议）→ **演进**（结果信号回流，更新影子「有效/无效」判断），每轮让影子对「这个人在这个组织里」的判断更准；成员层次/权限决定影子深度；成员增强效果检查（inject.md 协作边检测） | 影子对每个成员的增强建议采纳率持续提升（演进环有效）；组织协作效能可量化改善；三层结构（个人/基层组织/能力组织）在架构上均有对应 |
...
L2 = Engineering Anything·开发即交付：多Agent智能体协作框架Ready，Lucas 感知需求 → Andy 设计 → Lisa 实现，三角色闭环交付
L3 = 组织级AI·带着人一起进化：组织理解力 + 协作优化 + 和人一起进化组织，理解→增强→演进
...
**L2 与 L3 的核心差别：L3 带着人**。L2 是系统帮人做事——交付力越强，组织能做的事越多，但人不在进化循环里。L3 才是带着人一起进化：通过影子 Agent 理解每个成员在这个组织里的行为模式，在真实沟通前帮人想清楚，让协作洞察回流影响下一次判断。L2 让系统更强，L3 让组织运作更优——这是两件不同的事，设计时不能混同，评估时也不能混同。
...
| `before_tool_call` | 访客工具禁令（visitor: 前缀 userId 拦截指定工具）；**访客触发开发流水线**（`trigger_development_pipeline` 从封锁名单移除，改为 infra guard 检查：需求含基础设施关键词时拦截并转告主人审核）；**Lucas 专属工具 agentId 守卫**（21 个工具全部覆盖代码层 agentId 验证，非 lucas agent 调用被拦截返回 wrong_agent）；Andy 写操作守卫：write/edit 只允许 workspace-andy/ 内文件；exec 检测写操作特征（重定向/tee/sed -i 等），命中则拦截——但 **L4 自进化操作放行**（andy-goals/learning-state/spec-retro 等 6 个状态文件 + ChromaDB Python 客户端操作），项目代码仍然封锁；**核心框架文件保护**（trigger_lisa_implementation 中：Andy spec integration_points 含保护文件时即时 pushToChannel 通知系统工程师，warn-and-proceed 不阻断流水线） | `config/visitor-restrictions.json` blockedTools 列表；`config/infra-guard.json` infraKeywords |
