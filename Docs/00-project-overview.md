...
| **L3** | 组织协作进化 | 带着人一起进化 | **组织成员理解 + 协作优化 + 和人一起进化组织**：四层框架机制——**①成员画像**（从交互数据蒸馏每个成员的角色/沟通风格/关注点/当前状态，不靠手写档案）；**②关系图谱**（成员间关系边——不只是家庭关系，包括汇报关系/协作关系/影响力关系/分工关系，由实例定义关系类型）；**③影子 Agent**（per-member 专属增强视图，理解→增强→演进三元循环）；**④跨成员感知**（前端 Agent 作为协作中介，感知组织级信号并推动优化）。**外围成员（访客/客户/合作伙伴）是组织成员**——L2 体验与核心成员完全对齐（同样的协作链、同样的交付质量、同样的进度可见），但不可引导组织演化（不能触发 L3/L4 级别的组织结构调整和系统自进化方向）。**框架 vs 实例**：机制（蒸馏管道/图谱遍历/影子创建/跨成员扫描）= 框架层，内容（关系类型/画像维度/隐私规则）= 实例层 | 影子对每个成员的增强建议采纳率持续提升（演进环有效）；组织协作效能可量化改善；核心成员与外围成员 L2 体验一致 |
...
L3 = 组织协作进化：成员画像（从交互蒸馏）+ 关系图谱（实例定义类型）+ 影子增强（理解→增强→演进）+ 跨成员感知（协作中介）
