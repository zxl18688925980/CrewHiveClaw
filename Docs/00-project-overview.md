...
- 热路径：业务大师感知同类情况 ≥ 2 次 → 调用 `flag_for_skill` 写入 `skill-candidates.jsonl`；或基础设施层自动检测到 2+ 行动工具组合，自动写入 Skill 草稿（Phase 1 机制）
...
#### L1 Skill 自主管理（Agent 主动层，v673 落地）

Skill 固化由**两层独立机制**共同驱动，不互相替代：

| 层次 | 谁触发 | 机制 | 适合场景 |
|------|--------|------|---------|
| **Agent 主动层** | Agent 自己判断 | `skill_manage` 工具直接写入 | 当轮执行中 Agent 主动识别「这个路径值得固化」，立即保存，不等统计 |
...
业务大师 AGENTS.md 规则补充：
```
感知到同类组织情况处理 2 次以上 → 主动调用 flag_for_skill
