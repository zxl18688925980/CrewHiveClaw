...
**每个 Lx 层的健康评估由该层自身的机制产生，Main 的 `evaluate_l*` 是外部审计（兜底补救），不是主路径。**

靠 Main 触发的 evaluate_l* 是被动补救——它依赖系统工程师发现异常才运行。真正的自演化系统，每个层的质量评估必须内嵌在该层的正常运行循环里：

| 层次 | 内部自评机制 | 自评粒度 |
|------|------------|---------|
| **L1** | Andy 每任务 spec_reflection（事件级）+ 检查 13 周期汇总 L1 质量趋势（层级）| 每任务即时 + 每周汇总 |
| **L2** | 检查 13 扩展：L2 交付采纳审计（7~14 天后回望交付是否被使用）| 每周 |
| **L3** | Kuzu 关系图谱 + ChromaDB 成员记忆持续更新（通过日常交互隐式产生）| 每次对话触发 |
| **L4** | 检查 13 的自主发起频率/质量 + L1 趋势信号持续不改善时触发 l4_evolution_stalled | 每周 |

「内部产生，外部审计」：自评信号决定是否自主行动；Main 的 evaluate_l* 在系统工程师主动检视时提供独立视角，或在内部信号沉默过久时兜底触发。
