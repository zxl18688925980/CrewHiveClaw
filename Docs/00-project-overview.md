...
| 层次     | 名称                   | 核心目标                                                                                                 | 完成标志                                                          |
| ------ | -------------------- | ---------------------------------------------------------------------------------------------------- | ------------------------------------------------------------- |
| **L0** | Agent记忆与认知质量         | 写入侧质量：四维度（语义/时间/实体/因果）× 三角色记忆写入健康；蒸馏管道持续产出（design_learning / impl_learning / Kuzu facts / inject.md） | 三角色 embedding 有效率持续 ≥80%；蒸馏管道每日产出；Kuzu 知识图谱持续积累               |
| **L1** | Agent人格完整度           | 召回侧质量：记忆语义召回相关性持续改善；上下文工程形成人格连续性（家人感知不到「从零开始」的断层）；Skill 自动沉淀与持续积累；行为模式结晶（Kuzu has_pattern）           | 记忆召回 avg_dist ≤0.5；native skills 持续增长；反复出现的行为模式自动结晶为可复用 Skill |
...
蒸馏产物：ChromaDB 高质量记录 + Kuzu Fact 图谱 + inject.md 家人档案 + decisions 集合（design_learning / impl_learning）。**L0 核心目标：蒸馏管道持续健康，产物丰富且四维度有效。**
...
**L1 核心目标：每一轮对话，上下文工程都能把正确的"自我"组装进来，让 Agent 表现出连续的人格。**
...
|------|----------|---------|------------|
| **L0** | 路由阈值/工具链/管道配置变更 | 变更前验证 KPI 数据支撑该调整 | 变更后跟踪目标指标是否朝预期方向移动 |
| **L1** | spec 写作 | 查同域历史 spec_reflection，失败模式作为约束带入 | Andy 验收后写 spec_reflection，记录「预期什么/实际如何」 |
...
**设计取舍**：节点注册不需要独立服务，Gateway 内存注册表 + 持久化 JSON 足够（当前阶段）。独立服务只增加故障点。
