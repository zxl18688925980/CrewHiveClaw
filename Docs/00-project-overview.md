...
L4 = 系统自进化·架构自优化+模型本地内化：系统层自我改进（蒸馏/结晶/Andy 主动巡检 + **Andy Spec SFT 积累管道**）+ 模型层内化（DPO+SFT，训练数据集自动生成就绪）
```
...
Main 通过 `evaluate_l4` 持续跟踪三个维度：① 按 pattern_type 统计 DPO 信号积累进度（距 50 条内化阈值还差多少，近 7 天 vs 前 7 天趋势）；② L4 系统层临时干预效果（AGENTS.md 幻觉禁令条数，判断补丁是否在收敛问题）；③ 本地模型就绪状态（Qwen2.5-Coder-32B-4bit 已可用，Gemma 4 为进化终态）。L4 系统层（AGENTS.md 规则）是 L4 模型层未就绪前的临时拦截手段；模型层就绪后，对应系统层补丁随微调完成逐步撤除，SOUL.md 同步瘦身。
