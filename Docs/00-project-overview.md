...
| 层次     | 名称                   | 核心目标                                                                                       | 完成标志                                       |
| ------ | -------------------- | ------------------------------------------------------------------------------------------ | ------------------------------------------ |
| **L0** | Agent记忆与认知质量        | 写入侧质量：四维度（语义/时间/实体/因果）× 三角色记忆写入健康；蒸馏管道持续产出（design_learning / impl_learning / Kuzu facts / inject.md）。基础设施（Gateway/ChromaDB/磁盘）是L0正常工作的**前提条件**，不是L0定义本身 | 三角色 embedding 有效率持续 ≥80%；蒸馏管道每日产出；Kuzu 知识图谱持续积累   |
...
| **L4** | 系统自进化                | 让 L0~L3 越来越好：系统层（Andy 主动巡检系统信号、蒸馏、Skill 结晶）+ 模型层（DPO/SFT 让判断从外部约束变为内化能力）。「正朔→认知」是 L4 的控制回路 | AGENTS.md 规则条数持续减少；本地路由比例上升；同类错误趋势下降       |
...
| **L2 vs L4** | L2 是功能能力（做什么），L4 是元能力（如何做得更好） | L2 改进靠人（系统工程师修改 Readme → 刷新认知），L4 改进靠系统自身（蒸馏/结晶/DPO 信号积累）。「正朔→认知」是系统工程师影响 L4 的控制回路 |
