...
| `evaluate_l1`          | Agent 记忆写入质量——两个维度：**①结构健康**（四维度：语义/时间/实体/因果 × 三角色 embedding 有效率、蒸馏管道执行情况、家人档案 inject.md 存在性、Kuzu Fact 数量、数据新鲜度、时间分层退化）；**②内容质量**（目标：抽取最近 N 条蒸馏产出样本，检查描述字段是否有实质内容——非空/非占位符/有合理长度/有上下文，给出质量比率；结构完整不等于内容有意义，蒸馏出的 design_learning/impl_learning 决策要检查是否真的包含洞察，Kuzu fact 要检查实体关系是否有实质描述）；**Skill 结晶健康**：native/archive 积累量、AGENTS.md 大小监控（>10k = 溢出信号）、archive 中晋升候选积压检测（success≥3 + failRate<30%） |
