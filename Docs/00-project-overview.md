...
| **L4** | 系统自进化                | 让 L0~L3 越来越好。两条路径：**系统层**——Andy 利用 L2 的工程交付能力对准系统本身（架构优化 / 工具链改进 / 规则结晶 / 流水线健康），同一套 Andy→Lisa 管道，目标从外部需求换成系统自身；**模型层**——DPO/SFT 让判断从外部约束变为内化能力。「正朔→认知」是系统工程师控制 L4 演化方向的控制回路 | 系统自我改进的交付记录持续出现；AGENTS.md 规则条数持续减少；本地路由比例上升；同类错误趋势下降 |
...
| **L2 vs L4** | L2 是功能能力（做什么），L4 是元能力（如何做得更好） | **用同一套 Andy→Lisa 管道，目标不同**：L2 的目标是外部需求（家人要什么 → 系统交付什么）；L4 系统层的目标是系统自身（Andy 发现系统瓶颈 → 触发 Lisa 改进系统架构/工具/流水线）。L4 模型层才是 DPO/SFT 内化——两者并重，不能只强调模型内化。「正朔→认知」是系统工程师控制 L4 演化方向的控制回路 |
