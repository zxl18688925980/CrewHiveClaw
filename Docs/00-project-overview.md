...
| 层次     | 名称     | 口语     | 核心目标                                                                                                                                                                  | 完成标志                                                              |
| ------ | ------ | ------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------- |
| **L0** | Agent基础设施   | 稳不稳   | Agent 基础设施 Ready：运行框架跑通；Data/Knowledge/Capability 三层 schema 和写入渠道定义清楚；记忆 / 蒸馏 / 上下文注入三个通用机制接通（写入端 + 注入端）；软硬件性能评估（磁盘/内存/Gateway 延迟/ChromaDB 延迟）                                                                         | 信息流能被捕获持久化；进化信号结构化字段有落地；三机制管道可跑通，即使初始数据稀少                         |
| **L1** | Agent行为质量+自进化   | 好不好+自成长   | Agent 自身人格化/自进化 Ready：三个通用机制真正运转并产生质量——**记忆质量**（上下文组装准确率 + recall 命中率）+ **输出质量**（幻觉检测 + 人格化程度）+ **Skill 固化**（反复出现的行为模式自动结晶为可复用 Skill）。系统里每个 Agent 从无状态响应器变成有记忆、懂对方、能以人格自主响应、能自我积累技能的存在                                                       | 系统里每个 Agent 无论角色，都能记住对方说过的话、理解对方是谁、在对话中展现连贯人格、自主积累技能；人感知不到「每次从零开始」的断层    |
...
```
L0 = Agent基础设施：机器能跑，三机制管道接通，软硬件性能达标
L1 = Agent行为质量+自进化·好不好+自成长：记忆质量 + 输出质量 + Skill 固化，每个 Agent 都是有记忆、有理解、有人格、能自我积累技能的存在
