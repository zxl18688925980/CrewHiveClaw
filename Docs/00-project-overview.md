...
| **L2** | Agent人格完整度           | 召回侧质量：从 L1 产物中精准召回激活人格（四维度检索 + 上下文工程组装）；**L2 是 L1 的质量反馈源**——召回质量评估识别 L1 写入盲点，驱动 L1 改进蒸馏策略；Skills 可及性（native 全量注入，archive 语义召回 top-3），上下文工程把 L1 沉淀的 Skill 在合适的时机激活 | 记忆召回 avg_dist ≤0.5；native skills 按需激活；上下文预算合理分配 |
...
**L2 核心目标：每一轮对话，上下文工程都能把正确的"自我"组装进来，让 Agent 表现出连续的人格。L2 同时作为 L1 的质量镜——反射写入侧的问题，驱动 L1 自我改进。**
