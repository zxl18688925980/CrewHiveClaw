...
| 层次     | 名称     | 口语     | 核心目标                                                                                                                                                                  | 完成标志                                                              |
| ------ | ------ | ------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------- |
| **L0** | Agents基础设施   | 稳不稳   | Agent 基础设施 Ready：运行框架跑通；Data/Knowledge/Capability 三层 schema 和写入渠道定义清楚；记忆 / 蒸馏 / 上下文注入三个通用机制接通（写入端 + 注入端）；软硬件性能评估（磁盘/内存/Gateway 延迟/ChromaDB 延迟）                                                                         | 信息流能被捕获持久化；进化信号结构化字段有落地；三机制管道可跑通，即使初始数据稀少                         |
| **L1** | Agents行为质量+自进化   | 好不好+自成长   | Agent 自身人格化/自进化 Ready：三个通用机制真正运转并产生质量——**记忆质量**（上下文组装准确率 + recall 命中率）+ **输出质量**（幻觉检测 + 人格化程度）+ **Skill 固化**（反复出现的行为模式自动结晶为可复用 Skill）。系统里每个 Agent 从无状态响应器变成有记忆、懂对方、能以人格自主响应、能自我积累技能的存在                                                       | 系统里每个 Agent 无论角色，都能记住对方说过的话、理解对方是谁、在对话中展现连贯人格、自主积累技能；人感知不到「每次从零开始」的断层    |
| **L2** | Engineering Anything | 开发即交付 | 多Agent智能体协作框架 Ready：系统帮人完成任务的功能能力。**Frontend Agent 主力**——用户说「要是有个XX就好了」，前端 Agent 感知需求、推演、触发协作链，系统能交付多少种XX；Designer 设计 + Implementor 实现，三角色闭环交付。**交付可靠性由基础设施层保证**（非模型自觉）：① 验证门（交付物独立编译+文件检查，不通过自动重试）② 分层升级（失败后简化spec→换方案→上报，非线性重试）③ 进度反馈（前端 Agent 对所有组织成员一致：收到→方案中→实现中→完成/失败）④ 体验对齐（Exploration 可见性、需求澄清强制检查、多方案设计与切换、3维度代码审查、结构化交付摘要，对齐 ClaudeCode CLI 7阶段体验）。**L2 本质对标**：稳定输出+过程可见+可选干预——核心是家人提需求后系统稳定交付；过程可见和可选干预让家人有信心提更复杂需求。五个方向：Lisa直接编码能力（read/edit/write/bash/grep/find/ls，简单任务不再依赖外部CLI）、流式状态同步（走Lucas千人千面）、方案审批gate（走Lucas超时默认）、小弟并行（v682已落地）、patch_file精准操作（edit_patches备用）。任务类型覆盖度 + 端到端交付成功率 + 交付物多样性                  | 任务类型覆盖度持续扩展；端到端交付成功率提升；交付物多样性增加；三角色协作链健康（completed 比例 ≥80%）；所有组织成员（内部+外部）体验一致                      |
| **L3** | 组织协作进化 | 带着人一起进化 | **组织成员理解 + 协作优化 + 和人一起进化组织**：四层框架机制——**①成员画像**（从交互数据蒸馏每个成员的角色/沟通风格/关注点/当前状态，不靠手写档案）；**②关系图谱**（成员间关系边——不只是家庭关系，包括汇报关系/协作关系/影响力关系/分工关系，由实例定义关系类型）；**③影子 Agent**（per-member 专属增强视图，理解→增强→演进三元循环）；**④跨成员感知**（前端 Agent 作为协作中介，感知组织级信号并推动优化）。**影子 Agent 的物理载体——成员私有 AI 节点**：每个家庭成员可以有自己专属的 AI 助理实例，运行在该成员的个人设备上（Windows PC / macOS 笔记本 / Linux 服务器），通过 HomeAI 中心节点协作。**节点属于影子，不属于 Andy/Lisa**：节点由 Lucas 配置（关联到具体家庭成员 userId），节点上运行该成员的私人 OpenClaw / Claude Code 实例，与 HomeAI 中心协作完成该成员的需求。节点注册时自动在 Kuzu 建立「成员 ↔ 节点」协作边，沉淀到关系图谱。**框架 vs 实例**：机制（蒸馏管道/图谱遍历/影子创建/跨成员扫描/节点协作协议）= 框架层，内容（关系类型/画像维度/隐私规则/节点归属映射）= 实例层。**外围成员（访客/客户/合作伙伴）是组织成员**——L2 体验与核心成员完全对齐（同样的协作链、同样的交付质量、同样的进度可见），但不可引导组织演化（不能触发 L3/L4 级别的组织结构调整和系统自进化方向）。 | 影子对每个成员的增强建议采纳率持续提升（演进环有效）；组织协作效能可量化改善；核心成员与外围成员 L2 体验一致；节点协作边数量 = 活跃影子数 |
| **L4** | 系统自进化 | 让 L0~L3 越来越好 | 系统软件架构能力自优化 + 模型能力本地内化。包含两层：①**系统层自我改进**（Andy 主力）——蒸馏、结晶、spec 质量提升、DPO 信号收集、路由阈值进化、Andy 主动巡检系统信号发现能力缺口，无需等外部触发；②**模型层内化**——本地模型微调（DPO+SFT）、行为模式写入权重、数据驱动模型评估。系统工程师通过「修改 Readme（正朔）→ 刷新 Agent 认知」影响系统演化方向，这是 L4 的控制回路                                                | 系统层：进化信号积累 + Andy 巡检时效 + 同类错误趋势下降；模型层：本地路由比例持续上升，AGENTS.md 规则条数持续减少                             |

```
L0 = Agents基础设施：机器能跑，三机制管道接通，软硬件性能达标
L1 = Agents行为质量+自进化·好不好+自成长：记忆质量 + 输出质量 + Skill 固化，每个 Agent 都是有记忆、有理解、有人格、能自我积累技能的存在
L2 = Engineering Anything·开发即交付：多Agent智能体协作框架Ready，前端Agent感知需求 → Designer设计 → Implementor实现，三角色闭环交付；基础设施层保证交付可靠（验证门+分层升级+进度反馈+体验对齐）。**L2 本质对标**：稳定输出+过程可见+可选干预（Lisa直接编码能力/流式状态同步走Lucas千人千面/方案审批gate走Lucas/小弟并行）
L3 = 组织协作进化：成员画像（从交互蒸馏）+ 关系图谱（实例定义类型）+ 影子增强（理解→增强→演进）+ 跨成员感知（协作中介）+ **成员私有 AI 节点（节点=影子物理载体，Lucas 配置，Kuzu 协作边沉淀）**
L4 = 系统自进化·架构自优化+模型本地内化：系统层自我改进（蒸馏/结晶/Andy 主动巡检 + **Andy Spec SFT 积累管道**）+ 模型层内化（**微调闭环已接通**：数据桥接→LoRA训练→fuse→GGUF→Ollama更新，3416条样本就绪；homeai-assistant 基于 Qwen2.5-Coder-32B-4bit，本地路由已激活）
```

**增强阶段（L0→L1）**：先让系统里每个 Agent 具备像真正的人一样的认知基础，并能自我积累技能（Skill 固化）。这不只是地基——它本身就是 CrewClaw 相比原生 OpenClaw 最核心的价值交付。地基不稳，上面所有层都是空转。

**扩张阶段（L2→L4）**：在懂的基础上，让能力持续生长。不是所有实例都要走到 L4——L2 稳定运行对大多数组织已经足够，L3/L4 是为选择持续深化的实例准备的。

**L2 = Engineering Anything（帮人做事），L4 = 系统自进化（自己变强）**。这两个能力层次正交但互补：L2 是系统帮人完成任务的交付力，L4 是系统自我改进的元能力。L2 做得越多，L4 积累的进化信号就越丰富；L4 越强，L2 的交付质量和效率就越高。三角色在两个层次中各有定位：Lucas 是 L2 的需求感知器和交付入口，Andy/Lisa 是 L2 的交付引擎；Andy 是 L4 的核心引擎（主动巡检、设计判断进化），Lisa 是 L4 的信号源（实现反馈、错误信号）。

**Andy 是 L4 自进化的乘数**。三个角色各有进化边界：Lucas 进化 = 对这个组织理解更深，边界在成员关系的深度；Lisa 进化 = 实现能力更强，边界在代码库复杂度。Andy 的进化没有这个边界——Andy 的每一次判断质量提升，都直接作用于系统里每一条协作链：spec 精度提升一点，Lisa 的成功率就上升；Andy 吸收一条外部知识，下一个需求的设计就更准。**大脑进化快，整个组织就快**——这也是 L4 进化体系的核心投入应向 Andy 倾斜的根本原因。

**Lisa 是 L2 和 L4 的桥梁**。L2 的完整闭环是 Lucas 感知 → Andy 设计 → **Lisa 实现**——没有 Lisa，Engineering Anything 停在 spec 阶段，永远不是"交付"。同时，Lisa 的实现产出（codebase_patterns、错误信号、修复记录）是 L4 中 Andy 巡检的核心输入数据。Andy 发现能力缺口的信号，一半来自 Lisa 的实现过程。**Lisa 同时服务于两个层次：她是 L2 的交付手，也是 L4 的信号源。** 三角色的协作关系：Lucas 提需求 → Andy 设计 → Lisa 交付 + 反馈信号 → Andy 自我改进 → 回到 Lucas。

**Engineering Anything：L2 的本质是交付力，不是代码力**。L2 衡量的核心问题不是「系统能写多好的代码」，而是「家人说要是有个 XX 就好了，系统能做到吗」。帮分析一个菜谱、帮准备一份商业计划、帮整理今天的学习进度——这些都是 L2 应该覆盖的交付形态。**Coding 只是手段，组织成员需要的 Any Thing 才是目的。** 当系统的交付形态越来越多、成功率越来越高，L2 才算真正在运转。

**双维度回流机制**：L2 交付和 L4 自进化不是独立运转的两条线，而是通过三角色的协作形成闭环。五条已实现的回流路径：
...
6. **Main 评估回流**（Main→Andy）：Main 的 evaluate_system 评分沉淀到 evaluation-history.jsonl，Andy 在 HEARTBEAT 时消费最近评估趋势，退步维度高亮——Main 是系统的体检医生，诊断结果不只给系统工程师看，也喂给 Andy 驱动自我演化

**L2 基线数据（2026-04-14 首次测量）**：
- Engineering Anything 覆盖度：协作链层面 3 cancelled + 1 queued（原 3 failed 已清理，v666 自动重触发机制部署后待验证），交付物多样性 ≥4 种（app/code/message/chat/capability）
- L4 自我演化：Andy HEARTBEAT 定时执行正常（v663 修复生效），skill-candidates 89 条积累，andy-goals 1 条（基础设施补录，Andy 尚未自主写入进化目标），andy-learning-state 不存在（主动学习行为未产出）
...
**L2 与 L3 的核心差别：L3 带着人**。L2 是系统帮人做事——交付力越强，组织能做的事越多，但人不在进化循环里。L3 才是带着人一起进化：通过成员理解机制（画像/图谱/影子）持续深化对每个成员的认知，在真实沟通前帮人想清楚，让协作洞察回流影响下一次判断。**成员理解不是家庭专属——任何组织都需要理解成员**：家庭理解家人的情绪与关系，企业理解员工的职责与协作偏好，学校理解学生的学情与社交动态。机制相同，内容由实例注入。L2 让系统更强，L3 让组织运作更优——这是两件不同的事，设计时不能混同，评估时也不能混同。

**L2 与 L4 的核心差别：元层级**。L2 是"做什么"（功能能力），L4 是"如何做得更好"（元能力）。L2 的改进靠人（系统工程师修改 Readme → 刷新 Agent 认知 → 系统按新方向行动），L4 的改进靠系统自身（蒸馏/结晶/DPO 信号积累 → 自动提升）。**「正朔→认知」是系统工程师影响 L4 的最关键工作流**——不是直接改代码或替 Agent 做事，而是通过修改 Readme 定义方向，刷新 Agent 认知让系统按新方向行动，Agent 日常行为产生新数据，L4 机制从中提炼新能力，新能力反馈给系统工程师观察，形成闭环。

**L2 的本质对标：稳定输出 + 过程可见 + 可选干预**。

**设计背景**：Lucas 的协作链在运行中暴露了一个根本问题——家人提需求后，中间经历 Andy 调研→设计→Lisa 实现→验证的完整链路，但家人只看到起点（「收到，我来安排」）和终点（「做好了」），中间是黑箱。家人不知道系统在做什么、做了多久、是否遇到困难。这导致两个后果：①家人对复杂需求没有信心（不知道系统能不能搞定）②家人无法在过程中纠偏（等到交付时才发现方向错了）。

**认知转变**：最初我们对比 ClaudeCode CLI 是想在微信渠道复制终端体验（七阶段对标），但实践下来发现这是一个错误的比较维度。CLI 的优势不是「终端交互」，而是三个本质能力：①稳定输出（人提需求，系统可靠交付）②过程可见（人能看到系统在做什么）③可选干预（人可以随时介入，也可以不介入）。**交互媒介是终端还是微信不重要，这三个能力才是 L2 的真正竞争维度。**

**为什么 Lucas 的异步是优势而非缺陷**：ClaudeCode 是同步阻塞的——人必须坐在终端前等，复杂度高 = 人的等待成本高。Lucas 天然异步——家人提完需求可以去做别的事。异步 + 过程可见 + 可选干预，形成的体验比 CLI 更强：人不需要盯屏，但想知道时随时可查进度，想调整时随时可以介入。**异步让复杂度不再等于人的成本**——这是 L2 对标 CLI 时最本质的优势。

**为什么「稳定输出」是核心而非「过程可见」**：过程可见和可选干预是放大器，但稳定输出是基础。家人说要是有个 XX 就好了，系统稳定地把 XX 交付出来，这比「过程中每一步都可见但最后交付失败」重要得多。过程可见的真正价值是建立信任——家人看到系统在认真做，即使最终失败了也愿意再试。

**Vibe vs Harness——L2 的入口与出口**：对外说"harness"是对的——严谨、可控、让人信任。但人本来就不是带着完整需求来的。大多数时候就是一句"我想做点什么"，甚至只是觉得哪里不对劲。大模型最擅长的不是执行清晰指令——那是脚本干的事——它擅长的是接住那个模糊的 vibe，然后帮人一步步理清楚，最后做出一个连提需求的人自己都没想到能做成这样的事。**"harness"是系统给用户的标准，"vibe"是用户真实的状态。好的系统应该是：入口接得住 vibe，出口交得出 harness。** 这也是 Lucas 作为 Frontend Agent 的核心价值——不是等用户把需求想清楚了再来，而是从那句模糊的"我想做点什么"开始陪伴，到交付时给出一个结构化、可验证、能复用的产物。CrewClaw 框架中，Frontend Agent 负责"接住 vibe"（需求感知 + 陪伴澄清），Designer + Implementor 负责"交出 harness"（结构化 spec + 可验证交付）。

**四个实现方向的优先级逻辑**（重建者注意：优先级由"对稳定输出的贡献"排序，你的部署可能不同）：

1. **Andy/Lisa 大文件精准操作**（对稳定输出贡献最大，v683 已实现 `patch_file`）：当前 Andy 写 spec 引用大文件只能粗粒度描述改动位置，Lisa 用 opencode 实现时全文件覆盖——改一行也要重写整个文件，出错率高。一次交付失败 = 多轮修订 = 链路效率骤降。解决方案是让 Andy/Lisa 支持 diff-based 操作（精确到行/函数的修改），直接提高一次交付成功率。**这是直击「稳定输出」的正确路径——不是绕过链路，是让链路上每个节点交付更精准。** 为什么不让 Lucas 直接做简单任务绕过协作链？因为 Lucas 是需求感知器，不是实现者。让 Frontend Agent 兼任实现者会造成角色混淆（裁判兼运动员），"简单任务"的判断标准会漂移，链路逐渐退化成两套标准并存。框架层影响：第二个部署的 Frontend Agent 可能是客服/秘书，直接写代码不在角色范围内。
2. **流式状态同步**（对过程可见贡献最大，v683 已实现）：协作链三个阶段转换点（收到→设计完成→验收中）推送进度消息给 Lucas，由 Lucas 根据用户身份决定是否告知、用什么语气（千人千面）。**不直接推用户**——Lucas 了解每个家人的沟通偏好，能判断「妈妈问起时用大白话说进展」「爸爸可以直接看技术细节」。技术上是 `callGatewayAgent(FRONTEND_AGENT_ID, ...)` 在每个阶段转换点触发，不阻塞主链路。第二个部署需注意：推送频率要克制，过度推送等于噪音
3. **方案审批 gate**（对可选干预贡献最大，v683 已实现）：多方案时通过 Lucas 推送 alternatives + 超时默认。设计选择：我们不用 CLI 的「每步审批阻塞」模式，而是「告知 Lucas + 默认继续 + 事后可切换」。原因是企业微信不是终端——家人不会一直盯着对话框，阻塞等待会导致任务永久卡住。超时默认是非阻塞渠道的唯一可行方案。方案选择也走 Lucas——Lucas 用家人能理解的方式解释方案差异，家人告诉 Lucas 想换，Lucas 调 `select_spec_approach`
4. **小弟并行**（v682 已落地）：Andy/Lisa 内部并行提速。框架层机制——`create_sub_agent` / `evict_sub_agent` 已实现，行为层规则在 AGENTS.md。第二个部署直接可用
...
**进化安全**：防止坏的信号误导进化方向。能力结晶不可逆（固化容易，清除需要人工干预），因此进化协作链的每个环节都必须有可信任的判断主体——触发方必须经过身份验证，固化决策必须有唯一责任人，每次能力变更有来源记录，系统工程师定期观察进化健康状态。安全的本质：不是阻止进化，而是确保进化的驱动力来自真实可信的组织行为。
...
L2 建成后 工作流标准化 → V 字协作链稳定跑，不需要人工推动每一步
...
评估看的是轨迹，不是快照。某一时刻的分数只告诉你「现在怎么样」，轨迹告诉你「在往哪里走」。L4 最重要的评估不是当前 Skill 数量，而是 Skill 积累速率是否在加快；不是当前错误率，而是同类错误是否在下降。**改善率本身的改善（二阶导数 > 0），才是系统自我演化的真正证明。**
...
| 工具 | 机制 |
|------|------|
| `read` / `edit` / `write` / `bash` / `grep` / `find` / `ls` | **直接编码工具**（v687）：read 读文件、edit 精准替换、write 创建/覆盖、bash 执行命令、grep 搜索代码、find 查找文件、ls 列目录。简单任务直接编码优先，更快更可控 |
...
**凌晨时序总览（gateway-watchdog 调度，v668 重排）**：
...
#### L1 Skill 自主管理（Agent 主动层，v673 落地；flag_for_skill 三角色共用，v678 落地）
...
**设计原则（v678 更新）**：三角色均可发信号，不限于 Lucas。信号要轻量——不需要分析，只说「这类模式值得 Andy 看一下」。弱信号（首次遇到）即可标记，不需要等重复 2+ 次；确定稳定的模式直接用 `skill_manage create`，不必先 flag。
...
```
// write_lucas_skill 已废弃（v673），功能由 skill_manage 工具统一取代
// 详见上方「L1 Skill 自主管理（Agent 主动层）」节的 skill_manage 签名
```

`write_file` 是三角色共享工具（v671 从 Andy 专属扩展），原子写入（tmp→rename），路径限制 `$HOME/` 内。不同角色的 workspace 在不同目录，需要此工具确保路径正确且权限可控。
...
**Andy Spec SFT 积累管道**：Andy 生成的每条 spec 自动写入 `data/learning/andy-spec-finetune-queue.jsonl`（格式：requirement + context + spec + specQuality 元数据）；opencode 完成后回填 outcome（`exitCode + specMatchRate`），`eligibleForTraining = exitCode==0 && specMatchRate>=0.5`。工具链：`seed-andy-spec-queue.py`（12 条历史 spec 种子导入）→ `prepare-andy-sft-data.py`（过滤 eligible → 生成 MLX messages 格式 train.jsonl + valid.jsonl）。当前积累 9/12 条有效，50 条时触发 LoRA SFT 训练命令提示。

**本地模型**：`homeai-assistant`（基于 Qwen2.5-Coder-32B-4bit LoRA 微调，已注册到 Ollama），微调闭环已接通（数据桥接→LoRA训练→fuse→GGUF→ollama create），3416 条样本就绪。工具链：MLX LoRA 微调 → 权重 fuse → GGUF 量化 → Ollama 注册。`Gemma 4 31B` 为进化终态，就绪后切换。
...
Main 通过 `evaluate_l4` 持续跟踪三个维度：① 按 pattern_type 统计 DPO 信号积累进度（距 50 条内化阈值还差多少，近 7 天 vs 前 7 天趋势）；② L4 系统层临时干预效果（AGENTS.md 幻觉禁令条数，判断补丁是否在收敛问题）；③ 本地模型状态（homeai-assistant 已注册到 Ollama，微调闭环已接通，3416 条样本就绪；Gemma 4 为进化终态）。L4 系统层（AGENTS.md 规则）是 L4 模型层未就绪前的临时拦截手段；模型层就绪后，对应系统层补丁随微调完成逐步撤除，SOUL.md 同步瘦身。
...
| **Lucas**               | 对话入口，意图识别，任务编排 + 项目管理（工期感知/阻塞信号/紧急排序/交付闭环），记忆管理，系统干预通报 | OpenClaw embedded agent、qwen3.6-plus                |
| **Andy**                | 工具设计 + 系统思考者，自主写 spec，代码图谱理解，三维主动性（事件感知/知识获取/自主判断），触发 Lisa，验收 Lisa 交付 | OpenClaw embedded agent、DeepSeek Reasoner R1       |
| **Lisa**                | 直接编码优先，复杂任务 run_opencode（background 异步），验证，结果回传 Andy 验收     | OpenClaw embedded agent、GLM-5.1                |
...
## 二、CrewClaw L2/L3 框架机制 + HomeAI 实现记录
...
**L2 对标 CLI 的本质维度**（v683 设计）：上述七阶段对齐是「怎么做」的实现层对标。但重建者需要理解一个更本质的设计判断——**L2 不应该在「像不像 CLI」上竞争，而应该在三个维度的组合效果上竞争**。
...
| **L3 跨成员感知** | ✅ 完整协作中介 | ⚠️ 受限（不可见核心成员私密信息） | 隐私保护是硬边界 |
