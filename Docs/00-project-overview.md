...
**Andy 是 L4 自进化的乘数**。三个角色各有进化边界：Lucas 进化 = 对这个组织理解更深，边界在成员关系的深度；Lisa 进化 = 实现能力更强，边界在代码库复杂度。Andy 的进化没有这个边界——Andy 的每一次判断质量提升，都直接作用于系统里每一条流水线：spec 精度提升一点，Lisa 的成功率就上升；Andy 吸收一条外部知识，下一个需求的设计就更准。**大脑进化快，整个组织就快**——这也是 L4 进化体系的核心投入应向 Andy 倾斜的根本原因。
...
- Engineering Anything 覆盖度：流水线层面 3 cancelled + 1 queued（原 3 failed 已清理，v666 自动重触发机制部署后待验证），交付物多样性 ≥4 种（app/code/message/chat/capability）
...
| `evaluate_l2`          | Engineering Anything 交付能力：任务类型覆盖度（task-registry 去重）、端到端交付成功率（opencode 近 10 次成功率）、交付物多样性（App/generated/ 文件数）、**三角色流水线健康**（task-registry completed/failed 比例，不含 cancelled/queued）|
...
| `BACKGROUND.md` | 项目前世今生：HomeAI 是什么 / 幕后队友 Andy & Lisa / 双向V字流水线 / L0-L4 里程碑 / 文档查阅地图 | 系统工程师手工维护 | `context-sources` static-file appendSystemContext |
...
| `BACKGROUND.md` | 项目前世今生：框架打样定位 / 四角色结构 / 双向V字流水线 / L0-L4 演进 / 文档查阅地图（含何时读哪个文档）| 系统工程师手工维护 | `context-sources` static-file appendSystemContext |
...
| `BACKGROUND.md` | 项目前世今生：框架打样定位 / 四角色结构 / 流水线 / L0-L4 演进 / 文档查阅地图（**含 10-engineering-notes.md 必查提醒**）| 系统工程师手工维护 | `context-sources` static-file appendSystemContext |
...
静态管道：稳定知识（变化频率低，每次 session 都需要注入）
  ...
  AGENTS.md             人工节永久不动；Kuzu 渲染节由结晶流水线触发更新  L4：结晶流水线触发
...
| `SOUL.md` / `AGENTS.md` 渲染节 | `render-knowledge.py` | L4 结晶流水线触发 |
...
- **基础机制**：**V字流水线**是能力进化的驱动引擎。**三角色**在任务执行中感知弱信号（`flag_for_skill`）或 Andy 冷路径巡检识别高频模式，触发协作提炼：Andy 判断结晶形式（Skill / Tool / Web App）→ 直接写 Skill 或驱动 Lisa 实现 → 登记 capability-registry。流水线每跑一次 = 系统多一个能力。
...
> **设计来源**：Claude Code 的可靠性不靠模型强，靠工程结构保证。CrewClaw 流水线应该一样——机制保证交付质量，不依赖 Designer/Implementor 的模型自觉。

三条反馈回路保证了流水线能自我纠偏，但还有一个缺口：**Implementor 说"做完了"但实际没有交付**（`changedFiles: []`）。这是模型层无法解决的问题——需要基础设施层独立验证。
...
- 访客（`visitor:` 前缀）通过 `pushEventDriven` → 内存队列 → 前端轮询，与家人走同一流水线
...
**机制四：ClaudeCode 流水线体验对齐**

> **设计来源**：ClaudeCode CLI 的 7 阶段工作流是当前最成熟的 AI 辅助开发体验基准。CrewClaw 流水线在微信渠道无法复制 CLI 的实时 TaskList/进度条，但通过多节点通知 + 结构化输出对齐核心体验。
...
**用户审批模式**：与 ClaudeCode 不同，CrewClaw 采用「告知 + 默认继续 + 事后可切换」模式，不阻塞流水线等待用户批准。用户可在任意阶段通过 `cancel_task` 叫停，或通过 `select_spec_approach` 切换方案后重新触发。
