...
```
框架层（源文件，不直接使用）：
  ~/HomeAI/CrewHiveClaw/CrewClaw/daemons/workspace-templates/{agent}/skills/
...
# 语法验证（修改插件后）
cd ~/HomeAI/CrewHiveClaw/CrewClaw/crewclaw-routing
...
插件代码：  ~/HomeAI/CrewHiveClaw/CrewClaw/crewclaw-routing/index.ts
wecom 入口：~/HomeAI/CrewHiveClaw/CrewClaw/daemons/entrances/wecom/index.js
Gateway 启动：~/.openclaw/start-gateway.sh
Gateway 日志：~/.openclaw/logs/gateway.log
框架层 Skills：~/HomeAI/CrewHiveClaw/CrewClaw/daemons/workspace-templates/{agent}/skills/
watchdog：  ~/HomeAI/CrewHiveClaw/HomeAILocal/Scripts/gateway-watchdog.js
```
