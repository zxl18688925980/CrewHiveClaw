...
**触发**：Lucas 对开发完成状态的感知全靠记忆传递，缺乏基础设施约束——流水线完成了但家人不知道是系统性缺口，不是偶发问题。
...
**文档同步**：`00-project-overview.md` 流水线阶段可见性行 / 工具表 / Lucas 核心能力 / 数据流图 / 任务注册表描述；`HomeAI Readme.md` Lucas 角色列 + 协作段描述。
...
**背景**：三角色间原有通信是单向或半双工的——Andy 可通过 `query_requirement_owner` 主动找 Lucas 澄清需求，但 Lucas 无法主动联系 Andy 或 Lisa（只能被动等待，或走完整流水线）。观察到 Lucas 对双向通道不了解（他认为只能「单向提交需求」），决定补齐工具层并更新人格文件。

**新增 `ask_andy` 工具（Lucas 专属）**：
- 直接向 Andy 发出技术/设计/可行性问题，Andy 同步回答，不触发开发流水线
...
- 直接向 Lisa 咨询实现进展/代码细节，Lisa 同步回答，不触发流水线
...
**背景**：L2 的 Vibe Anything × 自进化飞轮框架已建立，但两个子层的主力归属一直隐含未说清。Lucas 是 Vibe Anything 的主力（感知外部需求触发流水线），Andy 是自进化飞轮的主力（主动巡检系统信号推动能力进化）——这个分工在设计文档里没有对称表达，Andy AGENTS.md 里的三维主动性体系也缺少「使命声明」，Check 13 停在「提案」层面而非「直接行动」。用户定位：Andy 在系统能力自进化上要有意志感，不是等人触发，是主动推动。
