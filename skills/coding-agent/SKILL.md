...
**⚠️ CRITICAL: Never review PRs in CrewClaw's own project folder!**
...
9. **NEVER checkout branches in ~/Projects/openclaw/** - that's the LIVE CrewClaw instance!
...
For long-running background tasks, append a wake trigger to your prompt so CrewClaw gets notified immediately when the agent finishes (instead of waiting for the next heartbeat):
