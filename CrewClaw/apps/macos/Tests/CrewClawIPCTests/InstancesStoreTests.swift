...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: CrewClawProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: CrewClawProtocol.AnyCodable] = [
            "presence": .init([CrewClawProtocol.AnyCodable(entry)]),
        ]
        let payload = CrewClawProtocol.AnyCodable(payloadMap)
