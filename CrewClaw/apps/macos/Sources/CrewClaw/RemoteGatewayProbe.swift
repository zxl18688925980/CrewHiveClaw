...
enum RemoteGatewayAuthIssue: Equatable {
    ...
    var body: String {
        ...
        case .setupCodeExpired:
            "Scan or paste a fresh setup code from an already-paired CrewClaw client, then try again."
        case .passwordRequired:
            "This onboarding flow does not support password auth yet. Reconfigure the gateway to use token auth, then retry."
        case .pairingRequired:
            "Approve this device from an already-paired CrewClaw client. In your CrewClaw chat, run `/pair approve`, then click **Check connection** again."
        }
    }
    ...
    var footnote: String? {
        ...
        case .setupCodeExpired:
            nil
        case .pairingRequired:
            "If you do not have another paired CrewClaw client yet, approve the pending request on the gateway host with `openclaw devices approve`."
    ...
    var statusMessage: String {
        ...
        case .pairingRequired:
            "Pairing required. In an already-paired CrewClaw client, run /pair approve, then check the connection again."
        }
    }
}
