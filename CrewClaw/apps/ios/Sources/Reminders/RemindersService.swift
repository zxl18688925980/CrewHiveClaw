...
final class RemindersService: RemindersServicing {
    func list(params: CrewClawRemindersListParams) async throws -> CrewClawRemindersListPayload {
        ...
        let payload: [CrewClawReminderPayload] = try await withCheckedThrowingContinuation { cont in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return CrewClawReminderPayload(
        ...
        return CrewClawRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: CrewClawRemindersAddParams) async throws -> CrewClawRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = CrewClawReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return CrewClawRemindersAddPayload(reminder: payload)
    }
