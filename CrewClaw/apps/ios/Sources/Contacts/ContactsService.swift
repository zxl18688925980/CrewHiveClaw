...
final class ContactsService: ContactsServicing {
    ...
    func search(params: CrewClawContactsSearchParams) async throws -> CrewClawContactsSearchPayload {
        ...
        return CrewClawContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: CrewClawContactsAddParams) async throws -> CrewClawContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return CrewClawContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return CrewClawContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> CrewClawContactPayload {
        CrewClawContactPayload(
