# CrewClaw iOS (Super Alpha)

This iPhone app is super-alpha and internal-use only. It connects to an CrewClaw Gateway as a `role: node`.
...
- Beta release also switches the app to `CrewClawPushTransport=relay`, `CrewClawPushDistribution=official`, and `CrewClawPushAPNsEnvironment=production`.
...
- `apps/ios/Sources/CrewClaw.entitlements` sets `aps-environment` to `development`.
- APNs token registration to gateway happens only after gateway connection (`push.apns.register`).
- Local/manual builds default to `CrewClawPushTransport=direct` and `CrewClawPushDistribution=local`.
- Your selected team/profile must support Push Notifications for the app bundle ID you are signing.
- If push capability or provisioning is wrong, APNs registration fails at runtime (check Xcode logs for `APNs registration failed`).
- Debug builds default to `CrewClawPushAPNsEnvironment=sandbox`; Release builds default to `production`.
...
This exists to keep the hosted relay limited to genuine CrewClaw official builds and to ensure a
