...
function parseInstallSpec(input: unknown): SkillInstallSpec | undefined {
  const parsed = parseCrewClawManifestInstallBase(input, ["brew", "node", "go", "uv", "download"]);
  if (!parsed) {
    return undefined;
  }
  const { raw } = parsed;
  const spec = applyCrewClawManifestInstallCommonFields<SkillInstallSpec>(
    {
...
): CrewClawSkillMetadata | undefined {
