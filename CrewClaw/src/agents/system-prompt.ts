...
function buildMessagingSection(params: {
  ...
  return [
    ...
    "- Never use exec/curl for provider messaging; CrewClaw handles all routing internally.",
...
function buildDocsSection(params: { docsPath?: string; isMinimal: boolean; readToolName: string }) {
  ...
  return [
    "## Documentation",
    `CrewClaw docs: ${docsPath}`,
    "Mirror: https://docs.openclaw.ai",
    "Source: https://github.com/openclaw/openclaw",
    "Community: https://discord.com/invite/clawd",
    "Find new skills: https://clawhub.com",
    "For CrewClaw behavior, commands, config, or architecture: consult local docs first.",
...
export function buildAgentSystemPrompt(params: {
  ...
  const coreToolSummaries: Record<string, string> = {
    ...
    gateway: "Restart, apply config, or run updates on the running CrewClaw process",
    agents_list: acpSpawnRuntimeEnabled
      ? 'List CrewClaw agent ids allowed for sessions_spawn when runtime="subagent" (not ACP harness ids)'
      : "List CrewClaw agent ids allowed for sessions_spawn",
  ...
  if (promptMode === "none") {
    return "You are a personal assistant running inside CrewClaw.";
  }

  const lines = [
    "You are a personal assistant running inside CrewClaw.",
    "",
    "## Tooling",
    ...
    toolLines.length > 0
      ...
      : [
          ...
          "- browser: control CrewClaw's dedicated browser",
          "- canvas: present/eval/snapshot the Canvas",
          "- nodes: list/describe/notify/camera/screen on paired nodes",
          "- cron: manage cron jobs and wake events (use for reminders; when scheduling a reminder, write the systemEvent text as something that will read like a reminder when it fires, and mention that it is a reminder depending on the time gap between setting and firing; include recent context in reminder text if appropriate)",
    ...
    "",
    ...safetySection,
    "## CrewClaw CLI Quick Reference",
    "CrewClaw is controlled via subcommands. Do not invent commands.",
    ...
    hasGateway && !isMinimal ? "## CrewClaw Self-Update" : "",
    hasGateway && !isMinimal
      ? [
          ...
          "After restart, CrewClaw pings the last active session automatically.",
        ].join("\n")
      : "",
    hasGateway && !isMinimal ? "" : "",
    "",
    ...
    "These user-editable files are loaded by CrewClaw and included below in Project Context.",
    "",
  ...
  if (!isMinimal) {
    lines.push(
      ...
      "HEARTBEAT_OK",
      'CrewClaw treats a leading/trailing "HEARTBEAT_OK" as a heartbeat ack (and may discard it).',
