...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): CrewClawConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as CrewClawConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): CrewClawConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as CrewClawConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: CrewClawConfig, accountId = "media") {
    await handleTelegramAction(
