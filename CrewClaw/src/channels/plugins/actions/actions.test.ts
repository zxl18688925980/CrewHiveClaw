...
function telegramCfg(): CrewClawConfig {
  return { channels: { telegram: { botToken: "tok" } } } as CrewClawConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: CrewClawConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as CrewClawConfig);
...
function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as CrewClawConfig;
...
function createSignalAccountOverrideCfg(): CrewClawConfig {
  return {
    channels: {
      signal: {
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as CrewClawConfig;
  ...
  it("lists moderation when at least one account enables it", () => {
    ...
    for (const channelConfig of cases) {
      const cfg = channelConfig as unknown as CrewClawConfig;
      const actions = discordMessageActions.listActions?.({ cfg }) ?? [];
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as CrewClawConfig,
        accountId: undefined,
        params: {
