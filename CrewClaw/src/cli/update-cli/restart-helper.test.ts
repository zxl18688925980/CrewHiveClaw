...
describe("restart-helper", () => {
  ...
  describe("prepareRestartScript", () => {
    ...
    it("creates a schtasks restart script on Windows", async () => {
      ...
      expect(content).toContain('schtasks /End /TN "CrewClaw Gateway"');
      expect(content).toContain('schtasks /Run /TN "CrewClaw Gateway"');
    ...
    it("uses OPENCLAW_WINDOWS_TASK_NAME override on Windows", async () => {
      ...
      expect(content).toContain('schtasks /End /TN "CrewClaw Gateway (custom)"');
      expect(content).toContain('schtasks /Run /TN "CrewClaw Gateway (custom)"');
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in Windows task name", async () => {
      ...
      expect(content).toContain('schtasks /End /TN "CrewClaw Gateway (production)"');
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
