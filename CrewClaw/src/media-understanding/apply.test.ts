...
function createGroqAudioConfig(): CrewClawConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): CrewClawConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): CrewClawConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: CrewClawConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  ...
  beforeAll(async () => {
    const baseDir = resolvePreferredCrewClawTmpDir();
