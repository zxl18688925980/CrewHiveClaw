...
function rawKeyValue(config: CrewClawConfig, provider: SearchProvider): unknown {
...
export function hasExistingKey(config: CrewClawConfig, provider: SearchProvider): boolean {
...
function applyProviderOnly(config: CrewClawConfig, provider: SearchProvider): CrewClawConfig {
  return {
    ...config,
    tools: {
      ...config.tools,
...
function preserveDisabledState(original: CrewClawConfig, result: CrewClawConfig): CrewClawConfig {
...
export async function setupSearch(
  ...
  if (useSecretRefMode) {
    ...
    await prompter.note(
      [
        "Secret references enabled — CrewClaw will store a reference instead of the API key.",
