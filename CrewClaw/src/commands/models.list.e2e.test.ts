...
const ensureCrewClawModelsJson = vi.fn().mockResolvedValue(undefined);
const resolveCrewClawAgentDir = vi.fn().mockReturnValue("/tmp/openclaw-agent");
...
vi.mock("../agents/models-config.js", () => ({
  ensureCrewClawModelsJson,
}));

vi.mock("../agents/agent-paths.js", () => ({
  resolveCrewClawAgentDir,
}));
...
beforeEach(() => {
  ...
  ensureCrewClawModelsJson.mockClear();
...
describe("models list/status", () => {
  ...
  it("loadModelRegistry does not persist models.json as a side effect", async () => {
    ...
    expect(ensureCrewClawModelsJson).not.toHaveBeenCalled();
  });
  ...
  it("modelsListCommand persists using the write snapshot config when provided", async () => {
    ...
    expect(ensureCrewClawModelsJson).toHaveBeenCalled();
    expect(ensureCrewClawModelsJson.mock.calls[0]?.[0]).toEqual(resolvedConfig);
  });
