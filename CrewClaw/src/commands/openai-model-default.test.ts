...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: CrewClawConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: CrewClawConfig) => { changed: boolean; next: CrewClawConfig };
  defaultModel: string;
  overrideConfig: CrewClawConfig;
  alreadyDefaultConfig: CrewClawConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: CrewClawConfig) => config,
      applyDefaultConfig: (config: CrewClawConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: CrewClawConfig = { agents: { defaults: {} } };
