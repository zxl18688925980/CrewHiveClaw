...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: CrewClawConfig | null = null;
  ...
  get(key: string, cfg: CrewClawConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: CrewClawConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: CrewClawConfig): void {
