...
describe("resolveCrewClawPackageRoot", () => {
  let resolveCrewClawPackageRoot: typeof import("./openclaw-root.js").resolveCrewClawPackageRoot;
  let resolveCrewClawPackageRootSync: typeof import("./openclaw-root.js").resolveCrewClawPackageRootSync;

  beforeAll(async () => {
    ({ resolveCrewClawPackageRoot, resolveCrewClawPackageRootSync } =
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    expect(resolveCrewClawPackageRootSync({ argv1 })).toBe(pkgRoot);
  });
  ...
  it("resolves package root via symlinked argv1", async () => {
    ...
    expect(resolveCrewClawPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveCrewClawPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });
  ...
  it("falls through from a non-openclaw moduleUrl candidate to cwd", async () => {
    ...
    expect(resolveCrewClawPackageRootSync({ moduleUrl, cwd: cwdPkgRoot })).toBe(cwdPkgRoot);
    await expect(resolveCrewClawPackageRoot({ moduleUrl, cwd: cwdPkgRoot })).resolves.toBe(
      cwdPkgRoot,
    );
  });
  ...
  it("ignores invalid moduleUrl values and falls back to cwd", async () => {
    ...
    expect(resolveCrewClawPackageRootSync({ moduleUrl: "not-a-file-url", cwd: pkgRoot })).toBe(
      pkgRoot,
    );
    await expect(
      resolveCrewClawPackageRoot({ moduleUrl: "not-a-file-url", cwd: pkgRoot }),
  ...
  it("returns null for non-openclaw package roots", async () => {
    ...
    expect(resolveCrewClawPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });
  ...
  it("async resolver matches sync behavior", async () => {
    ...
    await expect(resolveCrewClawPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });

  it("async resolver returns null when no package roots exist", async () => {
    await expect(resolveCrewClawPackageRoot({ cwd: fx("missing") })).resolves.toBeNull();
  });
});
