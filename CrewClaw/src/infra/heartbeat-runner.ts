...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: CrewClawConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: CrewClawConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: CrewClawConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: CrewClawConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: CrewClawConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: CrewClawConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: CrewClawConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
