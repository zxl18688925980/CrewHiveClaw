...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredCrewClawTmpDir: vi.fn(() => "/tmp/openclaw"),
}));
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredCrewClawTmpDir.mockReturnValue("/tmp/openclaw");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredCrewClawTmpDir.mockReturnValue("/tmp/openclaw-preferred");
    ...
    expect(tmpDirMocks.resolvePreferredCrewClawTmpDir).toHaveBeenCalled();
  });
