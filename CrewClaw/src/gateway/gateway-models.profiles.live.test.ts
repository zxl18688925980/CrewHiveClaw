...
import { resolveCrewClawAgentDir } from "../agents/agent-paths.js";
...
import { ensureCrewClawModelsJson } from "../agents/models-config.js";
import { isRateLimitErrorMessage } from "../agents/pi-embedded-helpers/errors.js";
import { discoverAuthStorage, discoverModels } from "../agents/pi-model-discovery.js";
import { loadConfig } from "../config/config.js";
import type { ModelsConfig, CrewClawConfig, ModelProviderConfig } from "../config/types.js";
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<CrewClawConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  ...
  const hostAgentDir = resolveCrewClawAgentDir();
  ...
  const agentDir = resolveCrewClawAgentDir();
  const sanitizedCfg: CrewClawConfig = {
    ...params.cfg,
            ...
            const toolProbe = await withGatewayLiveProbeTimeout(
              client.request<AgentFinalPayload>(
                "agent",
                {
                  ...
                  message: strictReply
                    ? "CrewClaw live tool probe (local, safe): " +
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureCrewClawModelsJson(cfg);
