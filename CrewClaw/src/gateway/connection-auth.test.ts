...
function cfg(input: Partial<CrewClawConfig>): CrewClawConfig {
  return input as CrewClawConfig;
}
...
async function expectFailClosedOnUnresolvedLocalAuth(config: CrewClawConfig, path: string) {
  await expect(
    resolveGatewayConnectionAuth({
      config,
