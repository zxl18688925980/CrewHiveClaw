...
let lastCreateCrewClawToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/openclaw-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateCrewClawToolsContext?.agentTo,
          agentThreadId: lastCreateCrewClawToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createCrewClawTools: (ctx: Record<string, unknown>) => {
      lastCreateCrewClawToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateCrewClawToolsContext = undefined;
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateCrewClawToolsContext?.allowMediaInvokeCommands).toBe(true);
