...
import { loadConfig, type CrewClawConfig, writeConfigFile } from "../config/config.js";
...
function createOpenAiFileModelsConfig(): NonNullable<CrewClawConfig["models"]> {
  return {
    providers: {
      openai: {
...
describe("secrets runtime snapshot", () => {
  ...
  it("normalizes inline SecretRef object on token to tokenRef", async () => {
    const config: CrewClawConfig = { models: {}, secrets: {} };
    const snapshot = await prepareSecretsRuntimeSnapshot({
      config,
