...
type AgentDefaults = NonNullable<NonNullable<CrewClawConfig["agents"]>["defaults"]>;
...
export function buildStatusMessage(args: StatusArgs): string {
  ...
  const versionLine = `🦞 CrewClaw ${VERSION}${commit ? ` (${commit})` : ""}`;
...
export function buildHelpMessage(cfg?: CrewClawConfig): string {
  const lines = ["ℹ️ Help", ""];
