...
import { resolveCrewClawPackageRootSync } from "../infra/openclaw-root.js";
...
import { discoverCrewClawPlugins } from "./discovery.js";
...
function listPluginSdkExportedSubpaths(params: { modulePath?: string } = {}): string[] {
  const modulePath = params.modulePath ?? fileURLToPath(import.meta.url);
  const packageRoot = resolveCrewClawPackageRootSync({
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: CrewClawPluginDefinition;
  register?: CrewClawPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as CrewClawPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as CrewClawPluginDefinition;
...
export function loadCrewClawPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverCrewClawPlugins({
    workspaceDir: options.workspaceDir,
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: CrewClawPluginModule | null = null;
    try {
      mod = getJiti()(safeSource) as CrewClawPluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
