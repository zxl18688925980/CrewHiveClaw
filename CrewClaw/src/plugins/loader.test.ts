...
function expectTelegramLoaded(registry: ReturnType<typeof loadCrewClawPlugins>) {
...
function loadRegistryFromSinglePlugin(params: {
  ...
  options?: Omit<Parameters<typeof loadCrewClawPlugins>[0], "cache" | "workspaceDir" | "config">;
}) {
...
describe("loadCrewClawPlugins", () => {
  ...
  it("re-initializes global hook runner when serving registry from cache", () => {
    ...
    const first = loadCrewClawPlugins(options);
    expect(getGlobalHookRunner()).not.toBeNull();

    resetGlobalHookRunner();
    expect(getGlobalHookRunner()).toBeNull();

    const second = loadCrewClawPlugins(options);
  ...
  it("does not reuse cached bundled plugin registries across env changes", () => {
    ...
    const first = loadCrewClawPlugins({
      ...options,
      env: {
        ...process.env,
    ...
    const second = loadCrewClawPlugins({
      ...options,
      env: {
        ...process.env,
  ...
  it("does not reuse cached registries when env-resolved install paths change", () => {
    ...
    const second = loadCrewClawPlugins(secondOptions);
    const third = loadCrewClawPlugins(secondOptions);
  ...
  register() {},
    ...
    const script = `
      import { loadCrewClawPlugins } from ${JSON.stringify(loaderModuleUrl)};
      const registry = loadCrewClawPlugins({
        cache: false,
