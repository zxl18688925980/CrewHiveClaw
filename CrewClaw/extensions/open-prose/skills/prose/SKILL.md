...
- **Task tool** in the upstream spec == CrewClaw `sessions_spawn`
- **File I/O** == CrewClaw `read`/`write`
- **Remote fetch** == CrewClaw `web_fetch` (or `exec` with curl when POST is required)
