...
import { resolvePreferredCrewClawTmpDir } from "../../../src/infra/tmp-openclaw-dir.js";
...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(
    path.join(resolvePreferredCrewClawTmpDir(), "openclaw-media-test-"),
  );
...
describe("web media loading", () => {
  beforeAll(() => {
    ...
    // Also keep it outside the CrewClaw temp root so default localRoots doesn't accidentally make all state readable.
...
describe("local media root guard", () => {
  ...
  it("allows local paths under an explicit root", async () => {
    const result = await loadWebMedia(tinyPngFile, 1024 * 1024, {
      localRoots: [resolvePreferredCrewClawTmpDir()],
    });
    expect(result.kind).toBe("image");
  });
  ...
  it("allows default CrewClaw state workspace and sandbox roots", async () => {
  ...
  it("rejects default CrewClaw state per-agent workspace-* roots without explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));
