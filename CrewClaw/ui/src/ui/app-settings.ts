...
import type { CrewClawApp } from "./app.ts";
...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "instances") {
    await loadPresence(host as unknown as CrewClawApp);
  }
  if (host.tab === "sessions") {
    await loadSessions(host as unknown as CrewClawApp);
  }
  if (host.tab === "cron") {
    await loadCron(host);
  }
  if (host.tab === "skills") {
    await loadSkills(host as unknown as CrewClawApp);
  }
  if (host.tab === "agents") {
    await loadAgents(host as unknown as CrewClawApp);
    await loadConfig(host as unknown as CrewClawApp);
    const agentIds = host.agentsList?.agents?.map((entry) => entry.id) ?? [];
    if (agentIds.length > 0) {
      void loadAgentIdentities(host as unknown as CrewClawApp, agentIds);
    }
    const agentId =
      host.agentsSelectedId ?? host.agentsList?.defaultId ?? host.agentsList?.agents?.[0]?.id;
    if (agentId) {
      void loadAgentIdentity(host as unknown as CrewClawApp, agentId);
      if (host.agentsPanel === "skills") {
        void loadAgentSkills(host as unknown as CrewClawApp, agentId);
      }
      if (host.agentsPanel === "channels") {
        void loadChannels(host as unknown as CrewClawApp, false);
      }
  ...
  if (host.tab === "nodes") {
    await loadNodes(host as unknown as CrewClawApp);
    await loadDevices(host as unknown as CrewClawApp);
    await loadConfig(host as unknown as CrewClawApp);
    await loadExecApprovals(host as unknown as CrewClawApp);
  }
  if (host.tab === "chat") {
  ...
  if (
    ...
    await loadConfigSchema(host as unknown as CrewClawApp);
    await loadConfig(host as unknown as CrewClawApp);
  }
  if (host.tab === "debug") {
    await loadDebug(host as unknown as CrewClawApp);
    host.eventLog = host.eventLogBuffer;
  }
  if (host.tab === "logs") {
    host.logsAtBottom = true;
    await loadLogs(host as unknown as CrewClawApp, { reset: true });
...
export async function loadOverview(host: SettingsHost) {
  const app = host as unknown as CrewClawApp;
...
async function loadOverviewLogs(host: CrewClawApp) {
...
function buildAttentionItems(host: CrewClawApp) {
...
export async function loadChannelsTab(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as CrewClawApp, true),
    loadConfigSchema(host as unknown as CrewClawApp),
    loadConfig(host as unknown as CrewClawApp),
  ]);
}
