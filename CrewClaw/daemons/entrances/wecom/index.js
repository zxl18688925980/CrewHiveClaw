...
function initDemoRoutes() {
  const result = _demoRoutesFactory(logger, { HOMEAI_ROOT, GATEWAY_URL, APP_GENERATED_DIR });
  app.use(result.router);
  ({ loadInvites, resolveInviteCode, isInviteValid } = result);
}
function initSEDashboard() {
  app.use(_seDashboardFactory(logger, { WECOM_OWNER_ID, HOMEAI_ROOT, readTaskRegistry, writeTaskRegistry, loadInvites }));
}
...
app.use(_evalDashboardFactory(logger, { HOMEAI_ROOT }));
