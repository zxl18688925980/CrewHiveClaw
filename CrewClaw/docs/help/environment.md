---
summary: "Where CrewClaw loads environment variables and the precedence order"
read_when:
...
CrewClaw pulls environment variables from multiple sources. The rule is **never override existing values**.
...
CrewClaw also injects context markers into spawned child processes:
...
- `COLORFGBG`: if your terminal exports it, CrewClaw uses the background color hint to auto-pick the TUI palette.
...
CrewClaw supports two env-driven patterns:
...
| Variable               | Purpose                                                                                                                                                                          |
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `OPENCLAW_HOME`        | Override the home directory used for all internal path resolution (`~/.openclaw/`, agent dirs, sessions, credentials). Useful when running CrewClaw as a dedicated service user. |
