---
summary: "Symptom first troubleshooting hub for CrewClaw"
read_when:
  - CrewClaw is not working and you need the fastest path to a fix
...
is using an old shape that CrewClaw no longer accepts.
...
```mermaid
flowchart TD
  A[CrewClaw is not working] --> B{What breaks first}
  B --> C[No replies]
