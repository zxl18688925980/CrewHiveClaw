...
- With Anthropic API-key auth profiles, CrewClaw seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, CrewClaw injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
CrewClaw exposes dedicated cache-trace diagnostics for embedded agent runs.
