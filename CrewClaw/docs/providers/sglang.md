---
summary: "Run CrewClaw with SGLang (OpenAI-compatible self-hosted server)"
read_when:
  - You want to run CrewClaw against a local SGLang server
...
CrewClaw can connect to SGLang using the `openai-completions` API.

CrewClaw can also **auto-discover** available models from SGLang when you opt
...
define `models.providers.sglang`, CrewClaw will query:
