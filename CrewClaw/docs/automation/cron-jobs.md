...
To reduce top-of-hour load spikes across many gateways, CrewClaw applies a
...
If `delivery` is omitted for isolated jobs, CrewClaw defaults to `announce`.
...
When a job fails, CrewClaw classifies errors as **transient** (retryable) or **permanent** (disable immediately).
...
- For removed run sessions no longer referenced by the session store, CrewClaw archives transcript files and purges old deleted archives on the same retention window.
...
- CrewClaw applies exponential retry backoff for recurring jobs after consecutive errors:
