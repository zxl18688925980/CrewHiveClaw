...
read_when:
  - Running CrewClaw behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of CrewClaw
...
- You run CrewClaw behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. CrewClaw checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. CrewClaw extracts the user identity from the configured header
...
- CrewClaw can stay on loopback HTTP behind the proxy.
...
If CrewClaw itself serves HTTPS directly (no TLS-terminating proxy), set:
...
3. Update CrewClaw config with trusted-proxy auth
