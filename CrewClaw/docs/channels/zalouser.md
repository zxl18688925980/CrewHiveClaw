...
read_when:
  - Setting up Zalo Personal for CrewClaw
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside CrewClaw.
...
- On startup, CrewClaw resolves group/user names in allowlists to IDs and logs the mapping.
...
- When a group message is skipped because mention is required, CrewClaw stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in CrewClaw state. Example:
...
- CrewClaw sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, CrewClaw sends delivered + seen acknowledgements (best-effort).
...
- The channel now runs fully in CrewClaw without external CLI binaries.
