...
```
  ...
  <Tab title="HTTP Events API mode">
    <Steps>
      ...
      <Step title="Configure CrewClaw HTTP mode">
...
- The interactive callback values are CrewClaw-generated opaque tokens, not raw agent-authored values.
- If generated interactive blocks would exceed Slack Block Kit limits, CrewClaw falls back to the original text reply instead of sending an invalid blocks payload.
...
`typingReaction` adds a temporary reaction to the inbound Slack message while CrewClaw is processing a reply, then removes it when the run finishes. This is a useful fallback when Slack native assistant typing is unavailable, especially in DMs.
...
```json
{
  "display_information": {
    "name": "CrewClaw",
    "description": "Slack connector for CrewClaw"
  },
  "features": {
    "bot_user": {
      "display_name": "CrewClaw",
      "always_online": false
    },
    "app_home": {
    ...
    "slash_commands": [
      {
        "command": "/openclaw",
        "description": "Send a message to CrewClaw",
        "should_escape": false
      }
    ]
  },
...
CrewClaw supports Slack native text streaming via the Agents and AI Apps API.
...
- If streaming fails mid-reply, CrewClaw falls back to normal delivery for remaining payloads.
