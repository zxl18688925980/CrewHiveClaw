...
read_when:
  - 向新用户介绍 CrewClaw
summary: CrewClaw 是一个多渠道 AI 智能体 Gateway 网关，可在任何操作系统上运行。
title: CrewClaw
x-i18n:
...
<Columns>
  <Card title="入门指南" href="/start/getting-started" icon="rocket">
    安装 CrewClaw 并在几分钟内启动 Gateway 网关。
  </Card>
...
CrewClaw 通过单个 Gateway 网关进程将聊天应用连接到 Pi 等编程智能体。它为 CrewClaw 助手提供支持，并支持本地或远程部署。
...
- 如果你**不做任何修改**，CrewClaw 将使用内置的 Pi 二进制文件以 RPC 模式运行，并按发送者创建独立会话。
