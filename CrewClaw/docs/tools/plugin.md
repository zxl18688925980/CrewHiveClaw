---
summary: "CrewClaw plugins/extensions: discovery, config, and safety"
read_when:
...
A plugin is just a **small code module** that extends CrewClaw with extra
features (commands, tools, and Gateway RPC).

Most of the time, you’ll use plugins when you want a feature that’s not built
into core CrewClaw yet (or you want to keep optional features out of your main
install).
...
those to a prerelease, CrewClaw stops and asks you to opt in explicitly with a
...
CrewClaw's plugin system has four layers:

1. **Manifest + discovery**
   CrewClaw finds candidate plugins from configured paths, workspace roots,
...
4. **Surface consumption**
   The rest of CrewClaw reads the registry to expose tools, channels, provider
...
That split lets CrewClaw validate config, explain missing/disabled plugins, and
...
CrewClaw plugins are **TypeScript modules** loaded at runtime via jiti. **Config
...
At startup, CrewClaw does roughly this:
...
The manifest is the control-plane source of truth. CrewClaw uses it to:
...
CrewClaw keeps short in-process caches for:
...
4. Bundled extensions (shipped with CrewClaw, mostly disabled by default)
...
- If `plugins.allow` is empty and non-bundled plugins are discoverable, CrewClaw logs a startup warning with plugin ids and sources.
- Candidate paths are safety-checked before discovery admission. CrewClaw blocks candidates when:
...
CrewClaw can also merge **external channel catalogs** (for example, an MPM
...
If a plugin exports `id`, CrewClaw uses it but warns when it doesn’t match the
configured id.
...
CrewClaw preserves config for disabled plugins so toggling them back on is not
destructive.
...
CrewClaw augments `uiHints` at runtime based on discovered plugins:
...
If stored integrity metadata changes between updates, CrewClaw warns and asks for confirmation (use global `--yes` to bypass prompts).
...
- When disabled, CrewClaw blocks `before_prompt_build` and ignores prompt-mutating fields returned from legacy `before_agent_start` while preserving legacy `modelOverride` and `providerOverride`.
...
setup inside CrewClaw, surface provider setup in onboarding/model-pickers, and
...
explicit method or let CrewClaw synthesize per-method choices.

CrewClaw validates provider wizard metadata when the plugin registers:
...
- `config`: The current CrewClaw config
