...
// Set HOME/state isolation before importing any runtime CrewClaw modules.
...
const createStubPlugin = (params: {
  ...
  config: {
    listAccountIds: (cfg: CrewClawConfig) => {
    ...
    resolveAccount: (cfg: CrewClawConfig, accountId?: string | null) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
      const entry = channels?.[params.id];
      if (!entry || typeof entry !== "object") {
        return {};
    ...
    isConfigured: async (_account, cfg: CrewClawConfig) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
