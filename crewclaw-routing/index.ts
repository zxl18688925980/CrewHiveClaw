...
function loadInstanceConfig<T>(filename: string): T {
  const p = join(PROJECT_ROOT, "crewclaw", "crewclaw-routing", "config", filename);
...
function loadAgentDeps(): DepSpec[] {
  const cfgPath = join(PROJECT_ROOT, "crewclaw/crewclaw-routing/config/agent-deps.json");
...
function loadDpoPatterns(): DpoPatternsJson {
  const cfgPath = join(PROJECT_ROOT, "crewclaw/crewclaw-routing/config/dpo-patterns.json");
  try {
  ...
  register(api: OpenClawPluginApi) {
    ...
    api.on("before_prompt_build", async (event, ctx) => {
      ...
      if (agentId === FRONTEND_AGENT_ID && !isVisitorSession) {
        try {
          ...
          const audienceCfgPath2 = join(PROJECT_ROOT, "crewclaw/crewclaw-routing/config", "channel-audience.json");
      ...
      if (agentId === FRONTEND_AGENT_ID && !isVisitorSession) {
        try {
          const audienceCfgPath = join(PROJECT_ROOT, "crewclaw/crewclaw-routing/config", "channel-audience.json");
      ...
      if (agentId === FRONTEND_AGENT_ID) {
        try {
          const intentCfgPath = join(PROJECT_ROOT, "crewclaw/crewclaw-routing/config", "intent-patterns.json");
    ...
    api.registerTool((toolCtx) => ({
      ...
      description: [
        ...
        "     格式：[{ \"file\": \"crewclaw/crewclaw-routing/index.ts\", \"symbol\": \"callGatewayAgent\" }]",
                    ...
                    return {
                      content: [{
                        ...
                        text: [
                          ...
                          '```json',
                          '"code_evidence": [',
                          '  { "file": "crewclaw/crewclaw-routing/index.ts", "symbol": "callGatewayAgent" }',
                          ']',
                          '```',
    ...
    // ── notify_engineer：Lucas 专属，向系统工程师通道发通知 ─────────────────
    //
    // 走企业应用 HTTP API，消息显示「系统工程师」名称，与家庭 bot 通道隔离。
    // 适用场景：
    //   - 需要系统工程师介入（流水线持续失败、架构边界被触碰等）
    //   - 流水线关键状态通报（Andy/Lisa 结果摘要等）
    //   - 技术性告警（不需要家人感知，只需工程师知道）
    //
