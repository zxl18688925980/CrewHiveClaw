...
import { clearPluginDiscoveryCache, discoverCrewClawPlugins } from "./discovery.js";
import {
...
async function discoverWithStateDir(
  stateDir: string,
  params: Parameters<typeof discoverCrewClawPlugins>[0],
) {
  return discoverCrewClawPlugins({ ...params, env: buildDiscoveryEnv(stateDir) });
}
...
describe("discoverCrewClawPlugins", () => {
  ...
  it("resolves tilde workspace dirs against the provided env", () => {
    ...
    const result = discoverCrewClawPlugins({
  ...
  it("reuses discovery results from cache until cleared", async () => {
    ...
    const first = discoverCrewClawPlugins({
      env: {
        ...buildDiscoveryEnv(stateDir),
    ...
    const second = discoverCrewClawPlugins({
      env: {
        ...buildDiscoveryEnv(stateDir),
        OPENCLAW_PLUGIN_DISCOVERY_CACHE_MS: "5000",
      },
    ...
    const third = discoverCrewClawPlugins({
      env: {
        ...buildDiscoveryEnv(stateDir),
        OPENCLAW_PLUGIN_DISCOVERY_CACHE_MS: "5000",
      },
