...
const mocks = vi.hoisted(() => ({
  writeConfigFile: vi.fn(async (_cfg: CrewClawConfig) => {}),
}));
...
describe("ensureGatewayStartupAuth", () => {
  async function expectEphemeralGeneratedTokenWhenOverridden(cfg: CrewClawConfig) {
  ...
  async function expectNoTokenGeneration(cfg: CrewClawConfig, mode: string) {
    const result = await ensureGatewayStartupAuth({
      cfg,
      env: {} as NodeJS.ProcessEnv,
      persist: true,
  ...
  function createMissingGatewayTokenSecretRefConfig(): CrewClawConfig {
    return {
      gateway: {
        auth: {
          mode: "token",
