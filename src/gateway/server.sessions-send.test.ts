...
const { createCrewClawTools } = await import("../agents/openclaw-tools.js");
...
type SessionSendTool = ReturnType<typeof createCrewClawTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createCrewClawTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      const tool = createCrewClawTools({
        config: {
          tools: {
            sessions: {
              visibility: "all",
