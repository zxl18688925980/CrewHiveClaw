...
const loadCrewClawPlugins = vi.hoisted(() => vi.fn());
...
function createSubagentRuntime(serverPlugins: ServerPluginsModule): PluginRuntime["subagent"] {
  ...
  loadCrewClawPlugins.mockReturnValue(createRegistry([]));
  ...
  const call = loadCrewClawPlugins.mock.calls.at(-1)?.[0] as
...
describe("loadGatewayPlugins", () => {
  test("logs plugin errors with details", async () => {
    ...
    loadCrewClawPlugins.mockReturnValue(createRegistry(diagnostics));
  ...
  test("provides subagent runtime with sessions.get method aliases", async () => {
    ...
    const call = loadCrewClawPlugins.mock.calls.at(-1)?.[0];
