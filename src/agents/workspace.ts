...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = (env.CREWCLAW_PROFILE ?? env.OPENCLAW_PROFILE)?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".crewclaw", `workspace-${profile}`);
  }
  return path.join(home, ".crewclaw", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".crewclaw";
