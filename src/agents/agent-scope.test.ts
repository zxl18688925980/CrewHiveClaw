...
describe("resolveAgentConfig", () => {
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: CrewClawConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: CrewClawConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: CrewClawConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgDisable: CrewClawConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: CrewClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: CrewClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
    ...
    const cfgAgentOverrideOnly: CrewClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("uses OPENCLAW_HOME for default agent workspace", () => {
    ...
    const workspace = resolveAgentWorkspaceDir({} as CrewClawConfig, "main");
  ...
  it("uses OPENCLAW_HOME for default agentDir", () => {
    ...
    const agentDir = resolveAgentDir({} as CrewClawConfig, "main");
