...
function asConfig(value: unknown): CrewClawConfig {
  return value as CrewClawConfig;
}

function findTool(name: string, config: CrewClawConfig) {
  const allTools = createCrewClawTools({ config, sandboxed: true });
...
async function prepareAndActivate(params: { config: CrewClawConfig; env?: NodeJS.ProcessEnv }) {
