...
import { resolvePreferredCrewClawTmpDir } from "../infra/tmp-openclaw-dir.js";
...
async function withOutsideHardlinkInCrewClawTmp<T>(
  params: {
...
describe("resolveSandboxedMediaSource", () => {
  ...
  it("rejects symlinked CrewClaw tmp paths escaping tmp root", async () => {
  ...
  it("rejects hardlinked CrewClaw tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInCrewClawTmp(
      {
        openClawTmpDir,
  ...
  it("rejects symlinked CrewClaw tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInCrewClawTmp(
