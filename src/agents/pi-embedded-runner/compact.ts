...
import { resolveCrewClawDocsPath } from "../docs-path.js";
...
function resolvePostCompactionIndexSyncMode(config?: CrewClawConfig): "off" | "async" | "await" {
...
export async function compactEmbeddedPiSessionDirect(
  ...
  const agentDir = params.agentDir ?? resolveCrewClawAgentDir();
  await ensureCrewClawModelsJson(params.config, agentDir);
  ...
  try {
    ...
    const toolsRaw = createCrewClawCodingTools({
      exec: {
    ...
    const promptMode =
      isSubagentSessionKey(params.sessionKey) || isCronSessionKey(params.sessionKey)
        ? "minimal"
        : "full";
    const docsPath = await resolveCrewClawDocsPath({
      workspaceDir: effectiveWorkspace,
