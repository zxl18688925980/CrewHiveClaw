...
import { planCrewClawModelsJson } from "./models-config.plan.js";
...
function resolveModelsConfigInput(config?: CrewClawConfig): {
  config: CrewClawConfig;
  sourceConfigForSecrets: CrewClawConfig;
} {
...
export async function ensureCrewClawModelsJson(
  config?: CrewClawConfig,
  agentDirOverride?: string,
): Promise<{ agentDir: string; wrote: boolean }> {
  const resolved = resolveModelsConfigInput(config);
  const cfg = resolved.config;
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : resolveCrewClawAgentDir();
  ...
  return await withModelsJsonWriteLock(targetPath, async () => {
    ...
    const plan = await planCrewClawModelsJson({
      cfg,
