...
export function isCliProvider(provider: string, cfg?: CrewClawConfig): boolean {
  const normalized = normalizeProviderId(provider);
...
function resolveAllowedFallbacks(params: { cfg: CrewClawConfig; agentId?: string }): string[] {
  if (params.agentId) {
