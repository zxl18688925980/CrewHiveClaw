...
function withDefaultModel(primary: string): CrewClawConfig {
  return {
    agents: { defaults: { model: { primary } } },
  } as CrewClawConfig;
}

function withPdfModel(primary: string): CrewClawConfig {
  return {
...
async function stubPdfToolInfra(
  ...
  vi.spyOn(modelsConfig, "ensureCrewClawModelsJson").mockResolvedValue({
    agentDir,
    wrote: false,
  });
