...
async function requireRiskAcknowledgement(params: {
  ...
  await params.prompter.note(
    [
      ...
      "",
      "CrewClaw is a hobby project and still in beta. Expect sharp edges.",
      "By default, CrewClaw is a personal agent: one trusted operator boundary.",
      "This bot can read files and run actions if tools are enabled.",
      "A bad prompt can trick it into doing unsafe things.",
      "",
      "CrewClaw is not a hostile multi-tenant boundary by default.",
      "If multiple users can message one tool-enabled agent, they share that delegated tool authority.",
      "",
      "If you’re not comfortable with security hardening and access control, don’t run CrewClaw.",
...
export async function runOnboardingWizard(
  ...
  await prompter.intro("CrewClaw onboarding");
  await requireRiskAcknowledgement({ opts, prompter });

  const snapshot = await readConfigFileSnapshot();
  let baseConfig: CrewClawConfig = snapshot.valid ? (snapshot.exists ? snapshot.config : {}) : {};
  ...
  let nextConfig: CrewClawConfig = applyOnboardingLocalWorkspaceConfig(baseConfig, workspaceDir);
