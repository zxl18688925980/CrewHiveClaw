...
function resolveProfileStateDir(
  ...
  return path.join(resolveRequiredHomeDir(env as NodeJS.ProcessEnv, homedir), `.crewclaw${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.CREWCLAW_PROFILE = profile;

  const stateDir =
    env.CREWCLAW_STATE_DIR?.trim() ||
    env.OPENCLAW_STATE_DIR?.trim() ||
    resolveProfileStateDir(profile, env, homedir);
  if (!env.CREWCLAW_STATE_DIR?.trim()) {
    env.CREWCLAW_STATE_DIR = stateDir;
  }

  if (!env.CREWCLAW_CONFIG_PATH?.trim()) {
    env.CREWCLAW_CONFIG_PATH = path.join(stateDir, "crewclaw.json");
  }

  if (profile === "dev" && !env.CREWCLAW_GATEWAY_PORT?.trim()) {
    env.CREWCLAW_GATEWAY_PORT = "19001";
  }
}
