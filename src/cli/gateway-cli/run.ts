...
async function runGatewayCommand(opts: GatewayRunOpts) {
  const isDevProfile =
    (process.env.CREWCLAW_PROFILE ?? process.env.OPENCLAW_PROFILE)?.trim().toLowerCase() === "dev";
  ...
  if (process.env.CREWCLAW_SERVICE_MARKER ?? process.env.OPENCLAW_SERVICE_MARKER?.trim()) {
