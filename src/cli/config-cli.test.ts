...
import type { ConfigFileSnapshot, CrewClawConfig } from "../config/types.js";
...
const mockWriteConfigFile = vi.fn<
  (cfg: CrewClawConfig, options?: { unsetPaths?: string[][] }) => Promise<void>
>(async () => {});

vi.mock("../config/config.js", () => ({
  readConfigFileSnapshot: () => mockReadConfigFileSnapshot(),
  writeConfigFile: (cfg: CrewClawConfig, options?: { unsetPaths?: string[][] }) =>
...
function setSnapshot(resolved: CrewClawConfig, config: CrewClawConfig) {
...
function withRuntimeDefaults(resolved: CrewClawConfig): CrewClawConfig {
  return {
    ...resolved,
    agents: {
      ...resolved.agents,
...
describe("config cli", () => {
  ...
  describe("config set - issue #6070", () => {
    it("preserves existing config keys when setting a new value", async () => {
      ...
      const runtimeMerged: CrewClawConfig = {
  ...
  describe("config set parsing flags", () => {
    it("falls back to raw string when parsing fails and strict mode is off", async () => {
      const resolved: CrewClawConfig = { gateway: { port: 18789 } };
      setSnapshot(resolved, resolved);
