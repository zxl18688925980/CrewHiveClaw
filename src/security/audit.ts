...
import type { ConfigFileSnapshot, CrewClawConfig } from "../config/config.js";
...
function isFeishuDocToolEnabled(cfg: CrewClawConfig): boolean {
  ...
  if (stateDirPerms.ok) {
    ...
    if (stateDirPerms.worldWritable) {
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; other users can write into your CrewClaw state.`,
      ...
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; group users can write into your CrewClaw state.`,
        remediation: formatPermissionRemediation({
          targetPath: params.stateDir,
          perms: stateDirPerms,
          isDir: true,
...
function collectLoggingFindings(cfg: CrewClawConfig): SecurityAuditFinding[] {
...
function collectElevatedFindings(cfg: CrewClawConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectExecRuntimeFindings(cfg: CrewClawConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
