...
function resolveAgentName(cfg: CrewClawConfig, agentId: string) {
...
function resolveAgentModel(cfg: CrewClawConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: CrewClawConfig): AgentSummary[] {
