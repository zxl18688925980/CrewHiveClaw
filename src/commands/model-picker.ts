...
type PromptDefaultModelResult = { model?: string; config?: CrewClawConfig };
...
function resolveConfiguredModelRaw(cfg: CrewClawConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: CrewClawConfig): string[] {
...
export function applyPrimaryModel(cfg: CrewClawConfig, model: string): CrewClawConfig {
...
export function applyModelAllowlist(cfg: CrewClawConfig, models: string[]): CrewClawConfig {
  const defaults = cfg.agents?.defaults;
