...
export function normalizeCompatibilityConfigValues(cfg: CrewClawConfig): {
  config: CrewClawConfig;
  changes: string[];
} {
  const changes: string[] = [];
  let next: CrewClawConfig = cfg;
  ...
  const seedMissingDefaultAccountsFromSingleAccountBase = () => {
    ...
    next = {
      ...next,
      channels: nextChannels as CrewClawConfig["channels"],
    };
  };
  ...
  const normalizeBrowserSsrFPolicyAlias = () => {
    ...
    next = {
      ...next,
      browser: migratedBrowser as CrewClawConfig["browser"],
    };
    changes.push(
