...
describe("detectMacCloudSyncedStateDir", () => {
  ...
  it("detects state dir under Library/CloudStorage", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "Dropbox", "CrewClaw", ".openclaw");
  ...
  it("anchors cloud detection to OS homedir when OPENCLAW_HOME is overridden", () => {
    ...
    const originalCrewClawHome = process.env.OPENCLAW_HOME;
    ...
    try {
      ...
      if (originalCrewClawHome === undefined) {
        delete process.env.OPENCLAW_HOME;
      } else {
        process.env.OPENCLAW_HOME = originalCrewClawHome;
      }
    }
  });
