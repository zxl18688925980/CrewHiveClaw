...
import { CrewClawSchema } from "../config/zod-schema.js";
...
export function stripUnknownConfigKeys(config: CrewClawConfig): {
  config: CrewClawConfig;
  removed: string[];
} {
  const parsed = CrewClawSchema.safeParse(config);
  if (parsed.success) {
...
export function noteOpencodeProviderOverrides(cfg: CrewClawConfig): void {
