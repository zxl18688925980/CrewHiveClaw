...
const triggerCrewClawRestartMock = vi.hoisted(() => vi.fn());
...
vi.mock("./restart.js", () => ({
  triggerCrewClawRestart: (...args: unknown[]) => triggerCrewClawRestartMock(...args),
}));
...
afterEach(() => {
  ...
  spawnMock.mockClear();
  triggerCrewClawRestartMock.mockClear();
...
function expectLaunchdSupervisedWithoutKickstart(params?: {
  ...
  expect(triggerCrewClawRestartMock).not.toHaveBeenCalled();
...
describe("restartGatewayProcessWithFreshPid", () => {
  ...
  it("launchd supervisor never returns failed regardless of triggerCrewClawRestart outcome", () => {
    clearSupervisorHints();
    setPlatform("darwin");
    process.env.OPENCLAW_LAUNCHD_LABEL = "ai.openclaw.gateway";
    // Even if triggerCrewClawRestart *would* fail, launchd path must not call it.
    triggerCrewClawRestartMock.mockReturnValue({
      ok: false,
      method: "launchctl",
  ...
  it("returns supervised when CrewClaw gateway task markers are set on Windows", () => {
    clearSupervisorHints();
    setPlatform("win32");
    process.env.OPENCLAW_SERVICE_MARKER = "openclaw";
    process.env.OPENCLAW_SERVICE_KIND = "gateway";
    triggerCrewClawRestartMock.mockReturnValue({ ok: true, method: "schtasks" });
    const result = restartGatewayProcessWithFreshPid();
    expect(result.mode).toBe("supervised");
    expect(triggerCrewClawRestartMock).toHaveBeenCalledOnce();
    expect(spawnMock).not.toHaveBeenCalled();
  });
