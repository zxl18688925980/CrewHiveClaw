...
describe("runMessageAction media behavior", () => {
  ...
  describe("sandboxed media validation", () => {
    ...
    it("allows media paths under preferred CrewClaw tmp root", async () => {
      const tmpRoot = resolvePreferredCrewClawTmpDir();
      ...
      try {
        ...
        const hostTmpOutsideCrewClaw = path.join(os.tmpdir(), "outside-openclaw", "test-media.png");
        await expect(
          runMessageAction({
            cfg: slackConfig,
            action: "send",
