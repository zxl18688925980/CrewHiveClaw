...
type ResolvePreferredCrewClawTmpDirOptions = {
...
export function resolvePreferredCrewClawTmpDir(
  options: ResolvePreferredCrewClawTmpDirOptions = {},
): string {
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback CrewClaw temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback CrewClaw temp dir: ${fallbackPath}`);
    }
