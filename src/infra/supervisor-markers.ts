const SUPERVISOR_HINTS = {
  launchd: [
    ...
    "LAUNCH_JOB_NAME",
    "XPC_SERVICE_NAME",
    "CREWCLAW_LAUNCHD_LABEL",
    "OPENCLAW_LAUNCHD_LABEL",
  ],
  systemd: [
    "CREWCLAW_SYSTEMD_UNIT",
    "OPENCLAW_SYSTEMD_UNIT",
    "INVOCATION_ID",
    "SYSTEMD_EXEC_PID",
    "JOURNAL_STREAM",
  ],
  schtasks: ["CREWCLAW_WINDOWS_TASK_NAME", "OPENCLAW_WINDOWS_TASK_NAME"],
} as const;
...
export const SUPERVISOR_HINT_ENV_VARS = [
  ...
  "CREWCLAW_SERVICE_MARKER",
  "CREWCLAW_SERVICE_KIND",
...
export function detectRespawnSupervisor(
  ...
  if (platform === "win32") {
    ...
    const marker = env.CREWCLAW_SERVICE_MARKER?.trim() || env.OPENCLAW_SERVICE_MARKER?.trim();
    const serviceKind = env.CREWCLAW_SERVICE_KIND?.trim() || env.OPENCLAW_SERVICE_KIND?.trim();
