...
const resolvePreferredCrewClawTmpDirMock = vi.hoisted(() => vi.fn(() => os.tmpdir()));

vi.mock("node:child_process", () => ({
  spawn: (...args: unknown[]) => spawnMock(...args),
}));
vi.mock("./tmp-openclaw-dir.js", () => ({
  resolvePreferredCrewClawTmpDir: () => resolvePreferredCrewClawTmpDirMock(),
}));
...
afterEach(() => {
  envSnapshot.restore();
  spawnMock.mockReset();
  resolvePreferredCrewClawTmpDirMock.mockReset();
  resolvePreferredCrewClawTmpDirMock.mockReturnValue(os.tmpdir());
...
describe("relaunchGatewayScheduledTask", () => {
  it("writes a detached schtasks relaunch helper", () => {
    ...
    expect(result).toMatchObject({
      ok: true,
      method: "schtasks",
      tried: expect.arrayContaining(['schtasks /Run /TN "CrewClaw Gateway (work)"']),
    });
    ...
    expect(script).toContain('schtasks /Run /TN "CrewClaw Gateway (work)" >nul 2>&1');
  ...
  it("prefers OPENCLAW_WINDOWS_TASK_NAME overrides", () => {
    ...
    relaunchGatewayScheduledTask({
      ...
      OPENCLAW_WINDOWS_TASK_NAME: "CrewClaw Gateway (custom)",
    });

    const scriptPath = [...createdScriptPaths][0];
    const script = fs.readFileSync(scriptPath, "utf8");
    expect(script).toContain('schtasks /Run /TN "CrewClaw Gateway (custom)" >nul 2>&1');
  });
  ...
  it("quotes the cmd /c script path when temp paths contain metacharacters", () => {
    ...
    resolvePreferredCrewClawTmpDirMock.mockReturnValue(metacharTmpDir);
