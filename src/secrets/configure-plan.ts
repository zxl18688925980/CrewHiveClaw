...
export function buildConfigureCandidates(config: CrewClawConfig): ConfigureCandidate[] {
...
export function buildConfigureCandidatesForScope(params: {
  ...
  authoredCrewClawConfig?: CrewClawConfig;
  authProfiles?: {
    agentId: string;
    store: AuthProfileStore;
  };
}): ConfigureCandidate[] {
  const authoredConfig = params.authoredCrewClawConfig ?? params.config;
