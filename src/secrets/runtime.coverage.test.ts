...
function buildConfigForCrewClawTarget(entry: SecretRegistryEntry, envId: string): CrewClawConfig {
  const config = {} as CrewClawConfig;
  const refTargetPath =
  ...
  it("handles every openclaw.json registry target when configured as active", async () => {
    ...
    for (const [index, entry] of entries.entries()) {
      ...
      const snapshot = await prepareSecretsRuntimeSnapshot({
        config: buildConfigForCrewClawTarget(entry, envId),
