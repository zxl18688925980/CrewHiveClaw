...
function resolveBrowserFetchOperatorHint(url: string): string {
  ...
  return isLocal
    ? `Restart the CrewClaw gateway (CrewClaw.app menubar, or \`${formatCliCommand("openclaw gateway")}\`).`
  ...
  if (looksLikeTimeout) {
    return new Error(
      appendBrowserToolModelHint(
        `Can't reach the CrewClaw browser control service (timed out after ${timeoutMs}ms). ${operatorHint}`,
      ),
    );
  }
  return new Error(
    appendBrowserToolModelHint(
      `Can't reach the CrewClaw browser control service. ${operatorHint} (${msg})`,
    ),
  );
}
