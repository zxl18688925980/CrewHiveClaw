...
function isWhatsAppConfigured(cfg: CrewClawConfig): boolean {
...
function isGenericChannelConfigured(cfg: CrewClawConfig, channelId: string): boolean {
  const entry = resolveChannelConfig(cfg, channelId);
  return recordHasKeys(entry);
}
...
function collectModelRefs(cfg: CrewClawConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: CrewClawConfig, providerId: string): boolean {
...
function collectCandidateChannelIds(cfg: CrewClawConfig, env: NodeJS.ProcessEnv): string[] {
...
function isPluginExplicitlyDisabled(cfg: CrewClawConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: CrewClawConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: CrewClawConfig, pluginId: string): CrewClawConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
