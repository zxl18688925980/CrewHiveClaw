...
import type { CrewClawConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareCrewClawVersions } from "./version.js";
...
function unsetPathForWrite(
  root: CrewClawConfig,
  pathSegments: string[],
): { changed: boolean; next: CrewClawConfig } {
...
function coerceConfig(value: unknown): CrewClawConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as CrewClawConfig;
}
...
function stampConfigVersion(cfg: CrewClawConfig): CrewClawConfig {
...
function warnIfConfigFromFuture(cfg: CrewClawConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareCrewClawVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer CrewClaw (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as CrewClawConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): CrewClawConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as CrewClawConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: CrewClawConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as CrewClawConfig)
        : cfgToWrite;
...
let runtimeConfigSnapshot: CrewClawConfig | null = null;
let runtimeConfigSourceSnapshot: CrewClawConfig | null = null;
...
export function getRuntimeConfigSnapshot(): CrewClawConfig | null {
  return runtimeConfigSnapshot;
}

export function getRuntimeConfigSourceSnapshot(): CrewClawConfig | null {
  return runtimeConfigSourceSnapshot;
}

function isCompatibleTopLevelRuntimeProjectionShape(params: {
  runtimeSnapshot: CrewClawConfig;
  candidate: CrewClawConfig;
}): boolean {
...
export function projectConfigOntoRuntimeSourceSnapshot(config: CrewClawConfig): CrewClawConfig {
...
export function loadConfig(): CrewClawConfig {
...
export async function readBestEffortConfig(): Promise<CrewClawConfig> {
