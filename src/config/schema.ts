...
export type ConfigSchema = ReturnType<typeof CrewClawSchema.toJSONSchema>;
...
function buildBaseConfigSchema(): ConfigSchemaResponse {
  ...
  const schema = CrewClawSchema.toJSONSchema({
    target: "draft-07",
    unrepresentable: "any",
  });
  schema.title = "CrewClawConfig";
  const hints = applyDerivedTags(mapSensitivePaths(CrewClawSchema, "", buildBaseHints()));
  const next = {
