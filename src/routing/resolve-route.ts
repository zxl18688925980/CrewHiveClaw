...
function listAgents(cfg: CrewClawConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: CrewClawConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<CrewClawConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: CrewClawConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: CrewClawConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: CrewClawConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<CrewClawConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  CrewClawConfig,
  {
    bindingsRef: CrewClawConfig["bindings"];
    agentsRef: CrewClawConfig["agents"];
    sessionRef: CrewClawConfig["session"];
...
function resolveRouteCacheForConfig(cfg: CrewClawConfig): Map<string, ResolvedAgentRoute> {
