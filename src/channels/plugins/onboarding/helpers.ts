...
export function setTopLevelChannelDmPolicyWithAllowFrom(params: {
  ...
  channel: string;
  dmPolicy: DmPolicy;
  getAllowFrom?: (cfg: CrewClawConfig) => Array<string | number> | undefined;
}): CrewClawConfig {
  const channelConfig =
    (params.cfg.channels?.[params.channel] as Record<string, unknown> | undefined) ?? {};
...
export async function runSingleChannelSecretStep(params: {
  ...
  applyUseEnv?: (cfg: CrewClawConfig) => CrewClawConfig | Promise<CrewClawConfig>;
  applySet?: (
    cfg: CrewClawConfig,
    value: SecretInput,
    resolvedValue: string,
  ) => CrewClawConfig | Promise<CrewClawConfig>;
}): Promise<{
  cfg: CrewClawConfig;
...
export async function promptSingleChannelSecretInput(params: {
  ...
  const selectedMode = await resolveSecretInputModeForEnvSelection({
    ...
    copy: {
      ...
      plaintextHint: "Stores the credential directly in CrewClaw config",
