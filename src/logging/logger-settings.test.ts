...
let originalCrewClawLogLevel: string | undefined;
...
beforeEach(() => {
  ...
  originalCrewClawLogLevel = process.env.OPENCLAW_LOG_LEVEL;
...
afterEach(() => {
  ...
  if (originalCrewClawLogLevel === undefined) {
    delete process.env.OPENCLAW_LOG_LEVEL;
  } else {
    process.env.OPENCLAW_LOG_LEVEL = originalCrewClawLogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();
