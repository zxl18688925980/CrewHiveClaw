...
const EXTRA_MARKERS = ["crewclaw", "openclaw", "clawdbot", "moltbot"] as const;
...
function hasGatewayServiceMarker(content: string): boolean {
  const lower = content.toLowerCase();
  const markerKeys = ["crewclaw_service_marker", "openclaw_service_marker"];
  const kindKeys = ["crewclaw_service_kind", "openclaw_service_kind"];
...
function isCrewClawGatewayLaunchdService(label: string, contents: string): boolean {
  ...
  return label.startsWith("ai.crewclaw.") || label.startsWith("ai.openclaw.");
}

function isCrewClawGatewaySystemdService(name: string, contents: string): boolean {
  if (hasGatewayServiceMarker(contents)) {
    return true;
  }
  if (!name.startsWith("crewclaw-gateway") && !name.startsWith("openclaw-gateway")) {
    return false;
  }
  return contents.toLowerCase().includes("gateway");
}

function isCrewClawGatewayTaskName(name: string): boolean {
  ...
  return (
    ...
    normalized.startsWith("crewclaw gateway") ||
...
async function scanLaunchdDir(params: {
  ...
  for (const { name: labelFromName, fullPath, contents } of candidates) {
    ...
    if (marker === "openclaw" && isCrewClawGatewayLaunchdService(label, contents)) {
      continue;
    }
    results.push({
      platform: "darwin",
...
async function scanSystemdDir(params: {
  ...
  for (const { entry, name, fullPath, contents } of candidates) {
    ...
    if (marker === "openclaw" && isCrewClawGatewaySystemdService(name, contents)) {
      continue;
    }
    results.push({
      platform: "linux",
...
export async function findExtraGatewayServices(
  ...
  if (process.platform === "win32") {
    ...
    for (const task of tasks) {
      ...
      if (isCrewClawGatewayTaskName(name)) {
        continue;
      }
