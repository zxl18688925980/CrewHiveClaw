...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.CREWCLAW_PROFILE ?? env.OPENCLAW_PROFILE;
  const resolvedLaunchdLabel =
    launchdLabel || (platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    CREWCLAW_PROFILE: profile,
    CREWCLAW_GATEWAY_PORT: String(port),
    CREWCLAW_LAUNCHD_LABEL: resolvedLaunchdLabel,
    CREWCLAW_SYSTEMD_UNIT: systemdUnit,
    CREWCLAW_WINDOWS_TASK_NAME: resolveGatewayWindowsTaskName(profile),
    CREWCLAW_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    CREWCLAW_SERVICE_KIND: GATEWAY_SERVICE_KIND,
    CREWCLAW_SERVICE_VERSION: VERSION,
  };
}
...
export function buildNodeServiceEnvironment(params: {
  ...
  const sharedEnv = resolveSharedServiceEnvironmentFields(env, platform);
  const gatewayToken =
    env.CREWCLAW_GATEWAY_TOKEN?.trim() ||
    env.OPENCLAW_GATEWAY_TOKEN?.trim() ||
    env.CLAWDBOT_GATEWAY_TOKEN?.trim() ||
    undefined;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    CREWCLAW_GATEWAY_TOKEN: gatewayToken,
    CREWCLAW_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    CREWCLAW_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    CREWCLAW_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    CREWCLAW_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    CREWCLAW_LOG_PREFIX: "node",
    CREWCLAW_SERVICE_MARKER: NODE_SERVICE_MARKER,
    CREWCLAW_SERVICE_KIND: NODE_SERVICE_KIND,
    CREWCLAW_SERVICE_VERSION: VERSION,
  };
}
...
function buildCommonServiceEnvironment(
  ...
  const serviceEnv: Record<string, string | undefined> = {
    ...
    CREWCLAW_STATE_DIR: sharedEnv.stateDir,
    CREWCLAW_CONFIG_PATH: sharedEnv.configPath,
  };
...
function resolveSharedServiceEnvironmentFields(
  ...
  const stateDir = env.CREWCLAW_STATE_DIR ?? env.OPENCLAW_STATE_DIR;
  const configPath = env.CREWCLAW_CONFIG_PATH ?? env.OPENCLAW_CONFIG_PATH;
