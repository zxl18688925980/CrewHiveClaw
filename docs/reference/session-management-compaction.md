...
This document explains how CrewClaw manages sessions end-to-end:
...
CrewClaw is designed around a single **Gateway process** that owns session state.
...
CrewClaw persists sessions in two layers:
...
CrewClaw resolves these via `src/config/sessions.ts`.
...
In `mode: "warn"`, CrewClaw reports potential evictions but does not mutate the store/files.
...
CrewClaw intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (CrewClaw consumes the events, but Pi decides when to compact).
...
CrewClaw also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, CrewClaw bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, CrewClaw leaves it alone.
...
CrewClaw supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- CrewClaw strips/suppresses this in the delivery layer.

As of `2026.1.10`, CrewClaw also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
CrewClaw uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but CrewClaw’s
