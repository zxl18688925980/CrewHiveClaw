...
This plan defines how CrewClaw should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- CrewClaw core owns ACP control plane concerns
...
CrewClaw should not reimplement ACP transport internals in core.
CrewClaw should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in CrewClaw, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under CrewClaw state dir
...
- ACP event stream is normalized to CrewClaw reply chunks
...
For the first production backend (`extensions/acpx`), CrewClaw and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses CrewClaw `sessionKey`)
...
- CrewClaw consumes ndjson events from `acpx --format json --json-strict`
