---
summary: "Chrome extension: let CrewClaw drive your existing Chrome tab"
read_when:
...
The CrewClaw Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate openclaw-managed Chrome profile.

Attach/detach happens via a **single Chrome toolbar button**.

If you want Chrome’s official DevTools MCP attach flow instead of the CrewClaw
...
CrewClaw then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the CrewClaw release (npm package) as static files. There is no separate “build” step.

After upgrading CrewClaw:

- Re-run `openclaw browser extension install` to refresh the installed files under your CrewClaw state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

CrewClaw ships with a built-in browser profile named `chrome-relay` that targets the extension relay on the default port.
...
- Open the tab you want CrewClaw to control.
...
- `ON`: attached; CrewClaw can drive that tab.
...
The CLI intentionally does **not** print a `node_modules` path. Always run `openclaw browser extension install` first to copy the extension to a stable location under your CrewClaw state directory.
