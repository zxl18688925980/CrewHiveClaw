...
title: "Browser (CrewClaw-managed)"
---

# Browser (openclaw-managed)

CrewClaw can run a **dedicated Chrome/Brave/Edge/Chromium profile** that the agent controls.
...
- `chrome-relay`: extension relay to your **system browser** (requires the
  CrewClaw extension to be attached to a tab).
...
- Default profile is `openclaw` (CrewClaw-managed standalone browser). Use `defaultProfile: "user"` to opt into the signed-in user browser, or `defaultProfile: "chrome-relay"` for the extension relay.
...
CrewClaw uses it automatically. Set `browser.executablePath` to override
auto-detection:
...
- **Remote CDP:** set `browser.profiles.<name>.cdpUrl` (or `browser.cdpUrl`) to
  attach to a remote Chromium-based browser. In this case, CrewClaw will not launch a local browser.
...
CrewClaw preserves the auth when calling `/json/*` endpoints and when connecting
to the CDP WebSocket. Prefer environment variables or secrets managers for
tokens instead of committing them to config files.

## Node browser proxy (zero-config default)

If you run a **node host** on the machine that has your browser, CrewClaw can
...
CDP endpoints over HTTPS. You can point a CrewClaw browser profile at a
...
the standard HTTP-based CDP discovery (`/json/version`). CrewClaw supports both:

- **HTTP(S) endpoints** (e.g. Browserless) — CrewClaw calls `/json/version` to
  discover the WebSocket debugger URL, then connects.
- **WebSocket endpoints** (`ws://` / `wss://`) — CrewClaw connects directly,
...
- If browser control is enabled and no auth is configured, CrewClaw auto-generates `gateway.auth.token` on startup and persists it to config.
...
CrewClaw supports multiple named profiles (routing configs). Profiles can be:
...
CrewClaw can also drive **your existing Chrome tabs** (no separate “openclaw” Chrome instance) via a local CDP relay + a Chrome extension.
...
- You click the **CrewClaw Browser Relay** extension icon on a tab to attach (it does not auto-attach).
...
CrewClaw can also attach to a running Chrome profile through the official
...
3. Keep Chrome running and approve the connection prompt when CrewClaw attaches
...
- CrewClaw does not launch Chrome for this driver; it attaches to an existing
  session only.
- CrewClaw uses the official Chrome DevTools MCP `--autoConnect` flow here, not
...
When launching locally, CrewClaw picks the first available:
...
- Download and trace output paths are constrained to CrewClaw temp roots:
  - traces: `/tmp/openclaw` (fallback: `${os.tmpdir()}/openclaw`)
  - downloads: `/tmp/openclaw/downloads` (fallback: `${os.tmpdir()}/openclaw/downloads`)
- Upload paths are constrained to an CrewClaw temp uploads root:
...
CrewClaw supports two “snapshot” styles:
