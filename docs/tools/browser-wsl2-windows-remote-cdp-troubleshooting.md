...
read_when:
  - Running CrewClaw Gateway in WSL2 while Chrome lives on Windows
...
- CrewClaw Gateway runs inside WSL2
...
Use the built-in `chrome-relay` profile plus the CrewClaw Chrome extension.
...
- CrewClaw points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, CrewClaw is not the problem yet.
...
Fix that before touching CrewClaw config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point CrewClaw at the address that is reachable from WSL2:
...
- test the same URL with `curl` before expecting CrewClaw to succeed
...
3. CrewClaw config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug CrewClaw config or Control UI auth
