...
- For shell scripts and direct interpreter/runtime file invocations, CrewClaw also tries to bind
...
- If CrewClaw cannot identify exactly one concrete local file for an interpreter/runtime command
...
These clients are opt-in. If a channel does not have exec approvals enabled, CrewClaw does not treat
...
topics, CrewClaw preserves the topic for the approval prompt and the post-approval follow-up.
