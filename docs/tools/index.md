---
summary: "Agent tool surface for CrewClaw (browser, canvas, nodes, message, cron) replacing legacy `openclaw-*` skills"
read_when:
...
CrewClaw exposes **first-class agent tools** for browser, canvas, nodes, and cron.
...
- If `tools.allow` only references unknown or unloaded plugin tool names, CrewClaw logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:openclaw`: all built-in CrewClaw tools (excludes provider plugins)
...
CrewClaw tracks recent tool-call history and blocks or warns when it detects repetitive no-progress loops.
...
Control the dedicated CrewClaw-managed browser.
...
- Omit `profile` for the safe default: isolated CrewClaw-managed browser (`openclaw`).
...
- `sessions_spawn` starts a sub-agent run and posts an announce reply back to the requester chat.
  ...
  - If `runTimeoutSeconds` is omitted, CrewClaw uses `agents.defaults.subagents.runTimeoutSeconds` when set; otherwise timeout defaults to `0` (no timeout).
...
- Sandbox clamp: when the current session is sandboxed and `agents.defaults.sandbox.sessionToolsVisibility: "spawned"`, CrewClaw clamps `tools.sessions.visibility` to `tree`.
