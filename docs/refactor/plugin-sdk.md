...
Accessed via `CrewClawPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: CrewClawConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: CrewClawConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: CrewClawConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: CrewClawConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: CrewClawConfig): string;
  };
};
```
...
- Add `api.runtime` to `CrewClawPluginApi` with the surface above.
