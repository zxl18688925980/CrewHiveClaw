...
CrewClaw exposes OAuth expiry health via `openclaw models status`. Use that for
...
- `scripts/claude-auth-status.sh`: Claude Code + CrewClaw auth checker (full/json/simple).
...
- `scripts/termux-sync-widget.sh`: sync Claude Code creds → CrewClaw.
