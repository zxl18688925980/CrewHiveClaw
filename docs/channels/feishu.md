...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects CrewClaw to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.

---

## Bundled plugin

Feishu ships bundled with current CrewClaw releases, so no separate plugin install
is required.
...
If you just installed CrewClaw, run the wizard:
...
2. Configuring app credentials in CrewClaw
