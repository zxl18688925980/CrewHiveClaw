...
If you do nothing, CrewClaw runs a single agent:
...
- If you later add the same binding for the same agent with an explicit account id, CrewClaw upgrades the existing channel-only binding to account-scoped instead of duplicating it.
...
`channels.<channel>.defaultAccount` (optional). When unset, CrewClaw falls back
