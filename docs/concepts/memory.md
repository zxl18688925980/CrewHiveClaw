---
title: "Memory"
summary: "How CrewClaw memory works (workspace files + automatic memory flush)"
read_when:
...
CrewClaw memory is **plain Markdown in the agent workspace**. The files are the
...
CrewClaw exposes two agent-facing tools for these Markdown files:
...
When a session is **close to auto-compaction**, CrewClaw triggers a **silent,
...
CrewClaw can build a small vector index over `MEMORY.md` and `memory/*.md` so
...
- Configure memory search under `agents.defaults.memorySearch` (not top-level
  `memorySearch`).
- Uses remote embeddings by default. If `memorySearch.provider` is not set, CrewClaw auto-selects:
...
Remote embeddings **require** an API key for the embedding provider. CrewClaw
...
BM25 + vectors + reranking. Markdown stays the source of truth; CrewClaw shells
...
- Searches run via `memory.qmd.searchMode` (default `qmd search --json`; also
  ...
  QMD build, CrewClaw retries with `qmd query`. If QMD fails or the binary is
  missing, CrewClaw automatically falls back to the builtin SQLite manager so
  memory tools keep working.
- CrewClaw does not expose QMD embed batch-size tuning today; batch behavior is
  controlled by QMD itself.
- **First search may be slow**: QMD may download local GGUF models (reranker/query
  expansion) on the first `qmd query` run.
  - CrewClaw sets `XDG_CONFIG_HOME`/`XDG_CACHE_HOME` automatically when it runs QMD.
  - If you want to pre-download models manually (and warm the same index CrewClaw
    uses), run a one-off query with the agent’s XDG dirs.

    CrewClaw’s QMD state lives under your **state dir** (defaults to `~/.openclaw`).
    You can point `qmd` at the exact same index by exporting the same XDG vars
    CrewClaw uses:

    ```bash
    # Pick the same state dir CrewClaw uses
...
- When `scope` denies a search, CrewClaw logs a warning with the derived
  `channel`/`chatType` so empty results are easier to debug.
- Snippets sourced outside the workspace show up as
  `qmd/<collection>/<relative-path>` in `memory_search` results; `memory_get`
  understands that prefix and reads from the configured QMD collection root.
- When `memory.qmd.sessions.enabled = true`, CrewClaw exports sanitized session
...
- If `memorySearch.multimodal.enabled = true`, CrewClaw also indexes supported image/audio files under `extraPaths` only. Default memory roots (`MEMORY.md`, `memory.md`, `memory/**/*.md`) stay Markdown-only.
- Symlinks are ignored (files or directories).

### Multimodal memory files (Gemini image + audio)

CrewClaw can index image and audio files from `memorySearch.extraPaths` when using Gemini embedding 2:
...
> CrewClaw will automatically reindex when it detects a model or dimension change.
...
- Reindex triggers: the index stores the embedding **provider/model + endpoint fingerprint + chunking params**. If any of those change, CrewClaw automatically resets and reindexes the entire store.

### Hybrid search (BM25 + vector)

When enabled, CrewClaw combines:

- **Vector similarity** (semantic match, wording can differ)
- **BM25 keyword relevance** (exact tokens like IDs, env vars, code symbols)

If full-text search is unavailable on your platform, CrewClaw falls back to vector-only search.
...
CrewClaw can cache **chunk embeddings** in SQLite so reindexing and frequent updates (especially session transcripts) don't re-embed unchanged text.
...
When the sqlite-vec extension is available, CrewClaw stores embeddings in a
...
- If the sqlite-vec extension is missing or fails to load, CrewClaw logs the
