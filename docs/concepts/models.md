...
CrewClaw selects models in this order:
...
- `agents.defaults.models` is the allowlist/catalog of models CrewClaw can use (plus aliases).
...
Marker persistence is source-authoritative: CrewClaw writes markers from the active source config snapshot (pre-resolution), not from resolved runtime secret values.
This applies whenever CrewClaw regenerates `models.json`, including command-driven paths like `openclaw agent`.
