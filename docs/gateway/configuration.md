...
read_when:
  - Setting up CrewClaw for the first time
...
CrewClaw reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.openclaw/openclaw.json`.

If the file is missing, CrewClaw uses safe defaults. Common reasons to add a config:
...
<Warning>
CrewClaw only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>
...
CrewClaw reads env vars from the parent process plus:
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, CrewClaw runs your login shell and imports only the missing keys:
