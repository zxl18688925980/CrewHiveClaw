...
summary: "Complete reference for every CrewClaw config key, defaults, and channel settings"
read_when:
...
Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — CrewClaw uses safe defaults when omitted.
...
- CrewClaw additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
...
- `commands.callbackUrl` must resolve to the CrewClaw gateway endpoint and be reachable from the Mattermost server.
...
CrewClaw spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
- If you add a non-default account via `openclaw channels add` (or channel onboarding) while still on a single-account top-level channel config, CrewClaw moves account-scoped top-level single-account values into `channels.<channel>.accounts.default` first so the original account keeps working.
...
Optional repository root shown in the system prompt's Runtime line. If unset, CrewClaw auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, CrewClaw assumes `anthropic` (deprecated).
...
noVNC observer access uses VNC auth by default and CrewClaw emits a short-lived token URL (instead of exposing the password in the shared URL).
...
For `type: "acp"` entries, CrewClaw resolves by exact conversation identity (`match.channel` + account + `match.peer.id`) and does not use the route binding tier order above.
...
- **`parentForkMaxTokens`**: max parent-session `totalTokens` allowed when creating a forked thread session (default `100000`).
  - If parent `totalTokens` is above this value, CrewClaw starts a fresh thread session instead of inheriting parent transcript history.
...
- When `openai.baseUrl` points to a non-OpenAI endpoint, CrewClaw treats it as an OpenAI-compatible TTS server and relaxes model/voice validation.
...
CrewClaw uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.openclaw/agents/<agentId>/agent/models.json`.
...
- `models.providers.*.models.*.compat.supportsDeveloperRole`: optional compatibility hint. For `api: "openai-completions"` with a non-empty non-native `baseUrl` (host not `api.openai.com`), CrewClaw forces this to `false` at runtime. Empty/omitted `baseUrl` keeps default OpenAI behavior.
...
- `cli.banner.taglineMode` controls banner tagline style:
  ...
  - `"default"`: fixed neutral tagline (`All your chats, one CrewClaw.`).
