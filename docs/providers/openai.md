---
summary: "Use OpenAI via API keys or Codex subscription in CrewClaw"
read_when:
  - You want to use OpenAI models in CrewClaw
...
OpenAI explicitly supports subscription OAuth usage in external tools/workflows like CrewClaw.
...
OpenAI API usage. CrewClaw forwards both through the `openai/*` Responses path.
CrewClaw intentionally suppresses the stale `openai/gpt-5.3-codex-spark` row,
because direct OpenAI API calls reject it in live traffic.

CrewClaw does **not** expose `openai/gpt-5.3-codex-spark` on the direct OpenAI
API path. `pi-ai` still ships a built-in row for that model, but live OpenAI API
requests currently reject it. Spark is treated as Codex-only in CrewClaw.
...
OpenAI's current Codex docs list `gpt-5.4` as the current Codex model. CrewClaw
maps that to `openai-codex/gpt-5.4` for ChatGPT/Codex OAuth usage.

If your Codex account is entitled to Codex Spark, CrewClaw also supports:

- `openai-codex/gpt-5.3-codex-spark`

CrewClaw treats Codex Spark as Codex-only. It does not expose a direct
`openai/gpt-5.3-codex-spark` API-key path.

CrewClaw also preserves `openai-codex/gpt-5.3-codex-spark` when `pi-ai`
...
CrewClaw uses `pi-ai` for model streaming. For both `openai/*` and
...
For `openai/*` (Responses API), CrewClaw also enables WebSocket warm-up by
...
OpenAI docs describe warm-up as optional. CrewClaw enables it by default for
...
CrewClaw, set `agents.defaults.models["openai/<model>"].params.serviceTier` to
...
CrewClaw exposes a shared fast-mode toggle for both `openai/*` and
`openai-codex/*` sessions:

- Chat/UI: `/fast status|on|off`
- Config: `agents.defaults.models["<provider>/<model>"].params.fastMode`

When fast mode is enabled, CrewClaw applies a low-latency OpenAI profile:
...
`baseUrl` on `api.openai.com`), CrewClaw now auto-enables OpenAI server-side
