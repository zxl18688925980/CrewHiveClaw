---
read_when:
  - 在 Oracle Cloud 上设置 CrewClaw
  - 寻找 CrewClaw 的低成本 VPS 托管
  - 想要在小型服务器上 24/7 运行 CrewClaw
summary: 在 Oracle Cloud 上运行 CrewClaw（Always Free ARM）
title: Oracle Cloud
x-i18n:
...
# 在 Oracle Cloud（OCI）上运行 CrewClaw

## 目标

在 Oracle Cloud 的 **Always Free** ARM 层上运行持久化的 CrewClaw Gateway 网关。

Oracle 的免费层非常适合 CrewClaw（特别是如果你已经有 OCI 账户），但有一些权衡：
