...
summary: 修复 Linux 上 CrewClaw 浏览器控制的 Chrome/Brave/Edge/Chromium CDP 启动问题
title: 浏览器故障排除
x-i18n:
...
CrewClaw 的浏览器控制服务器无法启动 Chrome/Brave/Edge/Chromium，出现以下错误：
...
在 Ubuntu（和许多 Linux 发行版）上，默认的 Chromium 安装是 **snap 包**。Snap 的 AppArmor 限制会干扰 CrewClaw 启动和监控浏览器进程的方式。
...
然后更新你的 CrewClaw 配置（`~/.openclaw/openclaw.json`）：
...
如果你必须使用 snap Chromium，配置 CrewClaw 附加到手动启动的浏览器：
...
你正在使用 `chrome` 配置文件（扩展中继）。它期望 CrewClaw 浏览器扩展附加到一个活动标签页。

修复选项：

1. **使用托管浏览器：** `openclaw browser start --browser-profile openclaw`
   （或设置 `browser.defaultProfile: "openclaw"`）。
2. **使用扩展中继：** 安装扩展，打开一个标签页，然后点击 CrewClaw 扩展图标来附加它。
