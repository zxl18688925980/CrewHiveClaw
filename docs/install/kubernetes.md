---
summary: "Deploy CrewClaw Gateway to a Kubernetes cluster with Kustomize"
read_when:
  - You want to run CrewClaw on a Kubernetes cluster
  - You want to test CrewClaw in a Kubernetes environment
title: "Kubernetes"
---

# CrewClaw on Kubernetes

A minimal starting point for running CrewClaw on Kubernetes — not a production-ready deployment. It covers the core resources and is meant to be adapted to your environment.

## Why not Helm?

CrewClaw is a single container with some config files. The interesting customization is in agent content (markdown files, skills, config overrides), not infrastructure templating. Kustomize handles overlays without the overhead of a Helm chart. If your deployment grows more complex, a Helm chart can be layered on top of these manifests.
