...
export async function handleWhatsAppStart(host: CrewClawApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: CrewClawApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: CrewClawApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: CrewClawApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: CrewClawApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: CrewClawApp): string {
...
function resolveGatewayHttpAuthHeader(host: CrewClawApp): string | null {
...
function buildGatewayHttpHeaders(host: CrewClawApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: CrewClawApp) {
...
export function handleNostrProfileToggleAdvanced(host: CrewClawApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: CrewClawApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: CrewClawApp) {
  const state = host.nostrProfileFormState;
