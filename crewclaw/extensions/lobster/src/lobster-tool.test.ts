...
import type { CrewClawPluginApi, CrewClawPluginToolContext } from "openclaw/plugin-sdk/lobster";
...
function fakeApi(overrides: Partial<CrewClawPluginApi> = {}): CrewClawPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<CrewClawPluginToolContext> = {}): CrewClawPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: CrewClawPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
