...
import type { CrewClawPluginApi } from "openclaw/plugin-sdk/device-pair";
import {
...
function resolveGatewayPort(cfg: CrewClawPluginApi["config"]): number {
  const envPort =
...
function resolveAuthLabel(cfg: CrewClawPluginApi["config"]): ResolveAuthLabelResult {
...
async function resolveGatewayUrl(api: CrewClawPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the CrewClaw iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the CrewClaw iOS app:",
            "",
            "```",
            qrAscii,
            "```",
