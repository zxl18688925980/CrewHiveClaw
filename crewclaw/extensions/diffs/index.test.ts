...
import type { CrewClawPluginApi } from "openclaw/plugin-sdk/diffs";
...
describe("diffs plugin registration", () => {
  ...
  it("applies plugin-config defaults through registered tool and viewer handler", async () => {
    ...
    type RegisteredHttpRouteParams = Parameters<CrewClawPluginApi["registerHttpRoute"]>[0];
    ...
    const api = createTestPluginApi({
      ...
      runtime: {} as never,
      registerTool(tool: Parameters<CrewClawPluginApi["registerTool"]>[0]) {
    ...
    plugin.register?.(api as unknown as CrewClawPluginApi);
