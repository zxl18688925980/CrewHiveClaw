...
class NodeForegroundService : Service() {
  ...
  override fun onCreate() {
    super.onCreate()
    ensureChannel()
    val initial = buildNotification(title = "CrewClaw Node", text = "Starting…")
    ...
    notificationJob =
      scope.launch {
        combine(
          ...
          val title = if (connected) "CrewClaw Node · Connected" else "CrewClaw Node"
          val micSuffix =
            if (micEnabled) {
  ...
  private fun ensureChannel() {
    ...
    val channel =
      NotificationChannel(
        ...
        description = "CrewClaw node connection status"
        setShowBadge(false)
      }
