...
final class NetworkStatusService: @unchecked Sendable {
    func currentStatus(timeoutMs: Int = 1500) async -> CrewClawNetworkStatusPayload {
    ...
    private static func payload(from path: NWPath) -> CrewClawNetworkStatusPayload {
        let status: CrewClawNetworkPathStatus = switch path.status {
    ...
    private static func fallbackPayload() -> CrewClawNetworkStatusPayload {
        CrewClawNetworkStatusPayload(
            status: .unsatisfied,
            isExpensive: false,
            isConstrained: false,
            interfaces: [.other])
