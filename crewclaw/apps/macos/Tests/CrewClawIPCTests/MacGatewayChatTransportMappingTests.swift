...
struct MacGatewayChatTransportMappingTests {
    @Test func `snapshot maps to health`() {
        let snapshot = Snapshot(
            presence: [],
            health: CrewClawProtocol.AnyCodable(["ok": CrewClawProtocol.AnyCodable(false)]),
    ...
    @Test func `health event maps to health`() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: CrewClawProtocol.AnyCodable(["ok": CrewClawProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func `chat event maps to chat`() {
        let payload = CrewClawProtocol.AnyCodable([
            "runId": CrewClawProtocol.AnyCodable("run-1"),
            "sessionKey": CrewClawProtocol.AnyCodable("main"),
            "state": CrewClawProtocol.AnyCodable("final"),
        ])
    ...
    @Test func `unknown event maps to nil`() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: CrewClawProtocol.AnyCodable(["a": CrewClawProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
