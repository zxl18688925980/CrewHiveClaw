...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: CrewClawProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: CrewClawKit.AnyCodable]()) {
                $0[$1.key] = CrewClawKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: CrewClawProtocol.AnyCodable?) -> [String: CrewClawProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: CrewClawProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: CrewClawKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: CrewClawProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
