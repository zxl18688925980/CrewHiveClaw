...
import Foundation
import Testing
@testable import CrewClawChatUI
...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> CrewClawChatHistoryPayload
{
    CrewClawChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> CrewClawChatSessionEntry {
    CrewClawChatSessionEntry(
        key: key,
        kind: nil,
        displayName: nil,
        surface: nil,
...
private func sessionEntry(
    ...
    updatedAt: Double,
    model: String?,
    modelProvider: String? = nil) -> CrewClawChatSessionEntry
{
    CrewClawChatSessionEntry(
        key: key,
        kind: nil,
...
private func modelChoice(id: String, name: String, provider: String = "anthropic") -> CrewClawChatModelChoice {
    CrewClawChatModelChoice(modelID: id, name: name, provider: provider, contextWindow: nil)
}

private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [CrewClawChatHistoryPayload],
    sessionsResponses: [CrewClawChatSessionsListResponse] = [],
    modelResponses: [[CrewClawChatModelChoice]] = [],
    ...
    -> (TestChatTransport, CrewClawChatViewModel)
{
...
private func sendUserMessage(_ vm: CrewClawChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, CrewClawChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [CrewClawChatHistoryPayload]
    private let sessionsResponses: [CrewClawChatSessionsListResponse]
    private let modelResponses: [[CrewClawChatModelChoice]]
    private let resetSessionHook: (@Sendable (String) async throws -> Void)?
    private let setSessionModelHook: (@Sendable (String?) async throws -> Void)?
    private let setSessionThinkingHook: (@Sendable (String) async throws -> Void)?

    private let stream: AsyncStream<CrewClawChatTransportEvent>
    private let continuation: AsyncStream<CrewClawChatTransportEvent>.Continuation
        ...
        setSessionThinkingHook: (@Sendable (String) async throws -> Void)? = nil)
    {
        ...
        var cont: AsyncStream<CrewClawChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> CrewClawChatHistoryPayload {
        ...
        return self.historyResponses.last ?? CrewClawChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
    ...
    func sendMessage(
        ...
        attachments _: [CrewClawChatAttachmentPayload]) async throws -> CrewClawChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        await self.state.sentThinkingLevelsAppend(thinking)
        return CrewClawChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> CrewClawChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? CrewClawChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: CrewClawChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let sessions = CrewClawChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesUseResolvedMainSessionKeyInsteadOfLiteralMain() async throws {
        ...
        let sessions = CrewClawChatSessionsListResponse(
            ...
            path: nil,
            count: 2,
            defaults: CrewClawChatSessionsDefaults(
                model: nil,
                contextTokens: nil,
    ...
    @Test func bootstrapsModelSelectionFromSessionAndDefaults() async throws {
        ...
        let sessions = CrewClawChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: CrewClawChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func selectingDefaultModelPatchesNilAndUpdatesSelection() async throws {
        ...
        await MainActor.run { vm.selectModel(CrewClawChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == CrewClawChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func selectingProviderQualifiedModelDisambiguatesDuplicateModelIDs() async throws {
        ...
        let sessions = CrewClawChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: CrewClawChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func lateModelCompletionDoesNotReplayCurrentSessionSelectionIntoPreviousSession() async throws {
        let now = Date().timeIntervalSince1970 * 1000
        let initialSessions = CrewClawChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
        ...
        let sessionsAfterOtherSelection = CrewClawChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
    ...
    @Test func explicitThinkingLevelWinsOverHistoryAndPersistsChanges() async throws {
        let history = CrewClawChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [],
            thinkingLevel: "off")
...
"""]],
        ...
        let vm = await MainActor.run { CrewClawChatViewModel(sessionKey: "main", transport: transport) }
