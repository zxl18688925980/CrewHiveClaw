...
@MainActor
public struct CrewClawChatView: View {
    public enum Style {
        case standard
        case onboarding
    }

    @State private var viewModel: CrewClawChatViewModel
    ...
    public init(
        viewModel: CrewClawChatViewModel,
    ...
    private var visibleMessages: [CrewClawChatMessage] {
        let base: [CrewClawChatMessage]
    ...
    private func mergeToolResults(in messages: [CrewClawChatMessage]) -> [CrewClawChatMessage] {
        ...
        for message in messages {
            ...
            let merged = CrewClawChatMessage(
                id: last.id,
                role: last.role,
                content: content,
    ...
    private func isToolResultMessage(_ message: CrewClawChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: CrewClawChatMessage) -> Bool {
    ...
    private func primaryText(in message: CrewClawChatMessage) -> String {
    ...
    private func hasInlineAttachments(in message: CrewClawChatMessage) -> Bool {
    ...
    private func toolCalls(in message: CrewClawChatMessage) -> [CrewClawChatMessageContent] {
    ...
    private func inlineToolResults(in message: CrewClawChatMessage) -> [CrewClawChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: CrewClawChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: CrewClawChatMessage) -> String {
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 18, style: .continuous)
                .fill(CrewClawChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 18, style: .continuous)
