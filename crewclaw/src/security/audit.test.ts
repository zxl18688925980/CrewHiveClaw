...
function stubChannelPlugin(params: {
  ...
  label: string;
  resolveAccount: (cfg: CrewClawConfig, accountId: string | null | undefined) => unknown;
  inspectAccount?: (cfg: CrewClawConfig, accountId: string | null | undefined) => unknown;
  listAccountIds?: (cfg: CrewClawConfig) => string[];
  isConfigured?: (account: unknown, cfg: CrewClawConfig) => boolean;
  isEnabled?: (account: unknown, cfg: CrewClawConfig) => boolean;
}): ChannelPlugin {
  return {
    id: params.id,
    meta: {
...
`,
  ...
  it("flags wildcard Control UI origins by exposure level", async () => {
    const loopbackCfg: CrewClawConfig = {
      gateway: {
        bind: "loopback",
        controlUi: { allowedOrigins: ["*"] },
      },
    };
    const exposedCfg: CrewClawConfig = {
      gateway: {
        bind: "lan",
        auth: { mode: "token", token: "very-long-browser-token-0123456789" },
        controlUi: { allowedOrigins: ["*"] },
  ...
  it("scores X-Real-IP fallback risk by gateway exposure", async () => {
    const trustedProxyCfg = (trustedProxies: string[]): CrewClawConfig => ({
      gateway: {
        bind: "loopback",
  ...
  it("warns when multiple DM senders share the main session", async () => {
    const cfg: CrewClawConfig = { session: { dmScope: "main" } };
  ...
  it("keeps channel security findings when SecretRef credentials are configured but unavailable", async () => {
    await withChannelSecurityStateDir(async () => {
      ...
      const resolvedConfig: CrewClawConfig = {
        channels: {
          discord: {
            enabled: true,
            groupPolicy: "allowlist",
  ...
  it("adds probe_failed warnings for deep probe failure modes", async () => {
    const cfg: CrewClawConfig = { gateway: { mode: "local" } };
    const cases: Array<{
      name: string;
  ...
  it("scores unrestricted hooks.allowedAgentIds by gateway exposure", async () => {
    ...
    } satisfies NonNullable<CrewClawConfig["hooks"]>;
    const cases: Array<{
      name: string;
      cfg: CrewClawConfig;
      expectedSeverity: "warn" | "critical";
  ...
  it("flags group/world-readable config include files", async () => {
    ...
    const cfg: CrewClawConfig = { logging: { redactSensitive: "off" } };
    const user = "DESKTOP-TEST\\Tester";
