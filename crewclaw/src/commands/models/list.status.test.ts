...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveCrewClawAgentDir: vi.fn().mockReturnValue("/tmp/openclaw-agent"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveCrewClawAgentDir: mocks.resolveCrewClawAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveCrewClawAgentDir).toHaveBeenCalled();
