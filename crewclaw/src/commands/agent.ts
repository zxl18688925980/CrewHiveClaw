...
function prependInternalEventContext(
  ...
  if (body.includes("CrewClaw runtime context (internal):")) {
    return body;
  }
...
async function prepareAgentCommandExecution(
  ...
  // CrewClaw: daemon agents manage their own workspace — skip template bootstrap.
  const isDaemonAgent = resolveAgentConfig(cfg, sessionAgentId)?.runtime?.type === "daemon";
  const workspace = await ensureAgentWorkspace({
    dir: workspaceDirRaw,
    ensureBootstrapFiles: !isDaemonAgent && !agentCfg?.skipBootstrap,
  });
...
async function agentCommandInternal(
  ...
  try {
    ...
    // CrewClaw: daemon proxy — if this agent has runtime.type="daemon", forward to local HTTP daemon.
    const agentRuntimeCfg = resolveAgentConfig(cfg, sessionAgentId)?.runtime;
    if (agentRuntimeCfg?.type === "daemon") {
      const daemonUrl = agentRuntimeCfg.daemon.url;
      const daemonPath = agentRuntimeCfg.daemon.path ?? "/api/chat";
      ...
      try {
        const fetchSignal =
          opts.abortSignal ??
          (timeoutMs > 0 ? AbortSignal.timeout(timeoutMs) : AbortSignal.timeout(60_000));
        const response = await fetch(`${daemonUrl}${daemonPath}`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ message: body, userId: opts.replyTo ?? "gateway-user" }),
          signal: fetchSignal,
        });
        ...
        const replyText =
          (data.success !== false ? data.response : undefined) ?? data.response ?? data.error ?? "";
        emitAgentEvent({
          runId,
          stream: "lifecycle",
          data: { phase: "end", endedAt: Date.now() },
        });
        const normalizedPayload = normalizeReplyPayload({ text: replyText });
        const payloads = normalizedPayload ? [normalizedPayload] : [];
        const result = {
          payloads,
          meta: {
