...
async function runRepair(cfg: CrewClawConfig) {
...
describe("maybeRepairGatewayServiceConfig", () => {
  beforeEach(() => {
    ...
    mocks.resolveGatewayAuthTokenForService.mockImplementation(async (cfg: CrewClawConfig, env) => {
      const configToken =
...
describe("maybeScanExtraGatewayServices", () => {
  ...
  it("removes legacy Linux user systemd services", async () => {
    ...
    expect(runtime.log).toHaveBeenCalledWith(
      "Legacy gateway services removed. Installing CrewClaw gateway next.",
    );
  });
});
