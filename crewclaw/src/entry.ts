...
import { ensureCrewClawExecMarkerOnProcess } from "./infra/openclaw-exec-env.js";
...
if (
  ...
  function ensureExperimentalWarningSuppressed(): boolean {
    ...
    child.once("error", (error) => {
      console.error(
        "[crewclaw] Failed to respawn CLI:",
  ...
  function tryHandleRootVersionFastPath(argv: string[]): boolean {
    ...
    Promise.all([import("./version.js"), import("./infra/git-commit.js")])
      .then(([{ VERSION }, { resolveCommitHash }]) => {
        ...
        console.log(commit ? `CrewClaw ${VERSION} (${commit})` : `CrewClaw ${VERSION}`);
        process.exit(0);
      })
      .catch((error) => {
        console.error(
          "[crewclaw] Failed to resolve version:",
          error instanceof Error ? (error.stack ?? error.message) : error,
        );
        process.exitCode = 1;
      });
  ...
  function tryHandleRootHelpFastPath(argv: string[]): boolean {
    ...
    import("./cli/program.js")
      ...
      .catch((error) => {
        console.error(
          "[crewclaw] Failed to display help:",
          error instanceof Error ? (error.stack ?? error.message) : error,
        );
        process.exitCode = 1;
      });
  ...
  if (!ensureExperimentalWarningSuppressed()) {
    ...
    if (!parsed.ok) {
      ...
      console.error(`[crewclaw] ${parsed.error}`);
      process.exit(2);
    }
