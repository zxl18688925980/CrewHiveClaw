...
import type { CrewClawConfig, ConfigFileSnapshot } from "../config/types.openclaw.js";
...
vi.mock("../infra/openclaw-root.js", () => ({
  resolveCrewClawPackageRoot: vi.fn(),
}));
...
const { resolveCrewClawPackageRoot } = await import("../infra/openclaw-root.js");
...
describe("update-cli", () => {
  ...
  const baseConfig = {} as CrewClawConfig;
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveCrewClawPackageRoot).mockResolvedValue(root);
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(resolveCrewClawPackageRoot).mockResolvedValue(process.cwd());
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("CrewClaw update status");
  });
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as CrewClawConfig,
        });
      },
