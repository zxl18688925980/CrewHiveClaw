...
import type { CrewClawConfig } from "../config/types.js";
...
export function isCommandEnabled(cfg: CrewClawConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: CrewClawConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as CrewClawConfig),
...
export function getCommandDetection(_cfg?: CrewClawConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: CrewClawConfig) {
