...
function applyModelProviderToolPolicy(
  ...
  // xAI/Grok providers expose a native web_search tool; sending CrewClaw's
...
function resolveExecConfig(params: { cfg?: CrewClawConfig; agentId?: string }) {
...
export function createCrewClawCodingTools(options?: {
  agentId?: string;
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createCrewClawReadTool(freshReadTool, {
        modelContextWindowTokens: options?.modelContextWindowTokens,
        imageSanitization,
      });
