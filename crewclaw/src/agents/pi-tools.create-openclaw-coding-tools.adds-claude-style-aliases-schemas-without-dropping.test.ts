...
import { __testing, createCrewClawCodingTools } from "./pi-tools.js";
import { createCrewClawReadTool, createSandboxedReadTool } from "./pi-tools.read.js";
...
describe("createCrewClawCodingTools", () => {
  ...
  it("exposes raw for gateway config.apply tool calls", () => {
    const gateway = createCrewClawCodingTools({ senderIsOwner: true }).find(
  ...
  it("keeps raw core tool schemas union-free", () => {
    const tools = createCrewClawTools();
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createCrewClawCodingTools({ messageProvider: "discord" });
  ...
  it("removes unsupported JSON Schema keywords for Cloud Code Assist API compatibility", () => {
    const googleTools = createCrewClawCodingTools({
  ...
  it("strips truncation.content details from read results while preserving other fields", async () => {
    ...
    const wrapped = createCrewClawReadTool(
      baseRead as unknown as Parameters<typeof createCrewClawReadTool>[0],
    );
