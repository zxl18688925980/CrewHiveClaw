...
function createOpenAiApiKeySourceConfig(): CrewClawConfig {
  return {
    models: {
      providers: {
        openai: {
...
function createOpenAiApiKeyRuntimeConfig(): CrewClawConfig {
  return {
    models: {
      providers: {
        openai: {
...
function createOpenAiHeaderSourceConfig(): CrewClawConfig {
  return {
    models: {
      providers: {
        openai: {
...
function createOpenAiHeaderRuntimeConfig(): CrewClawConfig {
  return {
    models: {
      providers: {
        openai: {
...
function withGatewayTokenMode(config: CrewClawConfig): CrewClawConfig {
  return {
    ...config,
    gateway: {
      auth: {
...
async function withGeneratedModelsFromRuntimeSource(
  params: {
    ...
    runtimeConfig: CrewClawConfig;
    candidateConfig?: CrewClawConfig;
  },
  ...
  await withTempHome(async () => {
    try {
      ...
      await ensureCrewClawModelsJson(params.candidateConfig ?? loadConfig());
      await runAssertions();
    } finally {
      clearRuntimeConfigSnapshot();
      clearConfigCache();
...
describe("models-config runtime source snapshot", () => {
  ...
  it("uses non-env marker from runtime source snapshot for file refs", async () => {
    await withTempHome(async () => {
      const sourceConfig: CrewClawConfig = {
        models: {
          providers: {
            moonshot: {
      ...
      const runtimeConfig: CrewClawConfig = {
        models: {
          providers: {
            moonshot: {
              baseUrl: "https://api.moonshot.ai/v1",
      ...
      try {
        ...
        await ensureCrewClawModelsJson(loadConfig());
  ...
  it("projects cloned runtime configs onto source snapshot when preserving provider auth", async () => {
    await withTempHome(async () => {
      ...
      const clonedRuntimeConfig: CrewClawConfig = {
        ...runtimeConfig,
        agents: {
          defaults: {
      ...
      try {
        setRuntimeConfigSnapshot(runtimeConfig, sourceConfig);
        await ensureCrewClawModelsJson(clonedRuntimeConfig);
  ...
  it("keeps source markers when runtime projection is skipped for incompatible top-level shape", async () => {
    await withTempHome(async () => {
      ...
      const incompatibleCandidate: CrewClawConfig = {
        ...createOpenAiApiKeyRuntimeConfig(),
      };

      try {
        setRuntimeConfigSnapshot(runtimeConfig, sourceConfig);
        await ensureCrewClawModelsJson(incompatibleCandidate);
        await expectGeneratedProviderApiKey("openai", "OPENAI_API_KEY"); // pragma: allowlist secret
      } finally {
        clearRuntimeConfigSnapshot();
        clearConfigCache();
