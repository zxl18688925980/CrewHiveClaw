...
function resolveSkillsLimits(config?: CrewClawConfig): ResolvedSkillsLimits {
...
function loadSkillEntries(
  ...
  const skillEntries: SkillEntry[] = Array.from(merged.values()).map((skill) => {
    ...
    return {
      skill,
      frontmatter,
      metadata: resolveCrewClawMetadata(frontmatter),
...
function applySkillsPromptLimits(params: { skills: Skill[]; config?: CrewClawConfig }): {
