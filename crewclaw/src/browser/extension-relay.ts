...
import {
  probeAuthenticatedCrewClawRelay,
  ...
  const initPromise = (async (): Promise<ChromeExtensionRelayServer> => {
    ...
    const routeCdpCommand = async (cmd: CdpCommand): Promise<unknown> => {
      switch (cmd.method) {
        case "Browser.getVersion":
          return {
            protocolVersion: "1.3",
            product: "Chrome/CrewClaw-Extension-Relay",
            revision: "0",
            userAgent: "CrewClaw-Extension-Relay",
            jsVersion: "V8",
          };
    ...
    const server = createServer((req, res) => {
      ...
      if (
        ...
        const payload: Record<string, unknown> = {
          Browser: "CrewClaw/extension-relay",
    ...
    try {
      ...
      if (
        ...
        (await probeAuthenticatedCrewClawRelay({
