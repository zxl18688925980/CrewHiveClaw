...
export function resolveCrewClawUserDataDir(profileName = DEFAULT_OPENCLAW_BROWSER_PROFILE_NAME) {
...
export async function launchCrewClawChrome(
  ...
  const userDataDir = resolveCrewClawUserDataDir(profile.name);
  ...
  if (needsDecorate) {
    try {
      decorateCrewClawProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
...
export async function stopCrewClawChrome(
