...
function localCrewClawProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "openclaw",
...
function createLocalCrewClawResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localCrewClawProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveCrewClawUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    ...
    const ops = createLocalCrewClawResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveCrewClawUserDataDir: () => profileDir,
    });
