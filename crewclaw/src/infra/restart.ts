...
export function triggerCrewClawRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      ...
      process.env.CREWCLAW_PROFILE ?? process.env.OPENCLAW_PROFILE,
    );
  ...
  const label =
    ...
    resolveGatewayLaunchAgentLabel(process.env.CREWCLAW_PROFILE ?? process.env.OPENCLAW_PROFILE);
