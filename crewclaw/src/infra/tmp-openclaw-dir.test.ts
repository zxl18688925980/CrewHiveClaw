...
import { POSIX_OPENCLAW_TMP_DIR, resolvePreferredCrewClawTmpDir } from "./tmp-openclaw-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredCrewClawTmpDir>[0]>;
...
function resolveWithReadOnlyTmpFallback(params: {
  ...
  return resolvePreferredCrewClawTmpDir({
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredCrewClawTmpDir({
    accessSync,
    chmodSync,
...
describe("resolvePreferredCrewClawTmpDir", () => {
  ...
  it("throws when fallback path is a symlink", () => {
    ...
    ).toThrow(/Unsafe fallback CrewClaw temp dir/);
  });
  ...
  it("throws when the fallback directory cannot be created", () => {
    expect(() =>
      resolvePreferredCrewClawTmpDir({
        accessSync: readOnlyTmpAccessSync(),
        lstatSync: vi.fn((target: string) => {
    ...
    ).toThrow(/Unable to create fallback CrewClaw temp dir/);
  });
});
