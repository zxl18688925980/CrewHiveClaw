...
vi.mock("./openclaw-root.js", () => ({
  resolveCrewClawPackageRoot: vi.fn(async () => null),
  resolveCrewClawPackageRootSync: vi.fn(() => null),
}));
...
describe("control UI assets helpers (fs-mocked)", () => {
  ...
  it("uses resolveCrewClawPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/openclaw");
    (
      openclawRoot.resolveCrewClawPackageRoot as unknown as ReturnType<typeof vi.fn>
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    ...
    (
      openclawRoot.resolveCrewClawPackageRootSync as unknown as ReturnType<typeof vi.fn>
