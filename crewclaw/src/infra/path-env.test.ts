...
let ensureCrewClawCliOnPath: typeof import("./path-env.js").ensureCrewClawCliOnPath;

describe("ensureCrewClawCliOnPath", () => {
  const envKeys = [
    "PATH",
  ...
  beforeAll(async () => {
    ({ ensureCrewClawCliOnPath } = await import("./path-env.js"));
  });
  ...
  function bootstrapPath(params: {
    ...
    ensureCrewClawCliOnPath(params);
