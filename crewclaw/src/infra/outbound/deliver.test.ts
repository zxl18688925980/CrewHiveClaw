...
import { resolvePreferredCrewClawTmpDir } from "../tmp-openclaw-dir.js";
import {
...
const telegramChunkConfig: CrewClawConfig = {
...
describe("deliverOutboundPayloads", () => {
  ...
  it("includes CrewClaw tmp root in telegram mediaLocalRoots", async () => {
    const sendTelegram = vi.fn().mockResolvedValue({ messageId: "m1", chatId: "c1" });
    ...
    expect(sendTelegram).toHaveBeenCalledWith(
      ...
      "hi",
      expect.objectContaining({
        mediaLocalRoots: expect.arrayContaining([resolvePreferredCrewClawTmpDir()]),
      }),
    );
  });

  it("includes CrewClaw tmp root in signal mediaLocalRoots", async () => {
  ...
  it("includes CrewClaw tmp root in whatsapp mediaLocalRoots", async () => {
  ...
  it("includes CrewClaw tmp root in imessage mediaLocalRoots", async () => {
  ...
  it("uses signal media maxBytes from config", async () => {
    const sendSignal = vi.fn().mockResolvedValue({ messageId: "s1", timestamp: 123 });
    const cfg: CrewClawConfig = { channels: { signal: { mediaMaxMb: 2 } } };
