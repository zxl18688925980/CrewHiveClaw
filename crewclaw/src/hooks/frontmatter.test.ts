...
describe("resolveCrewClawMetadata", () => {
  it("extracts openclaw metadata from parsed frontmatter", () => {
    ...
    const result = resolveCrewClawMetadata(frontmatter);
    ...
    const frontmatter = {
      metadata: JSON.stringify({
        openclaw: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with CrewClaw" },
    ...
    "openclaw":
      {
        ...
        "events": ["command:new", "command:reset"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with CrewClaw" }],
      },
  }
---
...
`;
    ...
    const openclaw = resolveCrewClawMetadata(frontmatter);
