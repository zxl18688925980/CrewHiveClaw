...
import type { CrewClawConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: CrewClawConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: CrewClawConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: CrewClawConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = CrewClawSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as CrewClawConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as CrewClawConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  const gatewayTailscaleBindIssues = validateGatewayTailscaleBind(validated.data as CrewClawConfig);
  if (gatewayTailscaleBindIssues.length > 0) {
    return { ok: false, issues: gatewayTailscaleBindIssues };
  }
  return {
    ok: true,
    config: validated.data as CrewClawConfig,
  };
}
