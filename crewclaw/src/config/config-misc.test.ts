...
import { buildWebSearchProviderConfig, withTempHome, writeCrewClawConfig } from "./test-helpers.js";
import { CrewClawSchema } from "./zod-schema.js";

describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = CrewClawSchema.safeParse({
  ...
  it("rejects non-string $schema", () => {
    const result = CrewClawSchema.safeParse({ $schema: 123 });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = CrewClawSchema.safeParse({
      cron: {
        enabled: true,
...
describe("config strict validation", () => {
  ...
  it("flags legacy config entries without auto-migrating", async () => {
    await withTempHome(async (home) => {
      await writeCrewClawConfig(home, {
        agents: { list: [{ id: "pi" }] },
