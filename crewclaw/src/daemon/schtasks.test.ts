    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\CrewClaw Share\\node.exe" "\\\\fileserver\\CrewClaw Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\CrewClaw Share\\node.exe",
            "\\\\fileserver\\CrewClaw Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
