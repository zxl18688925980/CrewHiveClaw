---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for CrewClaw deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, CrewClaw cron scheduling for periodic checks, or version status checks on a machine running CrewClaw (laptop, workstation, Pi, VPS).
---

# CrewClaw Host Hardening

## Overview

Assess and harden the host running CrewClaw, then align it to a user-defined risk tolerance without breaking access. Use CrewClaw security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim CrewClaw changes the host firewall, SSH, or OS updates; it does not.
...
5. CrewClaw gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if CrewClaw can access sensitive data.
...
### 2) Run CrewClaw security audits (read-only)
...
Offer to apply CrewClaw safe defaults (numbered):

1. `openclaw security audit --fix`

Be explicit that `--fix` only tightens CrewClaw defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check CrewClaw version/update status (read-only)
...
- Credential hygiene notes (location of CrewClaw creds, prefer disk encryption)
...
- CrewClaw security audit (re-run)
...
After CrewClaw install or first hardening pass, run at least one baseline audit and version check:

- `openclaw security audit`
- `openclaw security audit --deep`
- `openclaw update status`

Ongoing monitoring is recommended. Use the CrewClaw cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
Do not invent CLI flags or imply CrewClaw enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by CrewClaw compaction:
