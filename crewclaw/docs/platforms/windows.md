...
read_when:
  - Installing CrewClaw on Windows
...
CrewClaw on Windows is recommended **via WSL2** (Ubuntu recommended). The
...
- if Scheduled Task creation is denied, CrewClaw falls back to a per-user Startup-folder login item and starts the gateway immediately
- if `schtasks` itself wedges or stops responding, CrewClaw now aborts that path quickly and falls back instead of hanging forever
...
### 2) Install the CrewClaw gateway user service
