---
summary: "Use Anthropic Claude via API keys or setup-token in CrewClaw"
read_when:
  - You want to use Anthropic models in CrewClaw
...
In CrewClaw you can authenticate with an API key or a **setup-token**.
...
- Anthropic Claude 4.6 models default to `adaptive` thinking in CrewClaw when no explicit thinking level is set.
...
CrewClaw's shared `/fast` toggle also supports direct Anthropic API-key traffic.
...
- This is **API-key only**. Anthropic setup-token / OAuth auth does not honor CrewClaw fast-mode tier injection.
- CrewClaw only injects Anthropic service tiers for direct `api.anthropic.com` requests. If you route `anthropic/*` through a proxy or gateway, `/fast` leaves `service_tier` untouched.
...
CrewClaw supports Anthropic's prompt caching feature. This is **API-only**; subscription auth does not honor cache settings.
...
When using Anthropic API Key authentication, CrewClaw automatically applies `cacheRetention: "short"` (5-minute cache) for all Anthropic models. You can override this by explicitly setting `cacheRetention` in your config.
...
CrewClaw includes the `extended-cache-ttl-2025-04-11` beta flag for Anthropic API
requests; keep it if you override provider headers (see [/gateway/configuration](/gateway/configuration)).

## 1M context window (Anthropic beta)

Anthropic's 1M context window is beta-gated. In CrewClaw, enable it per model
...
CrewClaw maps this to `anthropic-beta: context-1m-2025-08-07` on Anthropic
requests.
...
OAuth/subscription tokens (`sk-ant-oat-*`). CrewClaw automatically skips the
...
Paste the token into CrewClaw (wizard: **Anthropic token (paste setup-token)**), or run it on the gateway host:
