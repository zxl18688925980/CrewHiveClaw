...
CrewClaw has three Vitest suites (unit/integration, e2e, live) and a small set of Docker runners.
...
- Pool note:
  - CrewClaw uses Vitest `vmForks` on Node 22, 23, and 24 for faster unit shards.
  - On Node 25+, CrewClaw automatically falls back to regular `forks` until the repo is re-validated there.
...
- Gemini API vs Gemini CLI:
  - API: CrewClaw calls Google’s hosted Gemini API over HTTP (API key / profile auth); this is what most users mean by “Gemini”.
  - CLI: CrewClaw shells out to a local `gemini` binary; it has its own auth and can behave differently (streaming/tool support/version skew).
