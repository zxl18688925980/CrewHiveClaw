---
summary: "Updating CrewClaw safely (global install or source), plus rollback strategy"
read_when:
  - Updating CrewClaw
...
CrewClaw is moving fast (pre “1.0”). Treat updates like shipping infra: update → run checks → restart (or use `openclaw update`, which restarts) → verify.
...
- `stable`: when a new version is seen, CrewClaw waits `stableDelayHours` and then applies a deterministic per-install jitter in `stableJitterHours` (spread rollout).
...
- Detect and migrate older gateway services (launchd/systemd; legacy schtasks) to current CrewClaw services.
