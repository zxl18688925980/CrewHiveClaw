...
- DM messages can carry `message_thread_id`; CrewClaw routes them with thread-aware session keys and preserves thread ID for replies.
...
<AccordionGroup>
  <Accordion title="Live stream preview (message edits)">
    CrewClaw can stream partial replies in real time:
    ...
    - DM: CrewClaw keeps the same preview message and performs a final edit in place (no second message)
    - group/topic: CrewClaw keeps the same preview message and performs a final edit in place (no second message)

    For complex replies (for example media payloads), CrewClaw falls back to normal final delivery and then cleans up the preview message.

    Preview streaming is separate from block streaming. When block streaming is explicitly enabled for Telegram, CrewClaw skips the preview stream to avoid double-streaming.

    If native draft transport is unavailable/rejected, CrewClaw automatically falls back to `sendMessage` + `editMessageText`.
  ...
  <Accordion title="Formatting and HTML fallback">
    ...
    - If Telegram rejects parsed HTML, CrewClaw retries as plain text.
...
```
  ...
  <Accordion title="Forum topics and thread behavior">
    ...
    **Thread-bound ACP spawn from chat**:
    ...
    - CrewClaw pins the spawn confirmation message in-topic after a successful bind.
...
```
  ...
  <Accordion title="Reaction notifications">
    ...
    When enabled, CrewClaw enqueues system events like:
...
```
  ...
  <Accordion title="Exec approvals in Telegram">
    ...
    Channel delivery shows the command text in the chat, so only enable `channel` or `both` in trusted groups/topics. When the prompt lands in a forum topic, CrewClaw preserves the topic for both the approval prompt and the post-approval follow-up.
...
<AccordionGroup>
  ...
  <Accordion title="Polling or network instability">
    ...
    - If logs include `TypeError: fetch failed` or `Network request for 'getUpdates' failed!`, CrewClaw now retries these as recoverable network errors.
...
- Multi-account precedence:
  ...
  - If neither is set, CrewClaw falls back to the first normalized account ID and `openclaw doctor` warns.
