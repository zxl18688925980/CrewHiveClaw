...
CrewClaw can convert outbound replies into audio using ElevenLabs, OpenAI, or Edge TTS.
It works anywhere CrewClaw can send audio; Telegram gets a round voice-note bubble.
...
Edge TTS does **not** require an API key. If no API keys are found, CrewClaw defaults
...
- If `provider` is **unset**, CrewClaw prefers `openai` (if key), then `elevenlabs` (if key),
  otherwise `edge`.
...
- **Edge TTS**: uses `edge.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Edge output format fails, CrewClaw retries with MP3.

OpenAI/ElevenLabs formats are fixed; Telegram expects Opus for voice-note UX.

## Auto-TTS behavior

When enabled, CrewClaw:
...
Discord note: `/tts` is a built-in Discord command, so CrewClaw registers
