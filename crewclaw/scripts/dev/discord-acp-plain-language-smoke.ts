...
function usage(): string {
  return (
    ...
    "1) CrewClaw spawned an ACP thread binding\n" +
    ...
    "  --openclaw-bin <path>        CrewClaw CLI binary for driver=openclaw (default: openclaw)\n" +
  ...
  try {
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from CrewClaw.`,
        diagnostics: {
          bindingCandidates: [
            {
