...
If you believe you've found a security issue in CrewClaw, please report it privately.
...
- Tested version details (CrewClaw version and/or commit SHA).
...
- Demonstrated impact tied to CrewClaw's documented trust boundaries.
- For exposed-secret reports: proof the credential is CrewClaw-owned (or grants access to CrewClaw-operated infrastructure/services).
...
**Jamieson O'Reilly** ([@theonejvo](https://twitter.com/theonejvo)) is Security & Trust at CrewClaw. Jamieson is the founder of [Dvuln](https://dvuln.com) and brings extensive experience in offensive security, penetration testing, and security program development.

## Bug Bounties

CrewClaw is a labor of love. There is no bug bounty program and no budget for paid reports. Please still disclose responsibly so we can fix issues quickly.
...
CrewClaw does **not** model one gateway as a multi-tenant, adversarial user boundary.
...
- CrewClaw can technically run multiple gateway instances on one machine, but recommended operations are clean separation by trust boundary.
- Recommended mode: one user per machine/host (or VPS), one gateway for that user, and one or more agents inside that gateway.
- If multiple users need CrewClaw, use one VPS (or host/OS user boundary) per user.
...
- This is expected in CrewClaw's one-user trusted-operator model. If you need isolation, enable sandbox mode (`non-main`/`all`) and keep strict tool policy.

## Trusted Plugin Concept (Core)

Plugins/extensions are part of CrewClaw's trusted computing base for a gateway.
...
- Using CrewClaw in ways that the docs recommend not to
...
- Exposed secrets that are third-party/user-controlled credentials (not CrewClaw-owned and not granting access to CrewClaw-operated infrastructure/services) without demonstrated CrewClaw impact
- Reports whose only claim is host-side exec when sandbox runtime is disabled/unavailable (documented default behavior in the trusted-operator model), without a boundary bypass.
- Reports whose only claim is that a platform-provided upload destination URL is untrusted (for example Microsoft Teams `fileConsent/invoke` `uploadInfo.uploadUrl`) without proving attacker control in an authenticated production flow.

## Deployment Assumptions

CrewClaw security guidance assumes:

- The host where CrewClaw runs is within a trusted OS/admin boundary.
...
CrewClaw's security model is "personal assistant" (one trusted operator, potentially many agents), not "shared multi-tenant bus."
...
CrewClaw separates routing from execution, but both remain inside the same operator trust boundary:
...
- Exec approvals bind exact command/cwd/env context and, when CrewClaw can identify one concrete local script/file operand, that file snapshot too. This is best-effort integrity hardening, not a complete semantic model of every interpreter/runtime loader path.
...
- Plugins can execute with the same OS privileges as the CrewClaw process.
- Runtime helpers (for example `runtime.system.runCommandWithTimeout`) are convenience APIs, not a sandbox boundary.
- Only install plugins you trust, and prefer `plugins.allow` to pin explicit trusted plugin ids.

## Temp Folder Boundary (Media/Sandbox)

CrewClaw uses a dedicated temp root for local media handoff and sandbox-adjacent temp artifacts:
...
- Sandbox media validation allows absolute temp paths only under the CrewClaw-managed temp root.
- Arbitrary host tmp paths are not treated as trusted media roots.
- Plugin/extension code should use CrewClaw temp helpers (`resolvePreferredCrewClawTmpDir`, `buildRandomTempFilePath`, `withTempDownloadPath`) rather than raw `os.tmpdir()` defaults when handling media files.
...
CrewClaw's web interface (Gateway Control UI + HTTP endpoints) is intended for **local use only**.

- Recommended: keep the Gateway **loopback-only** (`127.0.0.1` / `::1`).
  - Config: `gateway.bind="loopback"` (default).
  - CLI: `openclaw gateway run --bind loopback`.
- `gateway.controlUi.dangerouslyDisableDeviceAuth` is intended for localhost-only break-glass use.
  - CrewClaw keeps deployment flexibility by design and does not hard-forbid non-local setups.
...
CrewClaw requires **Node.js 22.12.0 or later** (LTS). This version includes important security patches:
...
When running CrewClaw in Docker:
