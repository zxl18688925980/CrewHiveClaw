import type { AnyAgentTool, CrewClawPluginApi } from "openclaw/plugin-sdk/feishu";
...
export function createToolFactoryHarness(cfg: CrewClawPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<CrewClawPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
