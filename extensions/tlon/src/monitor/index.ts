import type { RuntimeEnv, ReplyPayload, CrewClawConfig } from "openclaw/plugin-sdk/tlon";
...
export async function monitorTlonProvider(opts: MonitorTlonOpts = {}): Promise<void> {
  ...
  const cfg = core.config.loadConfig() as CrewClawConfig;
      ...
      if (senders.size > 0 && !senders.has(senderShip)) {
        ...
        runtime.log?.(
          `[tlon] ⚠️ SECURITY: Multiple users sharing DM session. ` +
            `Configure "session.dmScope: per-channel-peer" in CrewClaw config.`,
        );
        ...
        if (!sharedSessionWarningSent && effectiveOwnerShip) {
          ...
          const warningMsg =
            ...
            `Fix: Add to your CrewClaw config:\n` +
