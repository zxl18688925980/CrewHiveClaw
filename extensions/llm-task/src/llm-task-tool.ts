...
// NOTE: This extension is intended to be bundled with CrewClaw.
// When running from source (tests/dev), CrewClaw internals live under src/.
// When running from a built install, internals live under dist/ (no src/ tree).
// So we resolve internal imports dynamically with src-first, dist-fallback.
import type { CrewClawPluginApi } from "openclaw/plugin-sdk/llm-task";
...
export function createLlmTaskTool(api: CrewClawPluginApi) {
  return {
    name: "llm-task",
    label: "LLM Task",
    description:
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(
          path.join(resolvePreferredCrewClawTmpDir(), "openclaw-llm-task-"),
        );
