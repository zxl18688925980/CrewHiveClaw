...
import type { CrewClawPluginApi, CrewClawPluginService } from "openclaw/plugin-sdk/phone-control";
...
function normalizeDenyList(cfg: CrewClawPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: CrewClawPluginApi["config"]): string[] {
...
export default function register(api: CrewClawPluginApi) {
  ...
  const timerService: CrewClawPluginService = {
