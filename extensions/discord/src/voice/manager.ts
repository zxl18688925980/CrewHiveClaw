...
function resolveVoiceTtsConfig(params: { cfg: CrewClawConfig; override?: TtsConfig }): {
  cfg: CrewClawConfig;
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredCrewClawTmpDir(), "discord-voice-"));
