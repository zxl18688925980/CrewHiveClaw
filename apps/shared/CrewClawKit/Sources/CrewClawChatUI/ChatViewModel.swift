...
private let chatUILogger = Logger(subsystem: "ai.openclaw", category: "CrewClawChatUI")

@MainActor
@Observable
public final class CrewClawChatViewModel {
    public static let defaultModelSelectionID = "__default__"

    public private(set) var messages: [CrewClawChatMessage] = []
    public var input: String = ""
    public private(set) var thinkingLevel: String
    public private(set) var modelSelectionID: String = "__default__"
    public private(set) var modelChoices: [CrewClawChatModelChoice] = []
    public private(set) var isLoading = false
    public private(set) var isSending = false
    public private(set) var isAborting = false
    public var errorText: String?
    public var attachments: [CrewClawPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [CrewClawChatPendingToolCall] = []
    public private(set) var sessions: [CrewClawChatSessionEntry] = []
    private let transport: any CrewClawChatTransport
    private var sessionDefaults: CrewClawChatSessionsDefaults?
    ...
    private var pendingToolCallsById: [String: CrewClawChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        sessionKey: String,
        transport: any CrewClawChatTransport,
    ...
    public var sessionChoices: [CrewClawChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        let mainSessionKey = self.resolvedMainSessionKey

        var result: [CrewClawChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: CrewClawPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [CrewClawChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: CrewClawChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: CrewClawChatMessage) -> CrewClawChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> CrewClawChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return CrewClawChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageIdentityKey(for message: CrewClawChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [CrewClawChatMessage],
        incoming: [CrewClawChatMessage]) -> [CrewClawChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [CrewClawChatMessage]) -> [CrewClawChatMessage] {
        var result: [CrewClawChatMessage] = []
    ...
    private static func dedupeKey(for message: CrewClawChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [CrewClawChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> CrewClawChatAttachmentPayload in
            CrewClawChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> CrewClawChatSessionEntry {
        ...
        syncSelection: Bool)
    {
        if let index = self.sessions.firstIndex(where: { $0.key == sessionKey }) {
            ...
            self.sessions[index] = CrewClawChatSessionEntry(
                key: current.key,
                kind: current.kind,
    ...
    private func handleTransportEvent(_ evt: CrewClawChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: CrewClawChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: CrewClawAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = CrewClawChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> CrewClawPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
