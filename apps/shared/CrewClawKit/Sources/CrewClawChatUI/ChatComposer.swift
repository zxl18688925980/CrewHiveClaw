...
@MainActor
struct CrewClawChatComposer: View {
    private static let menuThinkingLevels = ["off", "low", "medium", "high"]

    @Bindable var viewModel: CrewClawChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(CrewClawChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(CrewClawChatTheme.composerBorder, lineWidth: 1))
    ...
    private var modelPicker: some View {
        ...
        {
            Text(self.viewModel.defaultModelLabel).tag(CrewClawChatViewModel.defaultModelSelectionID)
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \CrewClawPendingAttachment.id)
                { (att: CrewClawPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            CrewClawPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if !self.isComposerCompacted {
                Rectangle()
                    .fill(CrewClawChatTheme.divider)
                    .frame(height: 1)
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(CrewClawChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(CrewClawChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(CrewClawChatTheme.subtleCard)
        .clipShape(Capsule())
    }
