...
struct ChatMessageBubble: View {
    let message: CrewClawChatMessage
    let style: CrewClawChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? CrewClawChatTheme.userText : CrewClawChatTheme.assistantText
    ...
    private var inlineAttachments: [CrewClawChatMessageContent] {
    ...
    private var toolCalls: [CrewClawChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [CrewClawChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? CrewClawChatTheme.userBubble
        }
        if self.style == .onboarding {
            return CrewClawChatTheme.onboardingAssistantBubble
        }
        return CrewClawChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return CrewClawChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: CrewClawChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? CrewClawChatTheme.userText : CrewClawChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: CrewClawChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(CrewClawChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [CrewClawChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: CrewClawChatTheme.assistantText)
            }
        }
    }
}
