...
@MainActor
final class CrewClawAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
...
enum WatchPromptNotificationBridge {
    ...
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: CrewClawWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> CrewClawWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return CrewClawWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "CrewClaw" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [CrewClawWatchAction]) -> [UNNotificationAction] {
...
@main
struct CrewClawApp: App {
    @State private var appModel: NodeAppModel
    @State private var gatewayController: GatewayConnectionController
    @UIApplicationDelegateAdaptor(CrewClawAppDelegate.self) private var appDelegate
...
extension CrewClawApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("CrewClaw: installing uncaught exception handler")
