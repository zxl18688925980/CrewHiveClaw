...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> CrewClawDeviceStatusPayload {
        ...
        return CrewClawDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    func info() -> CrewClawDeviceInfoPayload {
        ...
        return CrewClawDeviceInfoPayload(
    ...
    private func batteryStatus() -> CrewClawBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: CrewClawBatteryState = switch device.batteryState {
        ...
        return CrewClawBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> CrewClawThermalStatusPayload {
        let state: CrewClawThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return CrewClawThermalStatusPayload(state: state)
    }

    private func storageStatus() -> CrewClawStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return CrewClawStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
