...
final class MotionService: MotionServicing {
    func activities(params: CrewClawMotionActivityParams) async throws -> CrewClawMotionActivityPayload {
        ...
        let mapped: [CrewClawMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return CrewClawMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: CrewClawPedometerParams) async throws -> CrewClawPedometerPayload {
        ...
        let payload: CrewClawPedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = CrewClawPedometerPayload(
