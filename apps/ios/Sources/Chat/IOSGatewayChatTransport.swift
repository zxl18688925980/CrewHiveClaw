...
struct IOSGatewayChatTransport: CrewClawChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> CrewClawChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(CrewClawChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> CrewClawChatHistoryPayload {
        ...
        return try JSONDecoder().decode(CrewClawChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [CrewClawChatAttachmentPayload]) async throws -> CrewClawChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [CrewClawChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(CrewClawChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(CrewClawGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: CrewClawGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: CrewClawChatEventPayload.self)
                        {
