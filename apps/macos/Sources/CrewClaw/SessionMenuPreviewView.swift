...
enum SessionMenuPreviewLoader {
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> CrewClawSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: CrewClawSessionPreviewEntry,
    ...
    private static func cache(payload: CrewClawSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: CrewClawChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [CrewClawKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [CrewClawKit.AnyCodable]) -> [CrewClawChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(CrewClawChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: CrewClawChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: CrewClawChatMessage) -> Bool {
    ...
    private static func toolNames(for message: CrewClawChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: CrewClawChatMessage) -> String? {
