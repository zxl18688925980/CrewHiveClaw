...
struct MacGatewayChatTransport: CrewClawChatTransport {
    ...
    func listSessions(limit: Int?) async throws -> CrewClawChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        let decoded = try JSONDecoder().decode(CrewClawChatSessionsListResponse.self, from: data)
        let mainSessionKey = await GatewayConnection.shared.cachedMainSessionKey()
        let defaults = decoded.defaults.map {
            CrewClawChatSessionsDefaults(
                model: $0.model,
            ...
            contextTokens: nil,
            mainSessionKey: mainSessionKey)
        return CrewClawChatSessionsListResponse(
            ts: decoded.ts,
            path: decoded.path,
            count: decoded.count,
            defaults: defaults,
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> CrewClawChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                CrewClawGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    CrewClawAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
    ...
    private static func mapModelChoice(_ model: CrewClawProtocol.ModelChoice) -> CrewClawChatModelChoice {
        CrewClawChatModelChoice(
            modelID: model.id,
            name: model.name,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<CrewClawChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any CrewClawChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = CrewClawChatViewModel(
            sessionKey: sessionKey,
            transport: transport,
        ...
        self.hosting = NSHostingController(rootView: CrewClawChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<CrewClawChatView>) -> NSViewController
    {
