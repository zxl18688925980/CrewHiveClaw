...
enum CrewClawLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = CrewClawOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = CrewClawFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        CrewClawLogging.bootstrapIfNeeded()
        let label = CrewClawLogging.makeLabel(subsystem: subsystem, category: category)
...
struct CrewClawOSLogHandler: AppLogLevelBackedHandler {
...
struct CrewClawFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = CrewClawLogging.parseLabel(self.label)
