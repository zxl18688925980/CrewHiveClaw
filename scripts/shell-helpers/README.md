...
Inspired by Simon Willison's [Running CrewClaw in Docker](https://til.simonwillison.net/llms/openclaw-docker).
...
On first command, ClawDock auto-detects your CrewClaw directory:
...
| `clawdock-cli <command>`  | Run CrewClaw CLI commands                      |
...
| `clawdock-cd`        | Jump to the CrewClaw project directory    |
| `clawdock-config`    | Open the CrewClaw config directory        |
...
2. Configure it in the CrewClaw config
...
- Bash or Zsh shell
- CrewClaw project (from `docker-setup.sh`)
